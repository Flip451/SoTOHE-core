...
- Treat batch composition as a work-conserving scheduling problem over the declared dependency DAG. Tasks are eligible to share a batch when each declared predecessor is in an earlier batch or appears earlier in that batch's implementation order, their described write targets do not overlap, and each per-scope estimate sum remains within its resolved ceiling or satisfies the structural gate's sole-contributor exception for a valid indivisible over-ceiling task. Among eligible compositions, minimize the number of ordered batches. Never infer a conflict merely because two tasks touch the same review scope; require an overlapping write target or another concrete interference.
