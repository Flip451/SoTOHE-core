...
2. **This capability definition** (v4 schema reference + action semantics + sample JSON in `## Catalogue Pattern Cookbook`) — authoritative for JSON structure, action semantics, and evaluator / codec behavior
...
- Pick the correct `role` value (from the per-section role space — see the **v4 Schema Reference** below) and the `kind` discriminator (`struct` with `shape` `unit`/`tuple`/`plain`, `enum`, or `type_alias`)
...
The specialist owns each `<layer>-types.json` and its derived views for this track, executed in the canonical order **baseline → catalogue → signals → views**:
...
## v4 Schema Reference (concise)

Catalogue files for this workspace use **`schema_version: 4`** — a 2-axis structure that separates the architectural **role** (DDD / Clean Architecture intent) from the language-level **kind** (Rust syntactic form). The top-level document is **3 BTreeMaps** (one per item kind) plus **2 top-level arrays** that hold impl blocks as independent entries:
...
This section is a derived reference for the v4 catalogue schema fields enumerated below. The canonical SSoT is the source code under `libs/domain/src/tddd/catalogue_v2/` — specifically `CatalogueDocument`, `TypeEntry`, `TraitEntry`, `FunctionEntry`, `TraitImplDeclV2`, and `InherentImplDeclV2`. If you suspect this reference is out of step with what `bin/sotp` actually accepts, read the source definitions and raise it as an Open Question rather than guessing.
...
`role` MUST be one of the **14 type-section role values**, written in **discriminated-object form** because `DataRole` is now a data-carrying enum:
...
| `Specification` / `Factory` / `Interactor` / `Command` / `Query` / `Dto` / `ErrorType` / `SecondaryAdapter` | `{ "<Variant>": {} }` | unit variants — always write the empty object payload |
...
└── TypeEntry — pick role first, then kind
    ...
    role (DDD / Clean Architecture intent) — one of the 14 type-section role values:
...
> **Schema Reference takes precedence.** The `role` values in the cookbook examples below are shown in the **legacy plain-string form** (`"role": "ValueObject"`, `"role": "SecondaryPort"`, etc.) retained for brevity. The codec no longer accepts this form — the normative wire format is the **discriminated-object form** defined in the **§ v4 Schema Reference** above (e.g. `"role": { "ValueObject": {} }`). When writing or editing catalogue entries, always follow the § v4 Schema Reference, not the cookbook literals.
...
1. Every entry under `types: { ... }` has `role:` set to one of the 14 type-section role values. Using a trait-section or function-section role triggers parse-time failure.
