...
- **Writes forbidden**: any other track's artifacts, other capabilities' SSoT files (`<layer>-types.json`, `impl-plan.json`, `task-coverage.json`, `metadata.json`), `plan.md`, any file under `knowledge/adr/` or `knowledge/conventions/`, any source code.
