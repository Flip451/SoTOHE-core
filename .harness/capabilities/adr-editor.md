...
- **Pre-merge draft vs post-merge record** (see `knowledge/conventions/adr.md` § Lifecycle). An ADR is **immutable only after it has landed on `main`**. Before that — while the ADR file still lives exclusively on a working branch / open PR — it is a draft and the specialist should amend it in-place when the briefing identifies a design flaw, missing constraint, or semantic contradiction. Do NOT create a new superseding ADR merely to fix a pre-merge draft; that is ceremony overhead.
