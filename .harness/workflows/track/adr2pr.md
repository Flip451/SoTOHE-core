...
- **Autonomy level** — this workflow is fully autonomous except for the mandated Phase 0
  ...
  Accepted Deviations in Step 10 (an inherited pause, not an adr2pr addition). The
  invocation-time input acquisition above interacts with the user before Step 1 begins and is
  outside this autonomy accounting. All commit messages are generated by the workflow.
  Irrecoverable hard failures also stop the workflow.
...
5. **Phase 0 approval exception**: after the Step 2 ADR-baseline review reaches `zero_findings`
   ...
   loop may terminate. adr2pr adds no pause beyond these two; the pre-Step-1 input
   acquisition (Inputs) happens before the sequence begins and is outside this pause
   accounting.
