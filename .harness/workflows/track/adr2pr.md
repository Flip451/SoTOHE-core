...
- **Autonomy level** — this workflow is fully autonomous except for the mandated Phase 0
  ...
  All commit messages are generated by the workflow. The Phase 0 exception is the only routine
  user pause; irrecoverable hard failures also stop the workflow.
...
Run this terminal phase only after Step 10 reaches its explicit zero-findings terminal state
(machine PASS or user-approved Accepted Deviations). Retain the direct primary-ADR filename
...
5. **Phase 0 approval exception**: after the Step 2 ADR-baseline review reaches `zero_findings`,
   ...
   ADR baseline. This is the sole mandated exception to Constraint 2.
