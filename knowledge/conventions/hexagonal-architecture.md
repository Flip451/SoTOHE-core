...
| アプリケーションサービスが必要とするポート | **usecase** | `ReviewHasher`, `Reviewer`, `DiffGetter` |
...
pub fn check_approved<R, H, D>(
    cycle: &ReviewCycle<R, H, D>,          // usecase aggregate
    review_reader: &impl ReviewReader,      // domain port (review_v2)
...
where
    ...
    D: DiffGetter,
{
    cycle.get_review_states(review_reader)
}
```
