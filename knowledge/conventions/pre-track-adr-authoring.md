...
ADR は track 内成果物ではなく **track 前段階** で author が作成する track 横断資産とする。track のライフサイクル (作成 → 進行 → 完了 → archive) と ADR のライフサイクル (書き換え可だが原則安定、全 track で参照される) を独立に保つため、`/track:plan` の入力となる ADR の**初期生成**を内部ステップから切り離し、事前確認のみを行う。Phase 1+ で必要になった track-born ADR draft はこの初期入力とは別の正規経路である。
...
- **作成タイミング**: `/track:plan` の入力となる ADR ファイルの **初期起草** (knowledge/adr/ への file 作成) は `/track:plan` **起動前** に user + main 対話 (または手動) で完了させる。`/track:plan` はその入力 ADR を自動生成しない。これは Phase 1+ の signal / gate が必要とした track-born 新 ADR draft を妨げない。その draft は adr-editor が作成し、user 裁定まで非 user 系根拠のまま 🟡 で保持する。**初回 commit はその ADR を最初に必要とする track の `/track:init` 直後に `/track:review` → `/track:commit` で行う**のが typical (この時点で ADR commit 履歴ありとなり、後続の `/track:plan` の back-and-forth `adr-editor` ループが「commit 履歴あり」 path で動作する)。ADR は track 横断資産であり 1 つの ADR が複数 track にまたがる関係 (ADR ⊇ track) を維持する。
...
- **back-and-forth 自動修正 (adr-editor)**: `/track:plan` の探索的精緻化ループで下流 signal が 🔴 になって ADR 側の修正が必要になった場合、adr-editor サブエージェントが ADR を working tree レベルで編集する (正規経路の全体像と刻印の扱いは下記「In-track 意味変更の裁定権」節)。in-place 編集の対象は effective merge target 判定 (`knowledge/conventions/adr.md`) で **pre-merge** の ADR に限る。新 decision が必要な場合、既存 ADR は編集せず、adr-editor が別の track-born ADR draft を作成する。merge 済み ADR が意味編集を要する場合もこの新 ADR 化に従う。
  - 既存 ADR に commit 履歴あり (かつ pre-merge) → auto-edit (working tree のみ、loop 中は commit しない)
  - 既存 ADR に commit 履歴なし → user pause (ADR を先に commit してから再開)
  - track-born ADR draft → adr-editor が working tree で起草・改稿する。user 裁定前の draft
    には commit 履歴も baseline 刻印も要求せず、同期 pause もしない。
- **終端処理**: `/track:plan` 終了時、Phase 1+ の正規 escalation 編集は既に刻印済みであり、track-born ADR draft は 🟡 のまま PR merge 監査へ進む。ADR diff があることだけを理由に同期の accept / revert / 手動修正を求めない。経路不明の baseline 乖離だけは fail-closed の不一致トリアージへ送る。
...
| 役割 | 担い手 | 責務 |
|---|---|---|
| 書き手 | adr-editor | 編集と許可された track-born ADR draft の作成・適用 (working tree のみ) |
| 守護者 | adr-diagnoser | **Phase 0 の baseline review ループ内の ADR 編集ごとに「元の決定を壊していないか」を判定する**。精緻化 (許容) と決定破壊 (不許可) の峻別はこの判定者に属する。決定を壊す提案には、**決定を保全する代案を示すか、修正が必要ない旨を理由付きで提示する義務**を負う (裸の差し止めで終わらない)。Phase 1+ では経路不明の baseline 乖離だけをトリアージする |
...
- **正規の意味変更経路は signal / gate 駆動の escalation ループのみ** (Phase 1-3 の grounding escalation、impl 以降の diagnose ルーティングが adr を指した場合等)。既存 ADR の編集と、必要な track-born ADR draft の起草は adr-editor が行う。
- **正規ループ内の編集は「予期された編集」であり、守護者判定を前提としない**: orchestrator 自身が escalation ループで adr-editor に指示した編集は、編集直後に escalation 種別の刻印 (自己完結の reason 必須、orchestrator が起動する原子的機械コマンドによる書込) へ直行する。adr-diagnoser のトリアージは下記「不意の変更」(経路不明の baseline 乖離) の専用経路であり、予期された編集の採用条件ではない。判定系 capability は read-only のまま、baseline への書込面は機械コマンド 1 本に集約される。
- **情報の精緻化** (決定を覆さない補強・明確化) は正規ループ内で許容されうる。ADR は決定を記録するものであり設計詳細に立ち入らないため、精緻化が意味を持つ場面は稀である。
- **新しい decision の裁定は同期 pause を要しない**: 正規ループ外からの意味変更提案 (レビュー所見としての選好・別案・scope 論等) や、escalation が新規 decision の起草に至る場合に対して、orchestrator は**自律裁定しない**。かつ**走行中の pipeline を user 確認のために止めない** — adr-editor が別の track-born ADR draft を作成し、非 user 系根拠 (`review_finding_ref` 等) のまま記録すれば、chain ⓪ が 🟡 と評価し、strict merge gate が **merge 前の user 裁定を機械的に強制**する (裁定は merge 段階の PR review で非同期に届けば足りる)。user 裁定が届いた時点で、orchestrator は adr-editor を dispatch して根拠を `user_decision_ref` へ昇格させ、その直後に track-born の新 ADR を working tree から刻印する (kind: new-adr、reason 必須) — 以後は通常の凍結対象となる。
- **不意の変更** (経路不明の baseline 乖離) は byte 照合で検出し、adr-diagnoser が分類する — これが守護者の持ち場である。「意味を変えない」ことの要求がこの検出機構の目的であり、意味を変えない差分のみ再刻印できる (kind: non-semantic-fix)。意味的な乖離は baseline を復元して経緯を発生元 briefing へ差し戻し、判断に迷う場合は逸脱側に倒す (fail-closed)。

### 機構との整合

本節が規範の正であり、機構は以下の形で追随している:

- `adr-baseline check-review` — review 入口の ADR 状態検査は「init 刻印の存在確認と帳簿完全性」のみであり、byte 照合の発火点は commit gate / CI に限られる (決定: `knowledge/adr/` の該当 ADR)。専用の承認 kind は存在しない。承認の記録は、ループで修正した decision 自身の front-matter `user_decision_ref` への承認 ref 追記が担い (chain ⓪ が検証)、その後の escalation 刻印の reason には自己完結の欠落説明だけを記す (承認の ledger 重複記録なし)。primary ADR の escalation 刻印では、この reason に origin-input の由来と要旨（少なくとも local review round / 外部 PR review round / spec→ADR の 🔴 signal / diagnose routing を区別）を含め、guardian verdict が実在する場合（Phase 0 裁定由来の stamp 等）のみその要旨を併記する — 予期された escalation 編集は verdict を持たないのが正常である。これは自己完結の欠落説明を具体化するものであり、user 承認の ledger 重複ではない。編集なしで収束した場合は追加の刻印を行わない。
- 守護者判定の workflow 配線 — capability 定義 (`.harness/capabilities/adr-diagnoser.md`) が編集判定モード (決定保存 / 決定破壊 + 保全代案または修正不要理由) を定義する。この判定が編集採用の前提となるのは **Phase 0 の baseline review ループ内**であり、review / plan / adr2pr の workflow SSoT はその範囲で「編集ごとに守護者判定を挟み、その出力をレビュアーへ還流させる」手順を刻む。Phase 1 以降の grounding escalation 編集は予期された編集として守護者判定を経ず escalation 刻印へ直行し、adr-diagnoser の残る持ち場は経路不明の baseline 乖離のトリアージ (非意味的再刻印 / 復元 / fail-closed) である。
...
- Good: Phase 1 (spec) で signal 🔴 発生 → `/track:plan` が adr-editor を自動 invoke (ADR に commit 履歴あり) → working tree 編集 → escalation 刻印 → loop 再開。新 decision が必要なら adr-editor が別の track-born ADR draft を作成し、🟡 のまま merge 監査へ進める。（Phase 2 の 🔴 は spec-designer が再 invoke される。adr-editor が呼ばれるのは Phase 1 信号が ADR 側の修正を要求する場合のみ。）
...
- Good: **Phase 0 の**レビュー所見が決定変更を要求 → adr-diagnoser が決定破壊と判定し保全代案を提示 → orchestrator が判定と代案をレビュアーへ伝達、adr-editor が代案を適用 → 再レビューで収束。レビュアーが所見を維持した場合はその対立を Phase 0 エスカレーションへ運ぶ。
- Bad: `/track:plan "feature X"` を実行、Phase 0 の入力 ADR を内部で自動生成 (spec に合わせて decision を後付けする rationalization の温床になる)。
...
- Bad: Phase 0 の baseline review ループ内で、adr-editor の編集を adr-diagnoser の決定保存判定を通さずに採用する (守護者の不在。精緻化か決定破壊かを orchestrator が自己裁定することになる)。
- Bad: Phase 1 以降の grounding escalation 編集に守護者判定や同期 user 裁定を追加要求して pipeline を停止させる (予期された編集は escalation 刻印へ直行し、新規 decision の裁定は 🟡 + strict merge gate の非同期経路が担う)。
...
- [ ] Phase 0 の baseline review ループ内の ADR 編集ごとに adr-diagnoser の決定保存判定を通しているか (守護者を迂回していないか)
- [ ] Phase 1+ の予期された escalation 編集を guardian 判定に戻さず、reason 必須の escalation 刻印へ直行させているか
