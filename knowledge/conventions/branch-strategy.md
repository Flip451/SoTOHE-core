...
各トラックの実装作業は専用のフィーチャーブランチ `track/<track-id>` で行い、`main` への直接変更は避け、PR ベースのマージワークフローを採用する。これにより複数トラックの並列開発と main の安定性を両立させ、レビュー履歴と CI 結果を PR 単位で残す。過去の plan-only lane で使われた `plan/<id>` ブランチは現行の自動解決対象ではない。直接の `git merge` / `git rebase` / `git cherry-pick` / `git reset` / `git switch` はガードフックでブロックし、ブランチ操作は必ずワークフローラッパーを経由させる。

## Scope

- 適用対象: `track/<id>` 実装ブランチの PR/ガード方針、トラックブランチの作成・切り替え・マージ・PR レビュー・ブランチ操作ガード。`/track:*` コマンド内のブランチ関連ステップ、`cargo make track-branch-*` / `cargo make track-pr-*` ラッパー、`bin/sotp pr ensure-pr`、ブランチガードフック。
...
- `main` ブランチにいる場合: branch 由来の自動解決は行わず `NotTrackBranch` として fail-closed する。READ 系 subcommand が explicit `--track-id` / 引数を定義している場合のみ対象トラックを明示して実行でき、WRITE 系は `track/<id>` ブランチ上で実行する。
...
タスクの `done` 遷移とコミットは必ず track ブランチ上で行う。マージ後の main ではブランチガードにより状態遷移がブロックされる。
...
- Good: `/track:plan <feature>` でトラックを開始すると `track/<track-id>` ブランチが自動作成され、すべての commit / push / PR 作成がそのブランチ上で行われる。
- Good: PR ベースレビューのみを使う運用で `bin/sotp review check-approved` が `review.json` 不在 + 全 scope `NotStarted` で exit 0 になり、commit ゲートを通過する。
- Bad: トラックの作業途中で `git switch main` を直接実行する（ガードフックでブロックされる。`cargo make track-branch-switch` 経由で別トラックに移る）。
...
- `main` 上での緊急 hotfix を想定する場合は別 ADR で取り扱う。本 convention 内ではガードフック越えのワークフローを定義しない。
