...
2. `bin/sotp pr push` でブランチを push する。
3. `bin/sotp pr ensure-pr` で PR を作成する。
4. `bin/sotp pr review-cycle` で `@codex review` をトリガーし、結果をポーリング・パースする。
5. CI が通過することを確認する。
...
1. `bin/sotp pr push` — トラックブランチを origin に push
2. `bin/sotp pr ensure-pr` — PR を作成 or 再利用
3. `bin/sotp pr review-cycle` — `@codex review` コメント投稿 → ポーリング → 結果パース
