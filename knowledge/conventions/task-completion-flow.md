...
2. 全タスクを done に遷移（commit_hash: None で OK）
   ...
   cargo make track-transition -- T002 done
   ...
...
4. commit_hash 埋め戻し（任意だが推奨）
   ...
   - cargo make track-transition -- T001 done --commit-hash <hash>
   - /track:commit (埋め戻しコミット)
5. /track:pr (ガードが全タスク done/skipped を検証)
6. /track:merge
7. /track:done (設定済みの base branch に切替、状態更新不要)
```
...
- Bad: マージ後に merge_target 上で metadata.json を編集してタスクを done に変更
...
- merge_target 上での直接 metadata 編集が含まれていないか
