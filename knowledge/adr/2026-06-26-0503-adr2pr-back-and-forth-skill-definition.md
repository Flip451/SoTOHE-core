...
**Cross-host artifact set (D8 推奨配置)**: capability の operational contract と invocation surface は既存 writer / verifier capability (`spec-designer` / `impl-planner` / `type-designer` / `adr-editor` 等) と同じ pattern で配布する必要がある。これは codex orchestrator parity (`capabilities.orchestrator.provider: codex` 時に同じ flow が動く保証) と Claude Code slash-command discoverability の双方を担保する。配置すべき artifact は以下 6 種:

- `.harness/capabilities/rollback-diagnoser.md` — provider-agnostic operational SSoT (routing taxonomy / mandatory context-file pre-read / output contract / boundary)。両ホストが読む単一の SSoT。
- `.claude/commands/track/diagnose.md` — Claude Code 用 slash command `/track:diagnose` spec (orchestrator-facing 薄ラッパー、`capabilities.rollback-diagnoser` へ dispatch)。
- `.claude/agents/rollback-diagnoser.md` — Claude subagent 用薄ラッパー (operational SSoT を参照)。
- `.claude/skills/diagnose/SKILL.md` — Claude skill registry 用薄ラッパー (operational SSoT を参照)。
- `.agents/skills/rollback-diagnoser/SKILL.md` — Codex skill wrapper (operational SSoT を参照)。
- `.codex/agents/rollback-diagnoser.toml` — Codex specialist agent TOML (operational SSoT を参照)。

この artifact set は既存 capability 全てが踏襲しているクロスホスト整合性パターン (`.claude/rules/08-orchestration.md` の "Codex root orchestration uses tracked `.codex` config, rules, hooks, agents, and repo-scoped `.agents/skills`") の延長であり、`rollback-diagnoser` だけ片方ホスト用しか配置しない設計選択は採用しない。加えて `.codex/instructions.md` の Specialist Routing section に `rollback-diagnoser` を追記し、codex orchestrator が capability の存在を認識できるようにする。soft prompt (D3) も `.harness/capabilities/rollback-diagnoser.md` を host-agnostic な path として参照する。
