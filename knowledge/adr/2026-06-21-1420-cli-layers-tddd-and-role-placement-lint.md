...
両者を分けるのは、`1328` が wire と invoke を層レベルで分離したことの型レベルの反映である（wire と invoke は別責務）。一方 invoke と render は primary adapter の双方向変換の表裏なので分けず、`PrimaryAdapter` が両方を担う。`PrimaryAdapter` 単独で wire まで兼ねると composition root と driving adapter の責務混在になり canonical でない（web の DI コンテナ＝composition root と Controller＝primary adapter の分離に対応）。`CompositionRoot` は `KindLayerConstraint`（cli_composition のみ）、`PrimaryAdapter` は `KindLayerConstraint`（cli_driver のみ）+ `NoRoleInMethodSignature`（domain 固有ロール型と usecase 内部実装ロール型（`SecondaryPort` / `UseCase` / `Interactor` / `Command` / `Query`）を公開 signature に出さない；ただし `ApplicationService` は PrimaryAdapter が DIP 上で依存すべき application-level contract であるため明示的に除外）という独自の機械検査を持つため、「ロールは独自検査を持つ場合のみ追加」原則を満たす。
...
[
  ...
  { "target_roles": ["Dto"], "kind": { "KindLayerConstraint": { "permitted_layers": ["usecase", "infrastructure", "cli", "cli_driver"] } } },
  { "target_roles": ["FreeFunction"], "kind": { "KindLayerConstraint": { "permitted_layers": ["domain", "usecase", "infrastructure", "cli", "cli_driver"] } } },
  { "target_roles": ["PrimaryAdapter"], "kind": { "NoRoleInMethodSignature": { "forbidden_roles": ["Entity", "AggregateRoot", "ValueObject", "Repository", "SecondaryPort", "UseCase", "Interactor", "Command", "Query"] } } }
]
```

`KindLayerConstraint` と `NoRoleInMethodSignature` は**補完的**な不変条件を担う。前者は「ロールをどの層で宣言できるか（宣言場所制約）」、後者は「自身のメソッドシグネチャにどのロールの型を出してよいか（シグネチャ依存制約）」を機械的に保証する。`ApplicationService` については、宣言場所の観点では `KindLayerConstraint` の `permitted_layers: ["usecase"]` により `usecase` 層のみに宣言が制限されており、`cli_driver` が `ApplicationService` を宣言することはできない。一方、`cli_driver` PrimaryAdapter のシグネチャにおいては `ApplicationService` を**意図的に許可**する。これは hexagonal architecture の DIP 要件に基づく設計判断であり、`cli_driver` の `*Driver` は `Arc<dyn *Service>`（`usecase` 層で宣言された `ApplicationService` トレイト）を注入されてそれを保持・呼び出す primary adapter だからである。`NoRoleInMethodSignature.forbidden_roles` に残る他のロール（`SecondaryPort` / `UseCase` / `Interactor` / `Command` / `Query` と domain ロール群）の禁止は、PrimaryAdapter が `ApplicationService` を迂回して `SecondaryPort` や usecase 内部実装ロール（`UseCase` / `Interactor`）に直接依存したり、usecase 内部契約の DTO を bin 側シグネチャに漏洩させることを防ぐためである。ADR 1328 D4（改訂）は「ステップ数・pass-through に関わらず application logic はすべて `ApplicationService` を経由する」を要求しており、この非対称な lint 設定（宣言は `usecase` に固定、`cli_driver` からの依存は `ApplicationService` のみ許可）がその要求の機械的保証になる。
