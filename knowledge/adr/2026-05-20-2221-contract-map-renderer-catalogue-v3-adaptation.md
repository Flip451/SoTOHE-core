...
decisions:
  ...
  - id: O
    ...
    status: proposed
  - id: P
    user_decision_ref: "chat_segment:contract-map-v3-renderer-port-adapter-api:2026-05-21"
...
旧 ADR §4.5 の layer-agnostic 不変条件 (層名ハードコード禁止 / 層リストは `architecture-rules.json` 由来 / layer_order はトポロジカルソート) を本 ADR でも継承する。実装上の責務分担: `CatalogueLoader.load_all` が `architecture-rules.json` トポロジカルソート由来の `Vec<LayerId>` を返し、`RenderContractMapInteractor` がこれを `layer_order` 引数に変換して `ContractMapRenderer::render` に渡す。renderer 自身は `architecture-rules.json` を直接走査せず、渡された `layer_order` を消費するだけでよい。この分担により renderer は layer-agnostic 不変条件を満たしつつ infrastructure の設定ファイル読み込みから完全に切り離される。

## Decision

### A: renderer の入力 type (採択: A-3')

`ContractMapRenderer` port の render メソッドが受け取る入力の型:
...
**注**: 旧 IN-24 プレースホルダーとして存在していた domain 自由関数 `render_contract_map(...)` は削除される。port method (上記) がその役割を置き換え、実際のレンダリングは infrastructure adapter (`ContractMapRendererAdapter`) が行う (Decision P 参照)。自由関数はどこからも呼ばれない。
...
prefix で entry 種別を識別し、length-prefix を加えることで英数字・アンダースコアのみから成る layer / crate / type 名の構成において injective 性を維持する (sanitize による衝突の可能性については下記注意を参照):
...
1 layer に N crate が存在する構成 (ADR 2026-05-08-0248 D6 の 1 catalogue = 1 crate 原則) では同一 layer 内で同名の Type / Trait が異なる crate に宣言される可能性があるため、`sanitized_crate` を含めることで node_id の衝突を回避する。`sanitized_crate` は `doc.crate_name` を mermaid node_id 用に sanitize した値。Function は `sanitized_full_path` (FunctionPath 全体の sanitized 値) を `sanitized_name` に代えて使用するため、`<len>` もその全体長で計算する。

**注意**: sanitize は英数字・アンダースコア以外の文字を `_` に置換するため、`my-gateway` と `my_gateway` のように非英数字文字のみで異なる名前は同一の sanitized 値となり node_id が衝突する可能性がある。`domain` / `usecase` / `infrastructure` のような実際のアーキテクチャ構成 (英数字と `_` のみ) ではこの問題は発生しない。この制限は実装 phase で必要に応じて追加の衝突回避策 (例: 元文字列の hash suffix 付加) を検討してよい。
...
- nesting 深さ: TypeEntry / TraitEntry は layer → top-module → entry → method = 4 重。FunctionEntry は standalone callable node (subgraph 化なし) のため layer → top-module → FunctionEntry = 3 重
- 各 collection の iter 順: BTreeMap は alphabetical、Vec は declaration order

### P: port method 署名・Adapter 構造・RenderContractMapInteractor との関係 (採択: Option A — Adapter がフルレンダリングを担う)

**P-1: infrastructure Adapter がフル v3 スタイルレンダリングを担う**

v3 mermaid レンダリング (syn ベース TypeRef parse + `.harness/config/contract-map-style.toml` スタイル設定の適用) はすべて infrastructure 層の `ContractMapRendererAdapter` が行う。`ContractMapRendererAdapter` は domain 層の `ContractMapRenderer` port を実装する。

**P-2: domain 自由関数 `render_contract_map` は削除**

旧 IN-24 プレースホルダーとして存在していた domain 自由関数 `render_contract_map(...)` は削除される (修正ではなく削除)。どこからも呼ばれない。レンダリングのすべての実処理は `ContractMapRendererAdapter::render(...)` に移る。

**P-3: スタイル設定 DTO は Adapter のプライベートな実装詳細**

`.harness/config/contract-map-style.toml` を読み込む設定型 (TOML schema DTO 群) は `ContractMapRendererAdapter` の内部実装詳細であり、公開 API に露出しない。catalogue の公開コントラクトに含まれず、カタログ宣言の対象外。Adapter は `style_config_path` を受け取り (`new` constructor は infallible)、render 呼び出し時に設定を読み込む (fail-closed: ファイル不在は `ContractMapRendererError::StyleConfigNotFound`、TOML parse 失敗は `StyleConfigInvalid` を返す)。

**P-4: domain 層の境界**

本 track が domain 層に新規追加する (action: add) renderer 関連型は次の 2 つに限定される:

- `ContractMapRenderer` port (trait) — action: add
- `ContractMapRendererError` (error type) — action: add

以下の 2 型は先行 track で既に domain 層に存在しており、本 track では参照のみ (action: reference、変更なし) とする:

- `ContractMapContent` (レンダリング結果の値オブジェクト) — action: reference
- `ContractMapRenderOptions` (render 呼び出しオプション) — action: reference

既存の `CatalogueLoader` / `ContractMapWriter` 等の domain ポートはこれとは独立して存在し続ける (reference — 変更なし)。domain 層への serde / presentation / スタイル設定型の追加は行わない。

**P-5: RenderContractMapInteractor の構造**

`RenderContractMapInteractor` は `ContractMapRenderer` port をジェネリクス `R` で inject される。interactor は削除された自由関数の代わりに `self.renderer.render(catalogues, layer_order, opts)` を呼ぶ。

```rust
<!-- illustrative, non-canonical -->
pub struct RenderContractMapInteractor<R: ContractMapRenderer> {
    renderer: R,
    writer: /* ContractMapWriter port */,
}
```

**根拠**: スタイルを伴うレンダリングは presentation/adapter の関心事である。スタイル設定型を domain に置くと domain が serde 依存・presentation 依存になり hexagonal architecture の境界が崩れる。また、`ContractMapRenderOptions` しか受け取れない domain 自由関数がスタイル設定ファイルを適用できないという構造的矛盾が解消される。スタイル DTO を非公開にすることで TOML schema の変更が公開コントラクトに影響しない。
...
```
flowchart LR    ← diagram type 宣言 (先頭行、以下の構造が続く)
...
- **R: エラー処理 / fallback** — TypeRef parse 失敗、cross-catalogue lookup 失敗の error type と fallback 挙動 (実装 phase)。設定ファイル不正値 (TOML parse 失敗) は Decision P-3 で `ContractMapRendererError::StyleConfigInvalid { path, reason }` として確定済みであり、本 Open Question から除外済み
