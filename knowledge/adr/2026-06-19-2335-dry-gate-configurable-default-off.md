...
decisions:
  ...
  - id: D3
    ...
    status: proposed
  - id: D4
    user_decision_ref: "chat_segment:dry-gate-configurable-default-off:2026-06-21"
    status: proposed
---
...
### D4: dry-checker の reasoning effort 設定を `.harness/config/dry-check.json` から `.harness/config/agent-profiles.json` の `capabilities.dry-checker` に移す

`fast_reasoning_effort` と `final_reasoning_effort` の 2 フィールドは、「dry-checker というエージェント capability をどのモデル・推論強度で動かすか」という設定であり、`agent-profiles.json` の `capabilities.dry-checker` が管理するモデル選択（`model` / `fast_model`）と同じカテゴリに属する。これらを `dry-check.json` に置くのは責務境界として誤りである。`model` と `fast_model` が `agent-profiles.json` で管理されているのに、同じ capability の推論強度（reasoning effort）だけが `dry-check.json` に置かれているのは非対称であり、利用者が設定すべき場所を見つけにくい。

`2026-06-10-0413-dfp-rfp-loop-cost-reduction.md` D4 は「reasoning effort のハードコードを capability 設定へ昇格する」と述べたが、移先のファイル名（`agent-profiles.json` か `dry-check.json` か）を確定していなかった。後続トラックの実装がその記述を `dry-check.json` と解釈して実装したため、現在は `dry-check.json` に置かれている。本 D4 はその解釈が誤りであったことを確定し、正しい配置を明示する。

移動後のキー名は `fast_reasoning_effort` / `final_reasoning_effort` をそのまま用いる。`dry-check.json` からはこれら 2 フィールドを削除する。この削除は D2 で決定した `enabled` 追加と同一トラックの変更であり、「`enabled` 追加 + `reasoning_effort` 削除」を 1 回の schema migration として v4 にまとめる（誰も見ていない中間 schema を作る不要な churn を避けるため）。`agent-profiles.json` では `capabilities.dry-checker` オブジェクトに `fast_reasoning_effort` / `final_reasoning_effort` を追加する。
...
- `fast_reasoning_effort` / `final_reasoning_effort` を `dry-check.json` から `agent-profiles.json` へ移す（D4）。`dry-check.json` の schema_version は D2 の `enabled` 追加と同じ v4 に統合されており、設定ファイルの migration は 1 度だけ発生する。`DryCheckConfig` の loader、バリデーション、fingerprint、テストの変更が生じる。
...
- `knowledge/adr/2026-06-10-0413-dfp-rfp-loop-cost-reduction.md` — DFP⇄RFP コスト削減（本 ADR は別アプローチ＝既定無効化）。D4 の「reasoning effort を capability 設定へ昇格」の曖昧な移先を、本 ADR D4 が確定して解消する
