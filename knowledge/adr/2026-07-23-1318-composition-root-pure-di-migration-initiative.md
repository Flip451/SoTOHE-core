...
decisions:
  ...
  - id: D3
    status: proposed
    user_decision_ref: "chat_segment:current-task:2026-07-23:phase0-d3-d5-responsibility-adopted"
  - id: D4
    status: proposed
    user_decision_ref: "chat_segment:current-task:2026-07-23:composition-root-pure-di-migration-hearing"
  - id: D5
...
各 command 文脈 track は担当実行経路から互換 shim と中間 `*ServiceImpl` への参照・呼出し・逆委譲をすべて除去し、driver → usecase interactor → port の単一路を成立させた時点で完了できる。参照されなくなった互換定義・export の物理削除は Final convergence track が一括して担い、それまでは新規利用を禁止する。
