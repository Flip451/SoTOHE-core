---
adr_id: 2026-05-13-0000-contract-map-renderer-catalogue-v3-adaptation
decisions:
  - id: A
...
### §2 catalogue schema v3 への移行 (ADR 2026-05-08-0248)
...
- Type: `T<len>_<sanitized_layer>_<sanitized_name>`
- Trait: `R<len>_<sanitized_layer>_<sanitized_name>` (R for tRait)
- Function: `F<len>_<sanitized_layer>_<sanitized_full_path>` (FunctionPath 全体を sanitize)

`<len>` は `sanitized_layer` + `_` + `sanitized_name` の文字数 (同名 Type / Trait の id 衝突を排除)。
...
旧 `SecondaryAdapter.trait_impl_methods` 経由の edge (旧 ADR 2026-04-17-1528 §D4 (2)) は ADR 2026-05-08-0248 D10 (TraitImplDeclV2 identity-only) により廃止されたため、本 ADR では新 schema に基づく edge 設計に置き換える。
...
- 入力 `catalogues` (= `Vec<CatalogueDocument>`) を全走査して 1 回だけ構築
...
- `module_path[0]` が同じ entry を 1 subgraph に集約
- subgraph id: `<sanitized_layer>_module_<sanitized_module_path_first_segment>`
- subgraph label: `<layer>::<module_path_first_segment>` (例: `domain::review`)
...
2. layer subgraph 群 (layer_order 順)
   2.1. top-module subgraph 群 (module_path[0] alphabetical)
   ...
   2.2. crate root entry / crate root FunctionEntry (subgraph 外、alphabetical)
...
- catalogue v3 schema (TypeKindV2 5 variants + LayerId + Role 軸分離) と完全に整合する
...
- action / signal overlay の必要性が顕在化した場合 (本 ADR で YAGNI 判断)
