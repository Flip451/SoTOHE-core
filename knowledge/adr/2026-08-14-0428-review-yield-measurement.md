...
decisions:
  ...
  - id: D2
    ...
    status: proposed
  - id: D3
    user_decision_ref: "chat_segment:claude-session-01Ccnvmids61xDrsVyFkKVti:2026-08-14"
    status: proposed
---
# review-yield を計測する
...
### D3: 計測対象を `sotp review local` の構造化 review round に限定する

D1 の telemetry 記録は既存の structured-review-round authority に委譲し、計測対象と記録点を `sotp review local` が実行する各構造化 review round のみに限定する。

`sotp review local` 以外の入口である PR review cycle、DRY gate の裁定、test-obligation fulfillment の judge、および binary pre-review gate は本 ADR の計測対象外とする。
