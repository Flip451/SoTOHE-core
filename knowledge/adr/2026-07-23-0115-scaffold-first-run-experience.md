...
タスクは `git init -b main` ⇒ `cargo generate-lockfile` ⇒ `git add -A` ⇒ 初期コミット ⇒ `cargo make bootstrap` を順に実行する。
初期ブランチ名を `main` と明示することで、利用者環境の `init.defaultBranch` に依存せず、D2 の出荷既定と一致させる。
...
bootstrap の呼び出しだけでは、利用者がリポジトリ作成と初期コミットを選択したとはみなせず、それらを暗黙に実行すると利用者の意図を確認しない副作用になるため却下した。
