...
decisions:
  ...
  - id: D8
    ...
    status: proposed
  - id: D9
    user_decision_ref: "chat_segment:tddd-v2-eval-provenance-and-command-rename:2026-05-11"
    status: proposed
---
...
### D9: 3-way 構造一致比較はトレイト実装の由来を区別しない

Phase 2 (D3) の「構造一致」判定において、トレイト実装の生成方式 (`#[derive(...)]` によって自動生成されたものか、手書きの `impl` ブロックか) を区別しない。

同じトレイト名・同じ generic 引数を持つ実装は、`#[automatically_derived]` 等の provenance 属性の有無にかかわらず、構造的に等しいものとして扱う。

**根拠**: カタログ (A) は「どのトレイトが実装されているか」だけを宣言し、実装の由来 (derive か手書きか) を記録しない。実装側 (C) の比較もこのカタログの粒度に合わせるべきである。`#[derive(Default)]` を `impl Default for X { ... }` に書き直したり、その逆を行ったりする変更は、型の振る舞いを変えないボイラープレート削減であり、signal 機構がこれを 🔴 または 🟡 として報告するのは逆効果である。

**既存の signal table との関係**: D3 の 11 領域テーブルと signal の意味論は変わらない。本決定は「S ∩ C の構造一致」の比較ルールを精緻化するものであり、テーブルの各行や signal の種類は変更しない。

**再評価が必要なとき**: カタログ側が将来「`#[derive]` 由来か手書きかを区別して宣言する」機能を持つようになった場合は、比較側もそれに合わせて再評価する。
...
- 後続 ADR:
  - `knowledge/adr/2026-05-11-2330-catalogue-impl-signals-command-layering.md` — 本 ADR の S/D/C 3-way 評価 (§D3 の 11 領域テーブル) を基盤とする SoT Chain ③ 診断コマンド (`bin/sotp track catalogue-impl-signals`) の設計記録。旧 `spec-code-consistency` の廃止・診断コマンドの導入・hexagonal レイヤー配置 (orchestration を `libs/usecase/` に置く)・インターフェース形式・CI ゲートの引き継ぎを記録している。
