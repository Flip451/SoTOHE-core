...
`action: "add"` / `"modify"` / `"delete"` のエントリに対しては
...
（`action: "reference"` エントリの例外については D5 を参照。）
...
### D5: `action: "reference"` エントリは informal-priority rule の適用対象外とする（D2 の拡張）

D2 は parent ADR `2026-04-23-0344-catalogue-spec-signal-activation.md` §D1.1 の informal-priority rule を
v3 エントリ全体に継承する規則を定めた。本決定はその規則を `action: "reference"` エントリに限り緩和する。

`action: "reference"` の意味論（`2026-05-08-0248-tddd-catalogue-layer-schema-axis-separation.md` §D4 参照）は
「既存型をそのまま参照目的で declare する、文書化 / 意図表明 / 構造変更検出のための明示的な宣言」であり、
当該型は baseline B にすでに存在している。新たに追加・変更・削除するわけではないため、
spec.json に対応する要件要素が存在しないのが本来の姿であり、`spec_refs` および `informal_grounds` を
設定する必然性がない。したがって、これらが空のまま `spec_refs: []`・`informal_grounds: []` であることは
grounding の欠落ではなく、baseline B への暗黙的な根拠付けとして正当である。

**評価規則（`action: "reference"` の場合）**:

| 条件 | signal |
|---|---|
| baseline B に型が存在する（reference-action の前提） + `spec_refs` 空 + `informal_grounds` 空 | 🔵 Blue |
| `spec_refs` 非空 または `informal_grounds` 非空 | D2 §D1.1 の通常規則を適用（明示 grounding を優先） |

`action: "add"` / `"modify"` / `"delete"` のエントリは D2 が定める §D1.1 の規則を引き続きそのまま適用する。

この緩和は parent ADR `2026-04-23-0344-catalogue-spec-signal-activation.md` §D1.1 が定める informal-priority rule が
`action: "reference"` エントリには設計意図として当てはまらないことを明示するものであり、
同 ADR の D1.1 自体は変更しない（v3 エントリへの継承の範囲でのみ、reference-action の例外を設ける）。
...
- `knowledge/adr/2026-04-23-0344-catalogue-spec-signal-activation.md` — parent ADR (SoT Chain ②)。
  §D1.1 の informal-priority rule を本 ADR が v3 エントリに継承する (D2)。ただし `action: "reference"` エントリはその適用対象外とする (D5 の緩和規則を参照)
