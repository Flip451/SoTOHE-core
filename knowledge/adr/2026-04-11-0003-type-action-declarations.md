...
Accepted (implemented in track `tddd-action-declarations-2026-04-11`; WIP-Yellow semantics for forward-check missing enabled in evaluator via track `catalogue-spec-signal-activation-2026-04-23`)
...
| `action` | 意味 | forward check (Blue 条件) | forward check (Yellow 条件) | reverse check (Red 条件) |
|---|---|---|---|---|
| `"add"` (デフォルト、省略時) | 型を新規追加する | 宣言したメソッドが C に存在しシグネチャ一致 | 宣言したメソッドが C に存在しない (WIP、前方欠損) またはシグネチャ不一致 | 宣言に**ない**メソッドが C に余剰存在 (reverse 余剰 → Red) |
| `"modify"` | 既存型を変更する | 宣言したメソッドが C に存在しシグネチャ一致 | 宣言したメソッドが C に存在しない (変更途中 WIP) またはシグネチャ不一致 | 宣言に**ない**メソッドが C に余剰存在 (reverse 余剰 → Red) |
| `"reference"` | 既存型をそのまま参照目的で宣言する | 宣言したメソッドが C に存在しシグネチャ一致 | 宣言したメソッドが C に存在しない (WIP) またはシグネチャ不一致 | 宣言に**ない**メソッドが C に余剰存在 (reverse 余剰 → Red) |
| `"delete"` | 型を意図的に削除する | 型が C に**存在しない** | 型が C に存在する (まだ削除されていない) | N/A (型ごと削除) |

**L1 `expected_methods` を宣言した場合の WIP セマンティクス**: `secondary_port` / `application_service` のように `expected_methods` を持つ kind で `action: modify` を使い、宣言した `expected_methods` に実装予定の新規メソッドが含まれる場合、それらのメソッドが実装前に C に存在しないのは正常な WIP 状態である。この場合の forward check は ADR `2026-04-11-0002-tddd-multilayer-extension.md` D2 の L1 ルール 1 (「メソッド名が見つからない → Yellow」) に従い **Yellow** を返す。**Red は発生しない**。`missing_items` が存在しても、それが forward check のスコープ (宣言済みのメソッドが未実装) である限り Red にしてはならない。Red が発生するのは逆方向 (L1 reverse check: コードに存在するが宣言されていないメソッド → Red) のみである。

`"add"`, `"modify"`, `"reference"` の forward check の Blue 条件ロジックは同一。区別する理由は意図の記録と、将来の検証精度向上:
