...
decisions:
  ...
  - id: D4
    ...
    status: proposed
  - id: D5
    user_decision_ref: "chat:2026-06-17:remove-knowledge-design-md"
    status: proposed
---
...
### D5: `knowledge/DESIGN.md` も同様に撤去する

`knowledge/DESIGN.md` (50 行、最終更新 2026-04-27) も D1/D2/D3 と同方針で撤去する。本 doc の内容は次の SSoT への pointer ばかりで独自情報が薄い:

- Architecture (mermaid 図) → `architecture-rules.json` と重複
- Module Structure table → `architecture-rules.json` の `canonical_modules` と重複
- Agent Roles → `.harness/config/agent-profiles.json` への pointer
- Key Design Decisions → `knowledge/adr/README.md` への pointer
- Crate Selection → 1 行 (`thiserror` のみ)

D1.1 と同様、撤去前に salvage する (`knowledge/research/YYYY-MM-DD-HHMM-<topic>.md` の独立 file 限定)。`knowledge/DESIGN.md` を cite している 10+ の現役 doc (`CLAUDE.md` / `README.md` / `.claude/rules/` / `.claude/commands/` / `.claude/settings.json` 等) は D4 の対象外 (現役 doc) であり、撤去に追従して通常通り参照を更新する。
...
- `knowledge/` 配下の working tree が軽くなり、新規参照者の認知負荷が下がる (15+3+2+1 = 21 ファイル削減; `knowledge/DESIGN.md` 含む)。
- 「戦略文書」「設計メモ」「スキーマメモ」「DESIGN.md (pointer hub)」という独立カテゴリ概念がプロジェクトから消え、track / ADR / convention の 3 系統に設計判断と運用ルールが集約される (architecture-rules.json / agent-profiles.json / adr/README.md などの SSoT を直接参照する形に統一)。
...
- 現役 doc (`knowledge/conventions/adr.md` Decision Reference、`knowledge/README.md`、`CLAUDE.md`、`.claude/rules/`、`.claude/commands/`、`.claude/settings.json` など 10+ ファイル) は `knowledge/strategy/` / `designs/` / `schemas/` / `DESIGN.md` の撤去に追従して参照を更新するコストがかかる (特に `DESIGN.md` は cite 元が広範)。
...
- `knowledge/DESIGN.md` — 撤去対象 (D5)。Architecture mermaid と Module Structure table は `architecture-rules.json` と内容重複
- `architecture-rules.json` — DESIGN.md 撤去後に Architecture / Module Structure の SSoT として直接参照される
- `.harness/config/agent-profiles.json` — DESIGN.md 撤去後に Agent Roles の SSoT として直接参照される
