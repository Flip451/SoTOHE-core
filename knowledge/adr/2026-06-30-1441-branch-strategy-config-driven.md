...
infrastructure 層には JSON config ローダ adapter と metadata snapshot reader を実装し、`BranchStrategyPort` 実装として usecase 層に注入する。

`BranchStrategyPort` は以下を expose する:
...
| **`apps/cli-composition/src/dry/shared.rs:91-115`** (`git rev-parse main` fallback 関数) | effective strategy から `base_branch()` を受け取り、git invocation では branch 名を個別 argv (`["rev-parse", base_branch]`) として渡す。shell command 文字列へ補間しない。**正本実装なので最優先**。エラーメッセージも同様に更新 |
