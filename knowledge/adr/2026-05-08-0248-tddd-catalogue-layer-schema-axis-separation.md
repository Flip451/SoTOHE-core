...
DataType 内の Composite / Sum 区別 (旧 DataKind) は、`TypeEntry.kind: TypeKind` の variant 構造で完全に encode される:
- `TypeKind::Struct { pattern, fields }` — Composite (struct 系)。`CompositePattern` を payload に内包
- `TypeKind::Enum { variants }` — Sum (enum 系)
- `TypeKind::TypeAlias { target }` — 型の別名

`TypeKind` の詳細は D7 で定義する。

Pattern × Language 制約は `TypeKind::Struct` の payload に `CompositePattern` を内包することで schema 構造で encode される。Struct 以外 (Enum / TypeAlias) は Pattern を declare できない (D3 で詳述)。
...
### D3: Pattern 軸 = CompositePattern を TypeKind::Struct payload に内包 (厳密 payload-encoded)

Pattern は TypeKind::Struct の payload に内包する (Q4.1 厳密 payload-encoded 採用):

```
<!-- illustrative, non-canonical -->
enum TypeKind {
    Struct { pattern: CompositePattern, fields: Vec<FieldDecl> },
    Enum { variants: Vec<VariantDecl> },
    TypeAlias { target: TypeRef },
}
```

Struct 以外の kind で Pattern を declare しようとすると parse 段階で reject される。Enum に fields を declare したり、TypeAlias に variants を declare したりする操作も同様に parse 段階で reject される。members × kind 制約を schema 構造で完全 encode する。

CompositePattern の 3 値は以下の通り:

```
<!-- illustrative, non-canonical -->
enum CompositePattern {
    Plain,
    TypestateState { of: TypeName, transition_methods: Vec<MethodName> },
    Newtype { inner: TypeRef },
}
```

- `Plain` — 特別なパターンなし (既定値)
- `TypestateState { of, transition_methods }` — typestate cluster の各 state を個別 entry として declare。`of` は cluster を特定する TypeName、`transition_methods` は当該 TypeEntry の `methods` (inherent) を method 名で参照する遷移メソッド名のリスト。D10 により `TraitImplDecl` は trait identity のみで `methods` field を持たないため、遷移メソッドは inherent methods からのみ参照する。各 method の `returns` から遷移先 type を導出し、linter が整合性を検証する (linter の具体的な検証ルール — receiver の制約、generics unwrap 範囲、エラーメッセージ等 — は Linter ADR で詳細化する。本 ADR は schema 構造のみを決定する)
- `Newtype { inner }` — inner type を wrap した newtype。`inner` は TypeRef (generics 含む文字列)
...
### D7: TypeKind 厳密 payload-encoded

D3 で示した TypeKind の構造を改めて明示する。各 kind が持てるフィールドは schema 構造で完全に制約される:
...
pub struct TypeEntry {
    action: ItemAction,
    role: DataRole,
    kind: TypeKind,                    // pattern / fields / variants / target を内包
    methods: Vec<MethodDeclaration>,
    trait_impls: Vec<TraitImplDecl>,
    module_path: ModulePath,           // serde default で省略可 (空 = crate root)
    docs: Option<String>,
}
...
Enum に `fields` を書いた場合、TypeAlias に `variants` を書いた場合はいずれも parse 段階で reject される。Struct の `fields` と `methods` は両方宣言可能であり、parent ADR M1 の「全 struct 系 kind に expected_members + expected_methods を均質化」の継承形である。
...
crate root の関数は crate name を prefix とする (`"domain_core::register_user"`)。cross-workspace 参照に対応するため、`"crate::"` prefix (自 crate のみを指す Rust 構文) や `"::"` leading (外部 crate も含む absolute path) ではなく crate name を直接 prefix する。
...
- crate name prefix (D11) により cross-workspace の関数参照と TypeRef の cross-crate 参照が統一表現になる。
...
- TypeKind enum の nested 構造により JSON serde 表現がやや重くなる (Struct / Enum / TypeAlias で異なる payload を持つため)。
- parent ADR M1 / S2 で決定した `methods_of()` renderer の均質化は、本 ADR の TypeKind ベース schema 上で再実装が必要になる。
...
- CompositePattern に新しい pattern 値 (例: `Builder` / `Phantom`) の追加要求が来た場合: Pattern 軸の拡張は schema 互換性に影響するため要評価。
