...
decisions:
  ...
  - id: D5
    ...
    status: proposed
  - id: D6
    user_decision_ref: "chat_segment:codex-orchestrator-settings-addition-implementation-review:2026-06-13"
    status: proposed
---
...
SoTOHE の **責任分界点**（`knowledge/conventions/responsibility-boundary.md`）の原則として、SoTOHE は自身の方法論・SoT Chain・architecture・framework コードの整合性を enforce する一方、利用者の provider/agent **設定**（`.claude` / `.codex`）は利用者の責任領域であり、SoTOHE は良いデフォルトと docs を提供物として配るが CI で強制しない。この責任分界点の帰結として、verify-orchestra のように利用者設定を CI で hard-fail させる verifier を SoTOHE は持つべきではない。D6 はこの原則を明示的に決定として記録し、verify-orchestra を全廃する。

## Decision

### D1: Codex root orchestrator は `.codex` project config で有効化する

Codex を SoTOHE の user-facing root orchestrator として扱えるように、repository root に `.codex/config.toml`、`.codex/instructions.md`、`.codex/rules/default.rules` を追加する。`.codex/config.toml` は Codex root session の既定 model、reasoning effort、sandbox、approval policy、hooks、subagent concurrency を定義し、`.codex/instructions.md` は SoTOHE 固有の orchestration rules と specialist routing を定義する。`.codex/rules/default.rules` は、SoTOHE の canonical `cargo make` wrapper や read-only Git inspection など、repo として永続的に許可する command prefix を定義する。

Codex の project-local config、rules、hooks は trusted project として扱われる checkout でのみ読み込まれる前提で運用する。未trusted checkout では repository-local `.codex` 設定が存在しても user/system layer の設定だけが有効になり得るため、運用文書は trust 前提と未trusted 時の fallback を明示する。
...
Codex custom agent TOML は、agent 名、説明、model、所有 artifact、境界、最終 status contract などの薄い role definition を持つ。per-agent sandbox は Codex 現行 schema が agent role file で受け付けない（`unknown field 'sandbox'`）ため agent TOML には設定せず、sandbox は `.codex/config.toml` の session-level `sandbox_mode` で与える。

実際の specialist workflow、必読資料、command discipline、self-verification、final response format は、provider 非依存の単一 SSoT として **`.harness/capabilities/<name>.md`** に置く。`.claude/agents/<name>.md`（Claude subagent）と `.agents/skills/<name>/SKILL.md`（Codex skill）はどちらもこの共有 SSoT を参照する薄い wrapper とし、frontmatter（model / tools / 起動方法）と provider 固有の注記だけを持つ。これにより同一 workflow を Claude 側と Codex 側へ二重に複製せず（DRY）、agent / skill 定義を薄くしても specialist 能力が空洞化しない。共有 SSoT は engineering convention ではなく capability/provider routing と同じ orchestration 層に属するため、`knowledge/conventions/` ではなく `.harness/`（`.harness/config/agent-profiles.json` と同じ階層）に配置する。
...
1. `.codex/config.toml`、`.codex/instructions.md`、`.codex/rules/default.rules`、`.codex/agents/*.toml` が repository root に配置されている。
...
5. hook adapter の挙動が `bin/sotp hook dispatch` の期待と一致することを実行確認している。
...
### D6: 利用者の provider/agent 設定は SoTOHE が CI 強制せず、verify-orchestra を全廃する

責任分界点（`knowledge/conventions/responsibility-boundary.md`）に従い、`.claude` / `.codex` の provider・agent **設定**はテンプレート利用者の責任領域とする。SoTOHE は良いデフォルトと docs を**提供**するが、その設定値・ファイル存在・permission allow/deny エントリ・散文文言を CI で hard-fail させて**強制**しない。

この帰結として `verify-orchestra` を**全廃**する。これは `.claude/settings.json` の permission allow/deny、`.codex/*` の config/rules/agents/skills/hooks、agent 定義、人間向け doc の散文スニペットを「期待リスト」と照合して CI で hard-fail させていた verifier であり、利用者所有領域を SoTOHE が enforce するもの全体に相当する。削除対象は `libs/infrastructure/src/verify/orchestra/` モジュール、CLI の `verify orchestra` subcommand、`cargo make verify-orchestra` task、CI からの呼び出し。Codex surface の設定ファイル自体（`.codex/*`、`.agents/skills`、`.harness/capabilities`、sample profile）は**提供物**として残す。

理由: verify-orchestra は (1) verifier を設定の正確値・文言へ結合させて脆く（無害な変更で CI が壊れる）、(2)「ゲートが緑＝正しい」という**偽の安心**を生み、(3) 本当に守るべき drift（DRY / 設計意図 / grounding）は string-match では捕まらず review でしか発見できない。DRY 違反（Claude subagent と Codex skill の workflow 二重保守、D2 参照）や grounding 破れといった設計 drift は、string-match gate ではなく semantic review（`ref-verify`）と人手 review によって発見される。

維持するもの（SoTOHE 自身の整合性ゆえ enforce 継続。これらは verify-orchestra とは別の verifier である）: SoT Chain の整合性（`ref-verify` semantic、`adr-signals`、`spec-states`、`catalogue-spec`）、生成ビュー freshness（`verify-view-freshness`）、architecture rule（`check-layers`）、framework 自身のコード品質（`cargo make ci` の fmt / clippy / test / deny）。これらは利用者設定ではなく SoTOHE の方法論・コードの整合性であり、責任分界点の内側にある。

設定の良し悪し（危険な allow を入れていないか等）は利用者の責任と review に委ね、SoTOHE は危険な例とその理由を docs（`.claude/rules/10-guardrails.md` 等）に書いて警告するが、CI で強制しない。
...
- 変更の中心が `.codex`、`.agents/skills`、`.harness/capabilities`、`.harness/config/agent-profiles.json`、運用文書に限定され、SoTOHE core mechanism への侵襲を抑えられる。
- specialist の濃い workflow を provider 非依存の `.harness/capabilities/<name>.md` に単一 SSoT として置き、`.claude/agents/<name>.md` と `.agents/skills/<name>/SKILL.md` を薄い参照 wrapper にするため、agent / skill 定義を薄くしても specialist 能力は失われず、Claude 側と Codex 側で同一 workflow を二重保守しない（DRY）。
...
- Template として Claude と Codex の両 orchestration surface を恒久的にサポートするため、運用文書が両対応になり、保守対象が増える。
- Codex hook semantics と Claude hook semantics の差分確認が必要で、trust / matcher / stdin-output 契約の検証を省けない。
- `verify-orchestra` を全廃したため、`.claude` / `.codex` の設定不備（危険な permission allow、欠落ファイル、agent / skill / shared-doc の対応崩れ）は CI で自動検出されず、利用者の責任と review に委ねられる。これは責任分界点上の意図的なトレードオフである（`knowledge/conventions/responsibility-boundary.md`）。
...
- `.codex/` — Codex project config、instructions、project-local rules、custom agent 定義の配置先。
