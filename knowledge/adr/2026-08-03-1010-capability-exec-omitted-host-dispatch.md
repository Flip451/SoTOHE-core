...
decisions:
  - id: D1
    review_finding_ref: "rollback-diagnoser:scope-conditional-pre-review-gates-2026-07-31:final-review-P0:T032:omitted-host-dispatch"
    status: proposed
  - id: D2
    review_finding_ref: "rollback-diagnoser:scope-conditional-pre-review-gates-2026-07-31:final-review-P0:T032:omitted-host-dispatch"
    status: proposed
...
### D3: operator-owned config は host を持たず、invocation surface が optional host を転送する

phase writer argv などの operator-owned command config 自体は provider host を固定せず、`--host` を保持しない。
その config を消費する phase-engine entry command などの invocation surface は、caller が指定できる optional host
parameter を受け取り、値がある場合だけ configured writer の `capability exec` invocation へ転送する。
転送された値は D1 の runtime-host 自己申告として扱われ、D7 の規則に基づく in-host 委譲の候補になりうる。
caller が値を指定しない場合は `--host` を転送せず、D2 の profile 解決による subprocess dispatch を適用する。

この決定は `2026-08-02-0806-operator-owned-phase-command-config.md` D1 のうち、engine が configured argv を
書き換えないとする部分を限定的に modify する。config に保存・解決された literal argv 自体は変更せず、
invocation surface が dispatch boundary で caller-supplied optional `--host` だけを実行時 argv に追加できる。
それ以外の argv を書き換えたり意味解釈したりしないという D1 の規則と、config が provider-neutral な
literal argv を宣言する決定は維持する。

host の永続的な選択を config から除外しつつ、runtime host を知る caller が invocation boundary でその事実を
明示できるため、config の provider-neutral 性と supplied `--host` の既存契約を両立できる。
