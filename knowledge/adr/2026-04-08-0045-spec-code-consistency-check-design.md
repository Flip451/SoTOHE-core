...
- ~~**逆方向チェックを信号機に統合**: 信号機は前方向 (spec → code) 専用。逆方向は別の ConsistencyReport~~ → **撤回** (2026-04-08): 設計書と実装のドリフト防止には逆方向も CI error としてブロックすべき。現時点では `sotp verify spec-code-consistency` が undeclared types/traits を error として報告し CI をブロックする。信号機 (ConfidenceSignal) への統合は次トラックで検討
...
- 逆方向チェックを ConfidenceSignal (信号機) に統合し、Red シグナルとして verify spec-states ゲートに組み込む (設計書と実装のドリフト防止を構造的に強制)
