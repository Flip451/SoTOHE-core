...
### 5. approved フィールドと Yellow の将来 → 却下
...
→ **却下** (ADR `2026-04-08-1800-reverse-signal-integration.md`): TDDD 方針により自動追加は採用しない。未宣言型は Red シグナルで CI ブロックし、`/track:design` (designer capability) による型定義の更新を強制する。
...
- ~~**逆方向チェックを信号機に統合**: 信号機は前方向 (spec → code) 専用。逆方向は別の ConsistencyReport~~ → **撤回・実装済み** (ADR `2026-04-08-1800-reverse-signal-integration.md`): TDDD 方針で逆方向チェックを Red シグナルとして信号機に統合。未宣言型は自動追加せず Red で CI ブロックし `/track:design` による型定義更新を強制する
...
- ~~逆方向チェックの auto-add (AI が未宣言型を自動追加) が実装された時点で Yellow 再導入を検討~~ → **却下** (ADR `2026-04-08-1800-reverse-signal-integration.md`): TDDD 方針により自動追加は採用しない。型定義は designer が先に書くべきもの
- ~~逆方向チェックを ConfidenceSignal (信号機) に統合し、Red シグナルとして verify spec-states ゲートに組み込む (設計書と実装のドリフト防止を構造的に強制)~~ → **対応済み** (ADR `2026-04-08-1800-reverse-signal-integration.md`): 未宣言型を Red シグナルとして verify spec-states に統合。2 段階ゲート: 途中コミット時は Red なし → pass (Yellow 許容)、merge 時は全 Blue 必須 (Yellow もブロック)
