...
decisions:
  ...
  - id: D4
    ...
    status: superseded
    superseded_by: "2026-05-20-0048-tddd-trait-impl-top-level-promotion.md (D1-D4)"
  - id: D5
    user_decision_ref: "chat_segment:make-catalogue-schema-permissive-2026-05-19:2026-05-20"
    candidate_selection: "from:[front-load-hrtb-support,narrow-skip-to-unsupported-bound-only,accept-yellow] chose:front-load-hrtb-support"
    status: proposed
---
...
3. **信号評価器 `strip_outlives_from_index` 撤廃 + 両側保持**: 片側ストリップを撤廃し、両側でライフタイム境界を保持する。`build_generics_fingerprint` を全境界種類 (ライフタイム境界 / HRTB / 精密捕捉等) および `BoundOp` に対応させ、`{ lhs, rhs, operator }` 単位でフィンガープリントを生成する。`rhs` の要素は集合として正規化 (順序非依存ソート) し、`T: A + B` と `T: B + A` を同一フィンガープリントにする。
   ...
   **残存制限 (D5 で解消)**: 本 D1 の実装において、`generics_eq.rs::bounds_supported` は `GenericBound::TraitBound.generic_params` が非空 (HRTB-on-TraitBound: `F: for<'_> Fn(&'_ str)` のような形) を ADR `2026-05-08-0305` D3 の fail-closed ルールの対象として扱い、両側が同一の HRTB-on-TraitBound を持つ場合でも `structs_structurally_equal` を `false` にする。rustdoc は elided-lifetime の Fn trait bound (`Fn(&str)`) を HRTB 形 (`for<'_> Fn(&'_ str)`) に desugar するため、Fn closure bound 付きコンストラクタを持つ struct は対称比較で genuine Blue にできない構造的 false-Yellow が生じる。本 D1 の「HRTB on トレイト境界」解消は A-codec エンコードのみを対象としており、信号評価器の HRTB-on-TraitBound サポートは D5 で完了させる。
...
### D4: 信号評価器の `for_is_external` filter を両側で撤廃し、cross-crate impl を対称に扱う

> **Note (superseded)**: 本 decision は ADR `2026-05-20-0048-tddd-trait-impl-top-level-promotion.md` (D1-D4) により superseded された。当該 ADR で `TraitImplDeclV2` を `CatalogueDocument` の top-level に並列化し `trait_ref` / `for_type` の 2 軸対称構造に統一することで、本 D4 が scope 外としていた「self type が外部クレートの新規 cross-crate impl (Add action)」も含めて declare 不可能性が解消される。本セクションは historical record として残す。

信号評価器は cross-crate impl (例: `impl From<CatalogueToExtendedCrateCodecError> for NewTypeGraphCodecError`) を C 側 (`build_impl_identity_map`) と B 側 (orphan impl 挿入) で非対称に扱っており、これが false-positive Red の根本原因となっている。

**問題の構造**: C 側の `build_impl_identity_map` (`signal_evaluator_v2/mod.rs` の `for_is_external` filter) は `for_` 型の `crate_id` をチェックして外部型を指す impl を `c_impls` から除外する。一方、B 側の orphan impl 挿入 (`phase1/builder.rs`) は外部型チェックなしに全 impl を S に挿入するため、外部型を対象とする impl が `s_impls` には存在するが `c_impls` には存在しない非対称な状態が生じ、`SMinusC_Reference` 扱いの false-positive Red が発生する。

**方針**: filter を揃えるのではなく、両側の filter を撤廃する。

- **C 側**: `build_impl_identity_map` 内の `for_is_external` filter を撤廃し、cross-crate impl も `c_impls` に含める。
- **B 側**: orphan impl 挿入処理は現状維持 (filter なしで全 impl を S に挿入する既存の挙動)。

結果として両側に cross-crate impl が含まれる状態になり、fingerprint が対称に生成されて Blue が得られる。

**catalogue 宣言との対応**: filter 撤廃により cross-crate impl が信号評価の対象になるため、catalogue 設計者が cross-crate impl を declare する場合は `TraitImplDeclV2` の `for_` フィールドに fully-qualified path (例: `domain::tddd::new_typegraph_codec_error::NewTypeGraphCodecError`) を記述する慣習を採用する。A-codec は fully-qualified path を parse して外部クレートを判定し、`external_crates` および `paths` に登録する。`for_` フィールドは `Option<String>` かつ `#[serde(default)]` で None をデフォルトとするため、既存の catalogue ファイルは無修正でロードでき、スキーマバージョンは変更しない (OS-04 の後方互換設計)。

### D5: 信号評価器の HRTB-on-TraitBound を対称比較でサポートする

rustdoc は elided-lifetime の Fn trait bound (`Fn(&str)`) を HRTB 形 (`for<'_> Fn(&'_ str)`) に desugar する。その結果、`new<F: Fn(&str) + Send + Sync + 'static>()` のような Fn closure bound 付きコンストラクタを持つ struct では、C 側 (rustdoc 出力) の `GenericBound::TraitBound.generic_params` が非空になる。

`generics_eq.rs::bounds_supported` は `TraitBound.generic_params` が非空であることを「HRTB-on-TraitBound = unsupported」として扱い (ADR `2026-05-08-0305-tddd-signal-evaluator-three-way-diff.md` D3 fail-closed)、`build_where_form_view` が当該境界を `unsupported_raw` に積む。`generics_structurally_equal` はいずれかの側の `unsupported_raw` が非空であれば無条件に `false` を返すため、A 側と C 側が同一の HRTB-on-TraitBound を持っていても `structs_structurally_equal` が `false` となる構造的 false-Yellow が生じる。

この false-Yellow は `2026-05-20-0413-tddd-struct-inherent-method-symmetric-comparison.md` で struct 対称比較を実装する過程で確認された。usecase の 2 つの interactor (`ReviewCheckApprovedInteractor` / `TaskOperationInteractor`) が該当し、genuine Blue が得られない。

**決定**: `bounds_supported` を拡張し、`TraitBound.generic_params` が非空 (HRTB-on-TraitBound) の場合も、両側の HRTB バインダーを含む TraitBound を対称に比較できるようにする。具体的には以下の変更を行う。

1. **`bounds_supported` の対象拡張**: `TraitBound.generic_params` 非空を unsupported とする条件を撤廃し、HRTB-on-TraitBound も支援境界として扱う。
2. **HRTB-on-TraitBound のフィンガープリント生成**: `format_generic_bounds_with_canon` (または同等の書式化関数) を HRTB-on-TraitBound に対応させ、`generic_params` (HRTB バインダーのライフタイム名) を正規化したうえで TraitBound 全体を文字列化する。バインダーのライフタイム名は elision 形式 (`'_`) と明示形式 (`'a`) を同一視できるよう正規化する。
3. **スコープ限定**: 今回の変更は「ジェネリックパラメータ位置の TraitBound に付いた HRTB バインダー」(`F: for<'_> Fn(&'_ str)` 形) のみを対象とする。`BoundPredicate.generic_params` 非空 (where 句レベルの HRTB: `where for<'a> T::Item<'a>: Bound`) は `WherePredicateDecl.lhs` に組み込む D1 の設計で既にカバーされており、本 D5 の変更対象ではない。ライフタイムパラメータ / const パラメータの impl/trait ジェネリックへの追加 (D2 スコープ限定項目) も引き続き将来の拡張トラックに委ねる。
...
### N. HRTB-on-TraitBound を unsupported のまま維持し、narrow-skip として Yellow を継続する案 (D5 の代替)

`bounds_supported` の変更範囲を最小にし、HRTB-on-TraitBound を依然として unsupported 扱いにするが、「両側が同一の unsupported predicates を持つ場合のみ false ではなく Yellow (不一致) を返す」という narrow-skip に変更する案。

却下理由: 両側が同一の HRTB-on-TraitBound を持つ場合は genuine Blue であるべきであり、Yellow を継続することは strict signal gate のもとでマージ不能な状態を恒久化する。Fn closure bound を持つ struct エントリが存在するかぎり構造的 Yellow が解消できず、TDDD の基本原則に反する。D5 の採択案 (対称比較サポート) が唯一 genuine Blue を得られる方向である。

### O. HRTB-on-TraitBound の false-Yellow を受容する案 (D5 の代替)

現状の Yellow を設計上の制約として受容し、spec entry に OS として明示する案。

却下理由: strict signal gate のもとで構造的に Blue 化できない Yellow を OS として設けることは、当該 spec entry をマージ不能にする。Fn closure bound を持つ interactor が usecase 層に存在する以上、Yellow の受容はトラック全体の CI を詰まらせる。TDDD の Yellow は「宣言を精緻化すれば Blue 化できる」状態を前提とするため、設計的に Blue 化不可能な Yellow を受容する運用は TDDD 原則と整合しない。
...
### M. B 側 orphan impl 挿入に同等の `for_is_external` filter を追加して両側 filter にする案 (D4 の代替)

cross-crate impl の非対称問題に対し、B 側の orphan impl 挿入処理に C 側と同等の `for_is_external` filter を追加し、両側で cross-crate impl を除外して対称にする案。外部型判定は B の paths または external_crates を参照して行う。

却下理由: cross-crate impl を両側で除外すると、catalogue 設計者は cross-crate impl を declare できなくなる (TDDD compliance hole が生じる)。外部クレート型を対象とする impl を catalogue で宣言したい場合に構造的に Blue 化できない Yellow が生まれ、strict signal gate のもとでマージ不能になる。本 ADR の精神 (catalogue を Rust 文法に対して寛容な設計にする) と反する方向への修正であり却下。filter を揃えるなら除外ではなく両側で取り込む方向 (D4 の採択案) が TDDD 原則と整合する。
...
- false-positive 4 パターン (ライフタイム境界の非対称性 / impl ブロックジェネリック欠落 / `patch_impl_for_ids` のクレート横断ターゲット誤動作 / `build_impl_identity_map` の `for_is_external` filter 片側適用による cross-crate impl の非対称扱い) が根本から解消する。
...
- C 側 `build_impl_identity_map` の `for_is_external` filter 撤廃により cross-crate impl が `c_impls` に含まれるようになるため、cross-crate impl を declare する catalogue 設計者は `TraitImplDeclV2.for_` に fully-qualified path を記述する慣習に従わなければならない (D4)。慣習を誤ると false-positive Red が継続する。
...
- `knowledge/adr/2026-05-20-0413-tddd-struct-inherent-method-symmetric-comparison.md` — struct 対称比較 ADR。D1 の struct method 対称比較実装の過程で D5 の HRTB-on-TraitBound false-Yellow が確認された。
