...
> **実装上の注記 — test-helper サブクラスタの保留**: D4 の方針自体は維持する。ただし test-helper サブクラスタ（`CwdGuard` / `init_git_repo` / usecase stub bindings の境界ごと集約、対応タスク T008–T010）は、現時点では着手を見送る。D4 が定めるテストコンパイル境界ごとの `#[cfg(test)]` 単一定義は、境界をまたぐバイト同一コピー（例: `apps/cli` と `libs/infrastructure` の `CurrentDirGuard`）を意図的に残存させるが、DRY ゲート（embedding 類似度 + Codex judge）はこれを architecture-rules.json / ADR のコンテキストなしにペアごとに評価するため違反と判定し続け、自律的な解消手段がない（詳細は `## Consequences → Negative` 参照）。constants サブクラスタ（`POLL_INTERVAL` / `"tmp/reviewer-runtime"` 定数集約）は予定どおり進める。
...
- DRY ゲートは、D4 が意図的に残存させる境界またぎ test ヘルパの重複（例: `apps/cli` と `libs/infrastructure` に両立する `CurrentDirGuard`）を「受理済み」として記録する手段を持たない。judge は architecture-rules.json / ADR コンテキストなしにフラグメントペアを個別評価するため、ADR 根拠の残存重複であってもアーキテクチャ上の判断として許容することができず、DFP フェーズがこれらのペアでブロックされる。この制約が test-helper サブクラスタの着手を見送る理由である（constants サブクラスタには当該ペアが存在せず影響しない）。
...
- DRY ゲートが、アーキテクチャ規則 / ADR 根拠に基づいた「受理済み偏差」を記録できるようになったとき（例: accept-list の導入、層認識型 judge コンテキストの追加）。この機能が整備され次第、D4 test-helper サブクラスタ（`CwdGuard` / `init_git_repo` / usecase stub bindings の境界ごと集約）の着手を再検討する。
