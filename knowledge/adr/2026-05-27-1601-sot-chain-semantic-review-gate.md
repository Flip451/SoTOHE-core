...
decisions:
  ...
  - id: D10
    ...
    status: accepted
  - id: D11
    user_decision_ref: "chat:2026-06-09:chain-specific-prompt-srp"
    status: accepted
  - id: D12
    status: accepted
    user_decision_ref: "chat:2026-06-09:probe-skip-when-no-cache-miss"
  - id: D13-1
    status: accepted
    user_decision_ref: "chat:2026-06-09:no-subprocess-timeout"
  - id: D13-2
    status: accepted
    user_decision_ref: "chat:2026-06-09:no-env-var-binary-resolution"
  - id: D13-3
    status: accepted
    user_decision_ref: "chat:2026-06-09:payload-only-no-tools-verifier"
---
...
参照ペアの並列処理には `agent-profiles.json` の Chain 別専用 capability (`ref-verifier-chain1` / `ref-verifier-chain2`) を用いる (D7/D11)。
...
### D7: 専用 capability 群 `ref-verifier-chain1` / `ref-verifier-chain2` を agent-profiles.json に新設する

意味論レビューの検証器を `reviewer` capability の流用ではなく、Chain ごとの専用 capability (`ref-verifier-chain1` / `ref-verifier-chain2`) として `agent-profiles.json` に新設する。D7 は code-review とは別 capability 群を持つことを定め、D11 はその capability 群を Chain① / Chain② に分ける責務境界を定める。

専用 capability 群とする理由は3点ある。第一に、モデル・プロンプトを `reviewer` の設定と独立に調整できる（D13-1 により timeout_seconds は capability 設定を持たない）。`reviewer` は通常の code review を対象とし `ref-verifier-chain1` / `ref-verifier-chain2` は参照ペアの意味論整合を対象とする — 求めるモデル特性が異なるため設定を分離する方が適切である。第二に、軽量モデルの選択肢を D13-2 で定めた `claude` / `codex` / `gemini` の3つから codex 系に限定されず claude の haiku 相当など自由に選べる。第三に、`reviewer` と各 ref-verifier capability を同時に並列実行する場合に設定が干渉しない。

**provider をまたいだ段階引き上げのサポート**: `agent-profiles.json` の各 capability 設定は `fast_provider` / `fast_model` を指定することで Fast round と Final round に異なる provider を割り当てられる。`libs/infrastructure/src/agent_profiles.rs` の `resolve_execution` (行 188-207) が `RoundType::Fast` に対して `fast_provider ?? provider` / `fast_model ?? model` を解決し、`RoundType::Final` には `provider` / `model` を返す。テスト `test_resolve_fast_with_cross_provider` (行 320-348) はこの provider をまたぐ解決が正しく動作することを保証している。`ref-verifier-chain1` / `ref-verifier-chain2` はこの機構をそれぞれ利用して段1 (Fast) と段2 (Final) に異なる provider を設定できる。
...
### D10: 翻訳ギャップの許容 — 意味論的 intent / behavioral consistency を問い、構造的詳細の完全 grounding は要求しない

自然言語 (ADR / spec) から型カタログおよび実装 (型シグネチャ / フィールド名 / メソッド名) への変換には、必然的に翻訳ギャップが生じる。自然言語で設計判断の核となる decision と behavior を記述することはできるが、すべての構造的詳細 (struct の全フィールド・メソッドシグネチャ等) を自然言語側で完全に言及するのは不可能であり、そのような要求は ADR と spec の記述を実装仕様書へと変質させる。

ref-verifier の判定基準を次のとおり定める。

- **pass とする**: catalogue の claim が evidence (ADR decision / spec element) の核となる decision・behavior と矛盾しない場合。struct shape / フィールド名 / メソッド名等の追加的な構造詳細が evidence に明示されていなくても、その decision の自然な実装として導かれるならば fail としない。
- **fail とする**: 次の3類型のいずれかに該当する場合。(a) evidence が示す behavioral / intent と矛盾する主張 — evidence の decision を覆す・否定する claim。(b) evidence に記載のない新規の behavioral commitment — evidence が言及していない外側の責務・契約を claim が追加している。(c) evidence に記載のない新規の design constraint — evidence が示す設計方針に存在しない制約を claim が導入している。

この基準は「主張が根拠と意味的に整合するか」を問う層③ の本来の責務 (D1 参照) に対応する。構造的詳細の有無を問うのではなく、decision の intent と behavioral consistency を問う。

### D11: Chain ごとに別 prompt template / 別 capability を使う — Chain1 と Chain2 の判定責務を分離する

Chain① (spec → ADR: 自然言語 → 自然言語) と Chain② (catalogue → spec: 型構造 → 自然言語) は意味論判定の対象 layer が異なる。Chain① は ADR の decision text と spec element の自然言語記述を比較して intent の整合を問う。Chain② は catalogue の型シグネチャ・フィールド・メソッド等の構造的記述が spec element の behavioral commitment と矛盾しないかを問う。どちらも「claim が evidence を裏付けるか」を問う層③ の検証だが、判定基準・許容すべき翻訳ギャップの種類・モデルへの指示が異なる。両者を単一の prompt template で共用すると、一方に特化した文脈がもう一方の判定に干渉し判定精度が落ちる。

この責務の違いに基づき、Chain ごとに独立した capability を `agent-profiles.json` に定義する。

- **Chain① 専用**: `ref-verifier-chain1` capability。自然言語 → 自然言語の比較に適した prompt template (`prompt_template_path`) を持つ。D10 が示す翻訳ギャップ許容は Chain② に強く効くため、Chain① の prompt はより厳密な intent grounding を問う基準で記述する — ADR decision text が spec element の主張を直接裏付けるかを問い、構造的詳細ではなく言語的・論理的整合を優先する。
- **Chain② 専用**: `ref-verifier-chain2` capability。自然言語 → 型構造の比較に適した prompt template を持つ。型シグネチャ・フィールド名・メソッド名等の構造的詳細は evidence 側に明示されていなくても decision の自然な実装として導かれれば許容する (D10 の翻訳ギャップ許容が中心的に作用する) という判定基準を反映した prompt とする。

`AgentRefVerifierAdapter` は `verify_pair` 呼び出し時の `cache_scope` 引数から Chain 種別を解決し、対応する capability (`ref-verifier-chain1` または `ref-verifier-chain2`) の `prompt_template_path` を選んで使用する。`RefVerifierPort::verify_pair` のシグネチャに `cache_scope` を渡す必要があるならば usecase 側もそれを受け入れる。能力の解決経路 (D7 の `agent-profiles.json` による `resolve_execution` 機構) は各 capability が独立して持つ。

D5 の段階引き上げ (fast/final モデル) は `ref-verifier-chain1` と `ref-verifier-chain2` それぞれに独立して設定できる。

### D12: production cache miss が 0 件のとき calibration probe 評価を skip する

known_bad probe (calibration probe) の目的は「新規 production 評価が走るバッチで検証器の劣化を検出する」ことにある。production の参照ペアが全て cache hit のとき、新規に呼び出す production verifier は 0 件であり、calibration の対象となるバッチ自体が存在しない。この状況でも probe を評価すると、全 cache hit run が完全コスト 0 にならない。

**ref-verifier interactor は production cache miss が 0 件のとき known_bad probe の評価を skip する。** これにより全 cache hit run は LLM 呼び出しコスト 0 で完了する。

production cache miss が 1 件以上ある run では従来どおり probe を評価し、D5 の段階引き上げ判定に供する。probe の注入率・検出率閾値は D6 で定めた設定ファイル管理に従う。

### D13: ref-verifier subprocess の実行規律 — タイムアウトなし・provider binary の固定リテラル解決・adapter から runner へ渡す判定入力は rendered prompt payload のみ

ref-verifier が LLM subprocess を呼び出す際の3つの実行規律を定める。

**タイムアウトを設けない。** ref-verifier の subprocess は終了するまで待機する。LLM の評価時間は対象ペアの長さとモデルの応答速度に依存し、任意のタイムアウト値を設けるとモデルや対象内容によっては評価が途中で打ち切られ、偽の escalation が発生する。capability 設定にも timeout 項目を持たない。

**provider binary の解決に環境変数間接参照を使わない。** binary 名は provider 名そのまま (`claude` / `codex` / `gemini`) の固定リテラルとし、解決は OS の PATH に任せる。環境変数による差し替え口 (例: `CLAUDE_BIN`) は、検証ゲートの実体を外部から無検証でつけ替えられる抜け穴になるため持たない。

**adapter から runner へ渡す判定入力は renderer が生成した prompt payload のみとする。** adapter は runner にレンダリング済み payload を渡し、runner 内で template 再読み込みは行わない。claude ではツール無効化引数 (`--disallowedTools` / `--tools ""`) を使い LLM function-calling tool 経由のファイルアクセスを機械的に排除する。codex では `--sandbox read-only` を使用してワークスペースへの書き込みを禁止し verify-cache artifact の改ざんを防ぐ; codex はファイル読み取りを OS レベルでは遮断しないが、structured output (`--output-schema`) で flat JSON verdict を強制し、prompt 設計によって claim / evidence テキストのみに基づく判定を誘導する。gemini は `-p <payload>` でレンダリング済み prompt payload を渡す。検証ゲートの判定は提示された証拠のみに基づくべきであり、ワークスペースへの自由な参照は D4 のハッシュ凍結による判定の再現性を損なうため、prompt 設計で参照を誘発しないよう対処する。
