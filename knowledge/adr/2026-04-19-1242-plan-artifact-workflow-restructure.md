...
##### metadata.json (track identity 専用)

- **配置**: `track/items/<id>/metadata.json`
- **含めるもの**: `schema_version, id, branch, title, status, created_at, updated_at`
- **含めないもの**: `tasks[]` / `plan.sections` / その他の Phase 3 成果物
- **作成タイミング**: /track:plan のフェーズ 0 (初期化、main が書く)
- **書き換え**: track の status 遷移と updated_at のみ更新。それ以外は不変
...
| ゲート | 対象 | 発火条件 |
|---|---|---|
| `verify-track-metadata-local` | metadata.json の存在と identity field の schema 妥当性 | 常時 (track directory があれば要求) |
...
- **状態フィールド廃止による手続き削減**: approved の形骸化が根治する
...
- 改訂対象:
  ...
  - metadata.json スキーマ: `tasks[]`, `plan` を削除し identity field のみに (D1.4 / D6.1)
