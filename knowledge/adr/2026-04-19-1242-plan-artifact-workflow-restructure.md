...
- `is_activated()` が true のとき `impl-plan.json` が必ず存在することを `check_impl_plan_presence(track, impl_plan)` で enforce する
...
- `SpecRef` には `hash` も required で導入。spec.json は構造化 (JSON) で anchor が示す要素 (id で特定される subtree) の境界が明確なので、hash 対象が曖昧にならず本 ADR で確定できる
...
- D1.5 で定めた参照方向制約は「どの field にどの ref 構造体が入るか」が型で決まっているため、CI 検証は field ごとに ref 構造体の妥当性 (file 存在 / anchor 解決 / hash 照合 / summary 非空) を検査するだけで済む (role dispatch 不要)
...
- hash による drift 検出 CI gate の本格実装 (基本骨格は D2.3)
...
新規 CLI サブコマンドの役割:
- 各成果物の ref field (`adr_refs[]` / `convention_refs[]` / `spec_refs[]` / `related_conventions[]` / `informal_grounds[]`) を走査:
  - 各 field に入っている ref 構造体が期待どおりの型か (schema validator で強制)
  - 参照先ファイルが実在するか (file ベース ref のみ、存在チェック)
  - `SpecRef.anchor` (`SpecElementId`): 参照先 spec 要素の `id` が解決できるか (本 ADR 範囲)
  - `SpecRef.hash` (`ContentHash`): 参照先 spec 要素の canonical serialization の SHA-256 と一致するか (本 ADR 範囲、drift 検出)
...
本 ADR では CLI コマンドの **存在と役割範囲**、`SpecRef.anchor` / `SpecRef.hash` の解決ロジック、`AdrAnchor` / `ConventionAnchor` の loose validation、および `InformalGroundRef` の newtype validation を決定。`AdrAnchor` / `ConventionAnchor` の semantic 厳密化・resolution ロジック、および `AdrRef.hash` / `ConventionRef.hash` の追加は Q15 の後続 ADR に委ねる。
...
- **評価の着目点** (field ごとに独立評価、結果の合成で総合 signal を決定):
  ...
  - 総合 signal = max(adr_refs, informal_grounds): 🔴 が最優先、次いで 🟡、全 🔵 でのみ 🔵
...
- ただし `sotp verify plan-artifact-refs` (D2.3) の `SpecRef.anchor` / `SpecRef.hash` 検証は本 ADR で有効 (schema / drift レベルの機械検証)
- designer プロンプトで正確な `spec_refs[]` 記述を要請する

**signal 実装後 (後続 ADR の範囲)**:
- 成立条件: 型カタログの各エントリの `spec_refs[]` 評価で 🔴 がゼロ、かつ `informal_grounds[]` が track 完了時に解消されていること (🟡 は commit 可・merge 不可)
- **評価の着目点** (field ごとに独立評価、結果の合成で総合 signal を決定):
  - `spec_refs[]`: 🔵 (全 SpecRef が anchor 解決 + hash 一致) / 🔴 (1 件でも失敗)
  - `informal_grounds[]`: 空 → 🔵、非空 → 🟡 (未永続化根拠あり、merge 前に formal ref (SpecRef) または ADR / convention への昇格要)
  - (型カタログには convention への参照 field がそもそも存在しないため除外ロジック不要)
  - 総合 signal = max(spec_refs, informal_grounds): 🔴 最優先、次いで 🟡、全 🔵 でのみ 🔵
- catalogue-signal の実装は本 ADR 対象外。`knowledge/adr/2026-04-18-1400-tddd-ci-gate-and-signals-separation.md` または別 ADR が担う
...
- **advisory 期間の `spec_refs[]` 品質**: signal が未実装の間は catalogue の `spec_refs[]` 評価が検証されないため、semantic な正確性は designer のプロンプト品質に依存する (`SpecRef.anchor` / `hash` の schema / drift 検証は D2.3 で本 ADR から有効)。signal 実装までは semantic 面の drift 防止が完結しない期間がある
...
- 改訂対象:
  ...
  - metadata.json スキーマ: `tasks[]`, `plan`, `status` を削除し identity field のみに (D1.4 / D6.1)。schema_version を v5 に変更。`status_override` (optional) を明示的に identity field として追加。`derive_track_status` / `check_impl_plan_presence` / `is_activated` を domain に新設 (D1.4 派生ステータス / activation 不変条件)
...
- `sotp verify plan-artifact-refs` の `AdrRef` / `ConventionRef` 側 resolution 実装 (`SpecRef` 側は本 ADR D2.3 で骨格を決定、`AdrAnchor` / `ConventionAnchor` の存在チェックも本 ADR で有効)
...
- 各 ref field の参照先整合 + `SpecRef.anchor` / `SpecRef.hash` 検証 CLI
- task-coverage.json の coverage 強制 + referential integrity 検査 (現行 `spec_coverage::verify` 踏襲)
- `cargo make ci` への組み込み
- **catalogue-signal (型契約 → 仕様書) は本 ADR スコープ外**。別 ADR (例: `tddd-ci-gate-and-signals-separation`) で実装する。signal 実装までは catalogue の `spec_refs[]` は semantic 面で advisory 扱い (schema / drift 検証は本 ADR D2.3 で有効、D3.2)
...
- **signal 実装の責務分担**: 本 ADR は catalogue に `spec_refs: Vec<SpecRef>` schema を追加し、値オブジェクト (`SpecElementId` / `ContentHash`) と共に `sotp verify plan-artifact-refs` で schema / drift 検証を提供するところまでを扱う。spec 側も `adr_refs: Vec<AdrRef>` / `convention_refs: Vec<ConventionRef>` + newtype (`AdrAnchor` / `ConventionAnchor`) を導入。signal 評価 (型契約 → 仕様書、README ロードマップの「計画中」項目) は本 ADR スコープ外で、上記別 ADR または後続 ADR が担当する。signal 実装前は `spec_refs[]` の semantic 品質は未検証 (schema / drift の機械検証は本 ADR で有効)
...
- `check_impl_plan_presence(track, impl_plan)` を domain に新設: activated track (branch あり) に impl-plan.json が必ず存在することを enforce
