...
decisions:
  - id: D1
    review_finding_ref: "adr-diagnoser admission bounce: 常に null の entries[].verdict_reason を既存 decoder の実在根拠なしに互換性だけで維持する案は no-backward-compat 規約の最小化原則を満たさない; commit-gate observation: strict spec_adr gate では user grounds の IN-04 / AC-04 が Yellow に留まり、ADR grounding なしの保存的解決は成立しない"
    status: proposed
---
