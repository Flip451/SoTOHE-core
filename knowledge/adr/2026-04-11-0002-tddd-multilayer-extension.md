...
| `FunctionInfo` | `params`, `returns`, `receiver`, `is_async` を追加 | rustdoc JSON から構造的に抽出。既存の `signature` 文字列はそのまま残す (人間可読用) |
...
| `TypeNode::outgoing` | `methods` の戻り型から導出 | `methods.iter().filter(returns ∈ typestate_names)` |
