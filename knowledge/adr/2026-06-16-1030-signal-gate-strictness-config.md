...
- **`signal check-adr-user` に `--strict` を新設**する。Strict 対応の adr-signal verifier として `execute_verify_adr_signals_with_strict(project_root, strict)` を新設し、既存の `execute_verify_adr_signals(project_root)` は非 strict のショートカットとして `_with_strict(project_root, false)` に delegate する形に保つ。`strict=true` のとき Yellow（`ReviewFindingRef`）も `Finding::error`、`strict=false` のとき `Finding::warning`。`Red`（`NoGrounds`）は常に `error`、`Grandfathered` は常に skip（不変）。
...
| `libs/infrastructure/src/verify/adr_signals.rs::execute_verify_adr_signals` / `execute_verify_adr_signals_with_strict` | **`_with_strict` を新設**し strict 評価を担当（Yellow block 経路の新設）。`execute_verify_adr_signals` は `_with_strict(_, false)` への薄い delegate として非 strict を維持 |
| `libs/domain/src/tddd/`（新規）+ `libs/usecase/src/merge_gate/chain2_gate.rs::check_chain2_for_layer` + `libs/infrastructure/src/verify/catalogue_spec_signals.rs` | chain ② の Red/Yellow 判定ロジックを domain 層に新規抽出（`check_catalogue_spec_signals(doc: &CatalogueSpecSignalsDocument, strictness: Strictness) -> VerifyOutcome` — chain ①③ と同型の純粋関数（`bool` ではなく `domain::Strictness` enum を貫通させる））。現状 infrastructure と usecase の 2 箇所に inline 実装されている同等ロジックを、新 domain 関数の呼び出しに置換して 1 本化し、config 駆動 strict を解決可能にする — ①③ との関数シグネチャ非対称も同時に解消 |
...
/// 永続化を伴う chain（① ② ③）の **純粋な gate 部分**。I/O は持たず、
/// `load → check_freshness → evaluate_gate` の `evaluate_gate` のみを担う。
...
// ---- usecase layer (I/O port contracts) ----

/// 全 chain が満たす check の最小契約（CLI check dispatch の入口）。
/// I/O を伴うため usecase 層に配置する。
...
/// `signal calc-adr-user` はこの契約を使い、永続ファイルは作らない。I/O port のため usecase 層。
...
/// 永続 I/O 部分（calc / load / freshness）。`PersistedSoTChainGate` の純粋部分と
/// 組み合わせて使う。I/O port のため usecase 層。
...
/// 純粋ゲートと I/O port を結合した「① ② ③ chain の総合契約」。
/// 中身は空の sealed supertrait で、`LoadablePersistedChain` を実装すれば自動的に成立する。
...
/// 永続 chain の SoTChain::check は blanket impl で 1 本化する（usecase 側で
/// `SoTChain` と `PersistedSoTChain` の両方が定義されているため孤立則を満たす）。
...
| `ChainIdentity` + `PersistedSoTChainGate` + `LoadablePersistedChain`（blanket impl で `SoTChain`） | ① ② ③ | 永続化ファイルを持ち freshness 検証が必要。`SoTChain::check` は `load → check_freshness → evaluate_gate` の blanket impl でだけ提供する |

⓪ を 1 本の永続化トレイトに押し込むと不要メソッドを `unimplemented!()` 等で埋めることになり、「make illegal states unrepresentable」（CLAUDE.md `.claude/rules/04-coding-principles.md`）に反する。`PersistedSoTChain` を ⓪ に実装しないことで **「⓪ では永続 `calc` / `check_freshness` を呼べない」をコンパイル時に保証**する。①②③ は `PersistedSoTChain` の blanket impl で `SoTChain::check` を得るため、`check` 経路から freshness を省いた個別実装を作れない。

**chain ⓪ の現行ディスパッチ実装（T006 完了までの暫定）**: `AdrUserChain::calc_live` は usecase 層に taxonomy 上のエントリとして存在するが、現状は T006 のポート配線が未完了のため `AdrFilePortError::ListPaths("not yet wired — see T006")` を返す placeholder である。`signal calc-adr-user` / `signal check-adr-user` の現行 CLI ディスパッチは cli-composition（composition root）から `infrastructure::verify::adr_signals::execute_verify_adr_signals_with_strict(project_root, strict)` を直接呼び出す経路を取り、AdrFilePort の DI 配線が終わるまで trait 経由の dispatch には乗らない（spec AC-10 / AC-11 参照）。T006 で `AdrFilePort` の adapter を usecase に注入できるようになった時点で `AdrUserChain::calc_live` 経由のディスパッチに切替予定。
...
| 変更箇所 | 変更内容 |
| --- | --- |
| `libs/domain/src/chain.rs`（新規） | `ChainId` enum + 純粋ドメイン traits（`ChainIdentity` / `PersistedSoTChainGate`）+ `Strictness` / `GateKind` / `ChainGateEntry` / `SignalGateMatrix` 値オブジェクト |
| `libs/usecase/src/chain/traits.rs`（新規） | dispatch / I/O port traits（`SoTChain` / `LiveSoTChain` / `LoadablePersistedChain` / `PersistedSoTChain`）+ blanket impls `impl<T: LoadablePersistedChain> PersistedSoTChain for T` + `impl<T: PersistedSoTChain> SoTChain for T`（hexagonal: I/O port は usecase 配置） |
| `libs/usecase/src/chain/`（新規, 4 モジュール） | `AdrUserChain`（`SoTChain` + `LiveSoTChain`）/ `SpecAdrChain` / `CatalogSpecChain` / `ImplCatalogChain`（後 3 つは `PersistedSoTChainGate` + `LoadablePersistedChain` 実装により `PersistedSoTChain` + `SoTChain` を blanket impl で得る） |
| `apps/cli/src/commands/signal/` | dispatch を集約。①②③ の `calc-*` は `LoadablePersistedChain::calc`、全 `check-*` は `SoTChain`（①②③ は blanket impl）を経由。chain ⓪ `calc-adr-user` / `check-adr-user` は AdrFilePort の DI 配線（T006）完了まで composition root から `execute_verify_adr_signals_with_strict` を直接呼ぶ経路（spec AC-10 / AC-11） |
...
`signal` 名前空間の実装において、per-layer 反復・active-track 解決・SHA-256 カタログハッシュ計算はすべて **domain または usecase の責務**とする。一方、TDDD レイヤー列挙（`architecture-rules.json` の filesystem 読み取りを含む）は I/O を伴うため **infrastructure 配置**とする（D8-4 のロジック配置表参照）。adapter（CLI コマンドファイル・cli-composition・Makefile タスク）へのドメインロジック / オーケストレーションの漏出を禁止し、ヘキサゴナルアーキテクチャの境界を構造的に固定する。
...
`apps/cli-composition/src/signal.rs` はアダプターグラフの構築・infrastructure primitive のクロージャ生成（依存注入）・usecase オーケストレーターへの呼び出しのみを行う。per-layer ループ・ハッシュ計算・パス構築・domain 判断をこのファイルに含んではならない。usecase オーケストレーターに渡すクロージャのシグネチャは `Fn(LayerId, &str, &str) -> VerifyOutcome` であり、第 2 引数は usecase 内で計算された SHA-256 ハッシュ文字列、第 3 引数は usecase オーケストレーターが `reader.active_track_id()` で解決する `track_id_str`（cli-composition は active-track 解決を行わない）。クロージャ本体は対応する infrastructure 関数への委譲のみとする（例: `|layer_id, hash, track_id_str| infra_check_impl_catalog(infrastructure::verify::tddd_layers::impl_catalog_signals_path(binding, workspace_root, track_id_str), hash, strict)`）。infrastructure 関数群（`catalogue_spec_signals_path` / `impl_catalog_signals_path`）が `workspace_root` + `track_id` + binding から `<workspace_root>/track/items/<track_id>/<file>` パスを構築する責務を持つ。クロージャ本体にビジネスロジック・パス構築・ハッシュ計算・active-track 解決を含んではならない。
...
| TDDD レイヤー列挙（`architecture-rules.json` から読み取るレイヤーセット） | `libs/infrastructure/src/verify/tddd_layers/`（filesystem I/O が伴うため infrastructure primitive — 例: `load_tddd_layers_from_workspace(workspace_root)`）。`ChainId` のような不変識別子は `libs/domain/src/chain.rs` に enum として保持するが、レイヤー列挙そのもの（`architecture-rules.json` の read を含む）は infrastructure 配置とする |
...
2. **TDDD calc/check primitive（`per_layer_fn` ジェネリックパラメーター）**: usecase オーケストレーターは infrastructure の TDDD primitive を `per_layer_fn: impl Fn(LayerId, &str, &str) -> VerifyOutcome` のジェネリックパラメーターとして受け取る。第 2 引数 `&str` は SHA-256 ハッシュ文字列（hex）であり、`reader.catalogue_bytes()` から usecase 内で計算される。第 3 引数 `&str` は `reader.active_track_id()` から usecase 内で解決した `track_id_str` であり、cli-composition は active-track 解決を行わない（D8-4 / CN-17）。ファイルパスは usecase 関数にもクロージャのシグネチャにも含まない — signals ファイルのパスは infrastructure primitive（`infra_calc_impl_catalog` 等）が layer ID から内部で構築する責務を持つ。check variants では cli-composition 側のクロージャが `strict` をキャプチャするため、`per_layer_fn` のシグネチャ自体に `bool` は含まない。
...
- `check-adr-user` が `--strict` を取り、Strict 対応版 `execute_verify_adr_signals_with_strict(project_root, strict)` を新設している（既存 `execute_verify_adr_signals(project_root)` は `_with_strict(_, false)` への delegate として非 strict のショートカットを維持）
- chain ② の Red/Yellow 判定が domain 層の `check_catalogue_spec_signals(&doc, strictness: Strictness)` に抽出され、`merge_gate/chain2_gate.rs` と `infrastructure/.../catalogue_spec_signals.rs` の 2 重 inline 実装がこの 1 本に集約されている（① ③ と同型の関数シグネチャ、`bool` ではなく `domain::Strictness` enum を貫通）
...
- `libs/domain/src/chain.rs` に `ChainId` enum と `ChainIdentity` / `PersistedSoTChainGate: ChainIdentity` の 2 pure-domain traits が定義され、`libs/usecase/src/chain/traits.rs` に `SoTChain: ChainIdentity` / `LiveSoTChain: SoTChain` / `LoadablePersistedChain: PersistedSoTChainGate` / `PersistedSoTChain: PersistedSoTChainGate + LoadablePersistedChain` の 4 usecase-layer traits と blanket impls `impl<T: LoadablePersistedChain> PersistedSoTChain for T` + `impl<T: PersistedSoTChain> SoTChain for T` が定義されている（D7 のハイブリッド配置）
- ⓪ `AdrUserChain` は `SoTChain` + `LiveSoTChain` を実装し、① `SpecAdrChain` / ② `CatalogSpecChain` / ③ `ImplCatalogChain` は `PersistedSoTChainGate` + `LoadablePersistedChain` を実装して blanket impl により `PersistedSoTChain` + `SoTChain` を得る
- chain ⓪ に対して永続 `calc` / `check_freshness` を呼ぶコードが**コンパイルエラー**になる
- ①②③ の `PersistedSoTChainGate::evaluate_gate` は既存 domain 純粋関数（`check_spec_doc_signals` / `check_catalogue_spec_signals`（D2 新設）/ `check_type_signals`）を呼ぶ薄いラッパーとする
- chain ⓪ の `signal calc-adr-user` / `signal check-adr-user` の現行 CLI ディスパッチは AdrFilePort の DI 配線が完了するまで（T006 待ち）`AdrUserChain::calc_live` を経由せず、composition root から `execute_verify_adr_signals_with_strict` を直接呼ぶ経路を取る（spec AC-10 / AC-11 と整合）。①②③ の `calc-*` は `LoadablePersistedChain::calc`、全 `check-*` は `SoTChain`（①②③ は blanket impl）に配線される
