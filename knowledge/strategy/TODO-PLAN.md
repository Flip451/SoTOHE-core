...
| # | 項目 | 難易度 | 状態 |
|---|---|---|---|
| 2b-1 | ~~**TSUMIKI-05** 構造化ヒアリング UX~~ | S | ✅ done (`hearing-ux-improvement-2026-04-01`) |
| 2b-2 | ~~**TSUMIKI-06** ヒアリング作業規模選定~~ | S | ✅ done (`hearing-ux-improvement-2026-04-01`) |
| 2b-3 | ~~**TSUMIKI-07** ヒアリング記録~~ | S | ✅ done (`hearing-ux-improvement-2026-04-01`) |
...
| proptest + typestate | domain impl | `sotp export-schema` のシグネチャ（関数の存在 = 有効遷移） |
| usecase モック自動生成 | usecase | `impl Fn` のクロージャモック |

| # | 項目 | 難易度 | 根拠 |
|---|---|---|---|
| 3-1 | **BRIDGE-01** `sotp export-schema` (syn) | M | 生成プロジェクトの Rust source roots（domain 中心、multi-path で usecase 等も可）から pub シグネチャを抽出。テスト生成 AI の唯一の型コンテキスト |
| 3-2 | **spec 例 → テストスケルトン自動生成** | M | spec の Given/When/Then → `#[test]` を `/track:plan` Phase B で生成 |
| 3-3 | **proptest テンプレート** | M | `sotp export-schema` + typestate から proptest テストを生成 |
...
| 3-12 | **spec ↔ code 整合性チェック** | M | BRIDGE-01 の後続トラック。`sotp export-schema` の出力と spec.md / spec.json の Domain States セクションを突合し、未実装・不一致を検出 |
...
| 3-14 | **CC-SDD-03** EARS 記法 — spec.json 要件に When/If/While/Shall 構造を導入。spec.json schema 拡張 + SKILL.md テンプレート更新 | M | 3-2（spec → テストスケルトン）の入力構造。同時実装が効率的 |
| 3-15 | **TSUMIKI-09** ヒアリング記録の質問粒度向上 — hearing_history に質問-回答ペア保存。「なぜ仕様が変わったか」の証跡 | M | TSUMIKI-04（要件網羅率）での「何を確認したか」の証跡。HearingRecord schema 拡張 |
...
> 更新: 2026-04-01。Phase 2b 完了（TSUMIKI-05/06/07）。Phase 3 に 3-14 CC-SDD-03（EARS 記法）、3-15 TSUMIKI-09（ヒアリング記録粒度向上）追加。TODO.md に TSUMIKI-09 新規登録、CC-SDD-03 に Phase 配置追記。
