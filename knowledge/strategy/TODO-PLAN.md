...
| 1.5-29 | **WF-43**: verdict 改ざん防止 — provenance + write-guard + CLI 封鎖 + SHA-256 検証 | L | archived (planned のまま凍結、review system v2 で superseded) |
...
| 4-6 | ~~ERR-08 pr-review 中断耐性~~ | M | ✅ done (`pr-review-flow-fix-2026-03-31`) |
...
| 5-8 | **WF-67** agent-router 廃止 + skill 遵守フック導入（Python → Rust 移行） — agent-router.py（intent 検出 + ルーティングヒント注入）を廃止し、代わりに `/track:*` コマンド検出時に SKILL.md フェーズ遵守を強制するフックを Rust（`sotp hook dispatch`）で実装。external guide injection も引き継ぐ。ADR: `2026-04-08-1200-remove-agent-router-hook.md` | M |
...
> 更新: 2026-04-06。Phase 2b 完了（`hearing-ux-improvement-2026-04-01`）。ERR-08/WF-66 完了（`pr-review-flow-fix-2026-03-31`）。RVW-34/RVW-37 完了。review-system-v2 完了。planner-claude-migration Phase 1 完了。
