...
- [ ] **harness-hardening-nutype-rustdoc-support** (HIGH) — `schema_export` が `pub use alias::<Type>;` 経由で公開される型 (特に `nutype` が生成する `#[doc(hidden)]` module 内の struct) を rustdoc JSON から抽出できない。現状は `Item::Struct` / `Item::Enum` / `Item::Trait` のみ列挙し、`Item::Use` のターゲットを辿らないため、`nutype` で wrap した型 (`TrackId` / `TaskId` / `CommitHash` / `TrackBranch` / `NonEmptyString` / `ReviewGroupName`) が TDDD forward check で常に `found_type=false` (Yellow) になる。track `tddd-contract-map-phase1-2026-04-17` では `LayerId` を素 struct で workaround したが、残存 6 nutype 型の catalogue declaration はすべて同じ問題を抱える。修正: `libs/infrastructure/src/schema_export.rs` に `Item::Use` resolution を追加し、alias 先の struct/enum を alias name で `TypeInfo` に記録する。影響範囲確認必要 (reverse check で undeclared red が増える可能性 → 既存 catalogue 側に nutype 型の reference declaration を追加するか、skip-list 機構を導入する)。関連: ADR 2026-04-11-0002 §D6 (rustdoc-only policy), `knowledge/conventions/prefer-type-safe-abstractions.md`
