...
- [x] ~~**RVW-33** (MEDIUM): review.json cycle の frozen partition scope 接続~~ ✅ 解決: autorecord-reviewjson-wiring T005/T006 で per-group scope hash を実装。`group_scope_hash` が `git ls-files -s` から (mode, path, blob_hash) manifest を構築し SHA-256 で hash 化。record-round (write) と check_approved (read) の両方が per-group scope hash を使用。auto-create cycle の空スコープ問題は残留（正式な cycle 開始は `start_review_cycle` 経由で解決済み）。policy_hash/partition_changed 検証は cycle.rs の `check_cycle_approved` / `check_cycle_staleness_any` で実装済み
