...
- [ ] **WF-82** (MEDIUM): agent file 内 CLI subcommand output 表記の整合性 audit — `.claude/agents/type-designer.md` 旧 Internal pipeline は「`bin/sotp track type-signals` writes `<layer>-types.md`」と記述していたが、実装 (`apps/cli/src/commands/track/tddd/signals.rs` L1004-1005 / L1073-1074) は assert で「`<layer>-types.md` must NOT be written by this function」を保証 (`<layer>-types.md` の rendering は `sync_rendered_views` に centralised)。本 track T002 で agent が偶然発見し fix したが、同様の **CLI 動作と doc 記述の不整合** が他 agent file (`.claude/agents/spec-designer.md` / `impl-planner.md` / `adr-editor.md` / `review-fix-lead.md`) や他 doc (`knowledge/conventions/` / `track/workflow.md`) にも潜在の可能性。設計案: agent file 内の `bin/sotp ...` invocation と「writes / generates / outputs」記述を抽出し、対応する Rust 関数の assert と相互チェックする `verify-agent-cli-claims` mechanism (CI 組み込み候補)。出典: type-designer-tuning-2026-04-25 track T002.

- [ ] **WF-83** (MEDIUM): `/track:full-cycle` の post-commit state finalization の扱い — 各 task commit 後に `cargo make track-transition T<n> done --commit-hash <hash>` を実行すると `impl-plan.json` (T<n>.commit_hash 記録) と auto-rendered `plan.md` が変化するが、その差分は次 task の commit に持ち越される。最後の task commit 後にも同様の差分が残り、loop の末尾に「state finalization commit」を 1 つ追加する必要が生じる (本 track では `chore(plan-artifacts): T003 done finalization` として 5fc2c37 を追加)。改善案: (a) 各 task commit に finalization 差分を pre-stage で組み込む、(b) `/track:full-cycle` SKILL に「最後に finalization commit を必ず追加する」を明記、(c) `track-transition done --commit-hash` を commit 前に呼ぶ運用に変更。出典: type-designer-tuning-2026-04-25 track full-cycle 完遂時。

- [ ] **WF-84** (LOW): `track-transition` の `todo` → `done` 直接遷移エラーと SKILL 記述の整合 — `/track:full-cycle` SKILL は「After commit, record the hash: `cargo make track-transition <track_dir> <task_id> done --commit-hash <hash>`」と書いているが、現実装 (`bin/sotp make track-transition`) は `todo → done` 直接遷移を `unsupported target status: cannot transition from 'todo' to 'done'` で reject (`in_progress` 経由必須)。改善案: (a) SKILL.md に「`in_progress` を経由する」を明記、(b) `track-transition` で `todo → done` 直接遷移を許可 (state machine の relaxation)。出典: type-designer-tuning-2026-04-25 track T001 完了時。

- [ ] **WF-85** (LOW): `bin/sotp make track-set-commit-hash` の命名 / help message が紛らわしい — help は `usage: track-set-commit-hash <track-id>` のみで task-id 引数なし。実態は `.commit_hash` (track-level diff base) ファイル更新であり、`impl-plan.json` の task-level `commit_hash` 記録ではない (後者は `track-transition done --commit-hash` で行う)。orchestrator が用途を誤解しやすい。改善案: (a) `track-set-diff-base-hash` などへの rename、(b) help message に scope (「diff base for review hash computation」など) を明示。出典: type-designer-tuning-2026-04-25 track T001 完了時の orchestrator 誤解。

- [ ] **WF-86** (MEDIUM): `block-direct-git-ops` hook error message の不親切さ — hook が拒否した際に表示される error は `PreToolUse:Bash hook error: [...]: No stderr output` のみで、どの pattern (`>` redirect / git keyword in literal / file-write command 等) がトリガーかが分からない。orchestrator が root cause 特定に時間を要し、推測で誤判断 (「特定キーワード `track` がブロックされている」など) を user に伝えるリスクが生じた (本 track では `2>/dev/null` という CLAUDE.md 既存禁則が trigger だった)。改善案: hook の `policy.rs` (`libs/domain/src/guard/policy.rs`) で blocked verdict を返す際に該当する `*_MESSAGE` (e.g., `OUTPUT_REDIRECT_MESSAGE` / `GIT_ADD_MESSAGE` / `ENV_COMMAND_MESSAGE`) を stdout に出力する path を再確認し、Claude Code hook integration が確実に表示するよう繋ぎ込みを修正 (現状 reason は出ているが Claude Code 側で `No stderr output` と表示されているのが正しいか調査必要)。出典: type-designer-tuning-2026-04-25 track 中の orchestrator hook ブロック誤対応。
