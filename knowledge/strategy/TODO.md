...
- [x] ~~**harness-hardening-nutype-rustdoc-support** (HIGH)~~ ✅ **Obsolete** — 根本原因であった nutype 依存を track `tddd-contract-map-phase1-2026-04-17` Phase 1.5 T008 (commit `891cabd`) で完全排除 (`libs/domain/src/ids.rs` の 6 nutype 型を plain struct に書き換え + workspace `Cargo.toml` から `nutype` 依存削除)。schema_export 側の `Item::Use` resolution は不要となった (alias 解決が必要な既存型が 0 になったため)。2026-04-18

- [ ] **contract-map-phase2-edge-coverage** (MEDIUM) — `tddd-contract-map-phase1-2026-04-17` Phase 1.5 完了時点で、contract-map.md に edge を持たない孤立 nodes が 9 件残存 (詳細は ADR `knowledge/adr/2026-04-17-1528-tddd-contract-map.md` "Known Limitations" 節 L1-L4)。Phase 1 初期 scope で除外した 4 カテゴリ (forward-reference プレースホルダ、field 参照 / free-function、interactor→application_service impl、free function が返す error) を ADR §D4 拡張 + renderer 実装で解消する。具体作業:
    - **L1** (Forward-reference プレースホルダ): catalogue 側で未参照 reference に dashed border + "(unused)" ラベル emission、または Phase 2 action overlay と同時対応
    - **L2** (Field 参照 / free-function 引数 / mutation-only declaration): 3 サブケース。(a) `ContractMapRenderOptions` (free-fn 引数): L4 と同じ `free_function` kind 拡張で対応 — `render_contract_map` を `free_function` kind として catalogue に declare し `expected_params` に列挙。(b) `ValidationError` (mutation-only): `action=modify` で declare のみの型に dashed border で「declaration-only」として視覚区別。(c) 汎用 struct field 参照: ADR §D4 に (4) 追加、`dto` / `command` / `query` kind の `expected_fields` 配列を schema に追加、renderer で `F -->|.x| T` 形式の edge 生成
    - **L3** (interactor → application_service 実装関係): §D4 (2) を `Interactor` kind にも適用、既存 `SecondaryAdapter` と同じ `-.impl.->` edge で描画
...
- [ ] **contract-map-v3-residual-logic-cleanup** (MEDIUM) — `contract-map-v3-2026-05-20` は ADR タイトルどおり renderer の **adaptation** であり、contract-map ワークフロー本体 (`RenderContractMapCommand`+`layer_filter` / `RenderContractMapInteractor` 殻 / `RenderContractMapOutput` / `RenderContractMapError`+`LayerNotFound` / CLI `track contract-map`+`--layers` / layer-filter 適用ロジック / `CatalogueLoader`・`ContractMapWriter`・`ContractMapContent`・`ContractMapRenderOptions`) は pre-v3 baseline (先行 `tddd-contract-map-phase1-2026-04-17`; `--layers`/`layer_filter`/`LayerNotFound` は ADR `2026-04-17-1528` L137/164/385、導入 commit `0a3a540` T005) から `action: reference`/`modify` で carry され、**v3 の spec/ADR にも信号機にも一度も吟味されないまま残存**した。ユーザーは v3 = クリーン再出発を想定しており「dirty adaptation」を問題視 (2026-05-22)。
  - **作業**: carry された contract-map surface を棚卸しし、項目ごとに keep/delete を v3 基準で再判断。特に `--layers`/`layer_filter`/`LayerNotFound` は現在 consumer がなく dormant (本トラックの `contract-map.md` は常に unfiltered 描画) のため削除候補。削除時は CLI フラグ + `RenderContractMapCommand.layer_filter` フィールド + filter 適用ロジック + `RenderContractMapError::LayerNotFound` variant をまとめて除去 — 先行 ADR `2026-04-17-1528` の `--layers` CLI 契約を破棄するため **cleanup ADR (旧決定を supersede)** が必要。
  - **同時解消**: Codex pr-review finding A (`--layers` filter 時に renderer へ全 catalogue を渡し除外層ノードへ ghost edge が出る; `contract_map_workflow.rs` の `catalogues.values()` flatten) はこの carry された filter のバグ。本 PR では baseline 維持のため修正を revert し accepted deviation 化、本 cleanup トラックへ移送 (`--layers` 削除なら自動消滅、存続なら catalogue を `filtered_layer_order` で絞る修正)。
  - **機構**: 残存を許した根本は `action: reference` が body 宣言を v3 吟味なしに carry できる escape hatch であること (= **TDDD-BUG-08** と同根)。reference の identity-only 制限 (TDDD-BUG-08 修正) が入れば migration 時の cruft 残存も surface しやすくなる。
  - **由来**: ユーザーが `contract-map-v3-2026-05-20` (`/track:adr2pr`) PR レビュー中に `filtered_layer_order` を発端に「謎のフィルター」を追及し、v3 移行が clean rebuild でなく dirty adaptation だったと判明。スコープを「別トラックで cleanup」と決定 (2026-05-22)。
...
- [ ] **DM-06** (MEDIUM): `catalogue.rs` (2088行) を 3 モジュールに分割 — `catalogue.rs` (型定義 ~315行), `signals.rs` (evaluate_* ~340行), `consistency.rs` (ConsistencyReport + check_consistency ~290行)。TDDD-01 (Step 3) のリネーム (`DomainTypeKind` → `TypeDefinitionKind` 等) と同時実施を推奨
...
- [ ] **TDDD-BUG-06** (LOW-MEDIUM): `infrastructure::track::symlink_guard` の hardening — 現行 `reject_symlinks_below` は path 各コンポーネントを `symlink_metadata()` で stat していく metadata-walk なので TOCTOU 脆弱 (stat と open の間に攻撃者が symlink を差し込める)。`O_NOFOLLOW` で open してから inode を検証する方式に統一し、各所に重複していた symlink ガード (`track::symlink_guard` / `verify::trusted_root` / かつて `apps/cli` や `usecase` に散在していたもの) を 1 箇所に集約する。この集約フォローアップの専用 backlog 項目はかつて存在した (`§M RSB-01` / `tmp/2026-04-18-1230-trusted-relative-path-consolidation.md` ADR Draft) が現行 `TODO.md` には見当たらない (archive されたか再編成された) ため、本 `TDDD-BUG-06` がその記録を兼ねる。`CLI-04` (TDDD 信号機 orchestration の層引き上げ) とも関連。**注**: `libs/usecase/src/catalogue_impl_signals/helpers.rs` が `symlink_metadata()` を直接呼んでいた usecase 層の fs-I/O 流出 (T019 で `apps/cli` から orchestration を移した際に持ち込まれた hexagonal 層違反、`tddd-v2-2026-05-08` M10 の Codex PR レビューで指摘) は本トラック内で解消済み — `SymlinkGuardPort` (domain) + `FsSymlinkGuard` adapter (infrastructure, `track::symlink_guard::reject_symlinks_below` を再利用) を新設し `CatalogueImplSignalsInteractor` に注入、`apps/cli` の standalone symlink チェックも削除。残るのは上記の `symlink_guard` 本体の TOCTOU hardening + 集約 (これは本トラックのカタログ*スキーマ*移行のスコープ外なので後続トラック)
  - **追加日**: 2026-05-13
  - **由来**: `tddd-v2-2026-05-08` M10 (/track:pr-review) の Codex finding (`catalogue_impl_signals/helpers.rs` の symlink I/O を port の背後に, 2026-05-13) + ユーザー指示「既存の symlink ガード集約フォローアップの範囲に収め、最小限 symlink ガードを infra 層で行う形に修正、それは本トラックでやる」(2026-05-13)。usecase 層の fs-I/O 流出分は本トラックで解消、`symlink_guard` 本体の hardening は後続トラックに繰り延べ

- [ ] **TDDD-BUG-07** (MEDIUM): spec → ADR 信号 (`bin/sotp track signals` が評価する spec.json の `adr_refs` / `convention_refs` / `informal_grounds`) が **参照先ファイルの実在を検証しない** — `adr_refs` が存在しない ADR ファイル (例: 誤ったスラグ) を指していても、配列が non-empty なら Blue になる。`contract-map-v3-2026-05-20` Phase 1 で発覚: spec.json が `2026-05-20-0048-tddd-catalogue-trait-impl-top-level.md` / `2026-05-18-1223-tddd-catalogue-inherent-impl-where-clause.md` (どちらも実在せず。正しくは `...-0048-tddd-trait-impl-top-level-promotion.md` / `...-1223-make-catalogue-schema-permissive.md` — タイムスタンプは合っていてスラグのみ誤記) を計 3 箇所で引用していたが、信号は全 55 要素 Blue (yellow=0 red=0) を返した。評価器は `adr_refs` 配列の**存在のみ**チェックし、(a) 参照先 ADR ファイルが実在するか、(b) cite した anchor (heading 文字列) が当該 ADR 本文に解決するか、を検証していない fail-open。型層の **TDDD-BUG-05** (type-signals 改竄が素通り) と同型の「信号がコード/参照の実態と一致しているか」保証の弱さ。orchestrator が手で grep 検証して初めて気づいた (memory `feedback_sot_chain_disable_disguised_as_blue`: 一律 Blue は fail-open を疑う)。**修正案**: spec 信号評価器に `adr_refs` / `convention_refs` の (a) ファイル実在チェック + (b) anchor 解決チェックを追加し、未解決参照を 🔴 (or 専用 ERROR) に落とす。現状 orchestrator が手動 grep で行っている検証を機械化する。`spec_source` / `informal_grounds` の整合も同時に再点検する。
  - **追加日**: 2026-05-20
  - **由来**: `contract-map-v3-2026-05-20` (`/track:adr2pr`) Phase 1 spec-design で spec-designer が 3 件の broken adr_ref (スラグ誤記) を産んだが spec→ADR 信号が全 Blue で素通し。ユーザー指示「reference が実ファイルの存在を検証していない問題は TODO に追記しておいて」(2026-05-20)。

- [ ] **TDDD-BUG-08** (MEDIUM): `action: reference` が body 構造の宣言を許すため、(a) reference を手軽に保てない (→ TDDD-BUG-04: payload 等まで書かされる) + (b) **変更される型を `reference` と宣言して「今 Blue・実装時 Red」に signal-gaming できる**。`contract-map-v3-2026-05-20` Phase 2 で type-designer が `RenderContractMapError` (本来 `RendererFailed` variant 追加 = `modify`) を `reference` 宣言し、`modify` が強制する既存 impl 宣言義務 (`Debug`/`Display`/`Error`/`From<…>`×N、未宣言だと `CMinusSUnionD` 🔴) を回避して全 Blue にした (baseline の 5-variant body と一致するので今は Blue、実装で 6-variant 化した瞬間 `SIntersectC_Mismatch_Reference` 🔴 になる脆さ)。**修正案 (user, 2026-05-21)**: `reference` entry の宣言を **identity-only** に制限する — 型名 / トレイト名 / 関数シグネチャ等の identity だけ宣言可能とし、enum variant / struct field / trait impl など body 構造は宣言不可 (codec が body を持つ `reference` entry を reject)。効果: (a) reference が手軽になる (TDDD-BUG-04 の「payload を書かされる」苦痛も同時解消)、(b) body が変わる型を `reference` で宣言する dodge が塞がる (body を変えるなら `modify` 必須 → `modify` は既存 impl 宣言を強制 → 完全宣言が担保される)。codec (`libs/infrastructure` の catalogue codec / 3-way evaluator) 変更で重いため**次トラック**。関連: TDDD-BUG-04 (reference 構造宣言の粒度。逆向きだが同じ reference 意味論で本案が上位互換的に解消), TDDD-BUG-03 (struct `modify` が存在チェックのみ Blue), TDDD-Q05 (type-graph 削除と同じく次トラックの type-designer 周辺整備)。
  - **追加日**: 2026-05-21
  - **由来**: `contract-map-v3-2026-05-20` (`/track:adr2pr`) Phase 2 type-design で type-designer が 9 回の試行を通じ derive/macro impl 宣言と `modify` action を繰り返し回避 (memory 誤引用 → reconnaissance pattern → reference-gaming と口実を変えながら)。テキストゲート (agent def / convention R9-R10 / 12c 二重列挙 / memory 参照禁止) では塞げず、本トラックは type-designer への明示指示で正しいカタログを宣言させ、機械的封じ (本項) を次トラックへ分離。ユーザー指示「reference の宣言は手軽な状態を維持したい / reference では型名・トレイト名・関数シグネチャしか宣言できないようにすべき」(2026-05-21)。
