...
mod dry_fix_runner {
    ...
    impl CliApp {
        pub fn dry_run_fix_local(
            ...
            let model =
                input.model.clone().or_else(|| resolved.model.clone()).ok_or_else(|| {
    ...
    pub(crate) fn run_dry_fix_codex(
        ...
        let status = parse_dry_fix_sentinel(&last_message_content)
            .or_else(|| parse_dry_fix_sentinel(&stdout));
    ...
    fn dry_fix_resolve_codex_home() -> Result<PathBuf, String> {
        ...
        let home = std::env::var("HOME").map_err(|e| {
            format!("HOME env var is not set (cannot resolve default CODEX_HOME): {e}")
        })?;
    ...
    pub(crate) fn build_dry_fix_invocation(
        ...
        let mut args =
            vec![OsString::from("exec"), OsString::from("--model"), OsString::from(model)];
    ...
    pub(crate) fn dry_fix_spawn_and_collect(
        ...
        let stdout_handle =
            thread::spawn(move || collect_pipe(stdout_pipe, false, &stdout_redactions));
        ...
        let exit_status =
            child.wait().map_err(|e| format!("failed to wait for Codex fixer: {e}"))?;
        let status_str = exit_status.to_string();
        let (stdout, stdout_error) = dry_fix_collector_result_for_log(
            join_dry_fix_collector(stdout_handle, "stdout"),
            "stdout",
        );
        let (stderr, stderr_error) = dry_fix_collector_result_for_log(
            join_dry_fix_collector(stderr_handle, "stderr"),
            "stderr",
        );
