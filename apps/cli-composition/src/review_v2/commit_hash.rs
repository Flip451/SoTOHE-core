//! Composition-layer helper for persisting the current HEAD SHA as the v2 diff base.
//!
//! Encapsulates all domain type construction (`TrackId`, `CommitHash`,
//! `CommitHashWriter`) so that the CLI layer (`commands/make.rs`) does not need
//! to import those domain types directly (CN-01 / AC-03).

use infrastructure::review_v2::FsCommitHashStore;
