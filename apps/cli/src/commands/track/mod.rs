...
impl TrackCommand {
    ...
    pub fn items_dir(&self) -> PathBuf {
        match self {
            ...
            // MergeBase is intentionally argument-free; it resolves the workspace
            // from the current directory in its dispatch path.
            TrackCommand::MergeBase => PathBuf::from("track/items"),
...
mod tests {
    ...
    fn test_track_command_merge_base_is_argument_free_and_uses_default_items_dir() {
        ...
        assert_eq!(command.items_dir(), PathBuf::from("track/items"));
    }
