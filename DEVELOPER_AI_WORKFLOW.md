...
- 既定の担当は次のとおり。
  - Claude Code: `orchestrator` / `planner` / `designer` / `implementer`
  - Codex: `reviewer`
  - Gemini: `researcher`
...
- AI:
  ...
  - 必要に応じて `planner` / `designer` / `researcher` / `implementer` / `reviewer` を内部的に使う（実体は `.harness/config/agent-profiles.json` と §5 を参照）
...
- provider resolution:
  ...
  - 既定は `orchestrator` / `planner` / `designer` / `implementer` = Claude Code、`reviewer` = Codex、`researcher` = Gemini
