...
- specialist capability の割り当ては `.harness/config/agent-profiles.json` で切り替える。
- 既定の担当は次のとおり。
...
flowchart TD
    ...
    K2 --> I2["/track:pr-review（push+PR+review）"]
    I2 --> M
    M --> N["/track:done"]
```
...
| `/track:done` | main に切り替え + pull + 完了サマリー | トラック完了後に main に戻る時 |
...
## 1.2 Track 前段階 + 3 フェーズ構成

track 成果物は「ADR → 仕様 → 型契約 → 実装計画」の SoT Chain に沿って段階的に作成する。
...
- `knowledge/conventions/workflow-ceremony-minimization.md` — 事後レビュー方式、`approved` 状態廃止、事前承認を不可逆 action に限定する原則

## 1.3 Project Conventions は早めに整備する
...
- AI:
  ...
  - 必要に応じて `spec-designer` / `type-designer` / `impl-planner` / `adr-editor` / `researcher` / `implementer` / `reviewer` を内部的に使う（実体は `.harness/config/agent-profiles.json` と §5 を参照）
...
8. `[Claude Code]` `/track:commit <message>`
   - 直接 `git commit` の代わりにガード付きコミットを実行する
9. `[Claude Code]` `/track:pr`
   - ブランチを push し PR を作成（既存 PR は再利用）
10. `[Claude Code]` `/track:merge <pr>`
    - CI 通過を待ち PR をマージする
11. `[Claude Code]` `/track:done`
    - main に戻り完了サマリーを表示する
...
- specialist capability:
  - `spec-designer`: 振る舞い契約 (spec.json) の author
  - `type-designer`: 型契約 (`<layer>-types.json`) の author (TDDD ワークフロー)
  - `impl-planner`: 実装計画 (impl-plan.json + task-coverage.json) の author
  - `adr-editor`: ADR の back-and-forth 修正 (`/track:plan` から自動 invoke)
...
3. 作業完了後、`/track:pr` でブランチを push して PR を作成する
4. `/track:merge <pr>` で CI 通過後に PR をマージする
5. `/track:done` で main に戻り完了サマリーを確認する
6. 必要に応じて `/track:archive <id>` でトラックをアーカイブする
