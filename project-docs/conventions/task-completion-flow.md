...
4. commit_hash 埋め戻し（任意だが推奨）
   ...
   - metadata.json のタスクに commit_hash フィールドを手動追加
...
`done → done` の遷移は状態機械で禁止されているため、`track-transition` で commit_hash を埋め戻せない。metadata.json を直接編集する必要がある。将来のテンプレート改善で解消予定。
...
- commit_hash の埋め戻しは任意（WF-40 解消後は track-transition で一括実行可能になる予定）
...
- `tmp/TODO.md` — WF-40（done → done 遷移の設計バグ）
