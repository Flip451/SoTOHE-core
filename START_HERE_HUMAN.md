...
3. レビューや運用判断が必要なときは `TAKT_TRACK_TRACEABILITY.md` の 2章（対応付けルール）と 4章（Interactive Implementation Contract）を参照する
...
- AI が担当すること:
  ...
  - takt 実行時の movement 遷移と profile-aware 実行
...
- `docs/architecture-rules.json` の `layers[].path` で定義された workspace member 配下（例: 初期状態では `apps/**`, `libs/**`）
- `track/**`
- `docs/**`
- `project-docs/**`
- `.github/workflows/**`
...
- `.claude/commands/**`, `.claude/docs/**`, `.claude/skills/**`, `.claude/rules/**`, `.claude/hooks/**`（AI 運用フローを変更する場合のみ）
...
- `.claude/agent-profiles.json`, `.claude/settings*.json`（運用ポリシー変更時のみ）
- `.codex/instructions.md`, `.gemini/GEMINI.md`（利用エージェント設定を見直す場合のみ）

人間が手動編集しない:

- `.takt/**`（状態ファイルと runtime persona）
- `target/**`（ビルド生成物）
- `.cache/**`（外部ガイドのキャッシュ）
- `.claude/logs/**`（実行ログ）
...
- 復旧後に `cargo make takt-render-personas` 等で再生成し整合を戻す

## 5. 安全運用ルール

- profile-aware な takt 実行は wrapper (`cargo make takt-*`) を使う
