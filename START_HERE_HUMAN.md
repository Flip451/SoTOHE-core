...
3. レビューや運用判断が必要なときは `track/workflow.md` の Task Workflow / track/registry.md 更新ルール / Quality Gates を参照する
...
2. `/track:review` または `cargo make ci` 完了後
   - 重大指摘がないこと、観測が必要なタスクでは `observations.md` が更新されていることを確認する (機械検証のみで完結するタスクでは不在が正常)
...
- `track/**`
- `knowledge/**`（Knowledge Base / ADR / convention / strategy）
- `.github/workflows/**`
- `tmp/**`（作業メモ・レポート）
...
- `.claude/commands/**`, `.claude/skills/**`, `.claude/rules/**`（AI 運用フローを変更する場合のみ）
...
- `target/**`（ビルド生成物）
- `.cache/**`（cargo registry / sccache / pytest 等のキャッシュ）
- `.claude/logs/**`（実行ログ）

例外:
- 障害復旧で緊急に編集する場合は、理由を `observations.md` に残す (存在しなければ新規作成する)
