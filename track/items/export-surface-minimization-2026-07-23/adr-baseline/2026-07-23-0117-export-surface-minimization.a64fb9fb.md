...
つまり出荷 Makefile の原則は既決でも、それを守らせる恒常機構が存在せず、原則制定の直後に追加されたゲートで早速 regression が実証された。
...
規約索引は自動生成のため、export 後の scaffold で部分集合の索引へ再生成される手当て（export 時実行または bootstrap 組み込み）を同時に行う。
...
出荷対象は dev-environment / orchestration / guardrails 相当のみとし、maintainer-checklist は exclude、language は exclude か中立版を overlay で差し替える。
CLAUDE.md・rules 相互参照・workflows の参照更新を同 track の co-update に含める。

### D4: 既決の出荷 Makefile 原則に、スモークゲートの恒常検査を与える
...
検査は既存のスモークゲートに組み込む。
スモークは export 済みツリーを検分する maintainer CI のゲートであり、違反を作れるのも直せるのも maintainer だけなので、ここで main 到達前に止めれば足りる。
`sotp template export` 本体は変更せず、テンプレート利用者の export には一切触れない。
既に肥大化している verify 系列（`sotp verify` サブコマンド群と ci 依存列）にも載せない。

検査の導入は、Context に記した現存 regression 2 件の修復を実装上強制する——review workflow の wrapper 参照は `bin/sotp adr-baseline check-review` 直呼びへ正規化して単発 wrapper をソース Makefile から削除し（既決の直呼び原則と「単発は bin/sotp 直、cargo make は集約のみ」の dev-environment 規約の適用）、scaffold の ci-track には `bin/sotp adr-baseline check-commit` のステップを補完する（既決の採録基準の適用）。
これらの修復は既決事項の適用であり、本 ADR の新規決定ではない。
...
2 件の修復は既決事項の適用として D4 の検査導入に随伴させ、本 ADR の決定は強制機構に限定する。
...
- 検査は maintainer CI のスモークゲートのみに住み、参照欠落クラスは main 到達前に止まる。利用者の export には変更が及ばず、verify 系列も伸びない。
- D4 の検査導入が現存 regression 2 件の修復を強制し、scaffold の commit ゲートが成立する。
...
- `knowledge/adr/2026-07-16-1438-consumer-scaffold-host-first-makefile.md` — 配布 Makefile の採録基準・直呼び原則の正本（D4 はその強制機構）
