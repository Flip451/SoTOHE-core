...
---
status: approved
approved_at: "2026-04-17T00:32:18Z"
version: "1.0"
...
- 3 層で sotp track type-graph --cluster-depth {1,2} --edges all を実行し verification.md に層別 node/edge/cluster 数・可読性比較・trait impl 破線の hexagonal 可視化有効性を記録する。ADR 2026-04-16-2200 §Phase 2 Scope Update §S4/§S5 に実測データ (node/edge/cluster 数、dedup 結論、scope (K) 延期正当性) を補強追記する (§S1-§S5 の本体構造は planning 時に追加済み) [source: knowledge/adr/2026-04-16-2200-tddd-type-graph-view.md §D7 Phase 2 Acceptance, §D8, knowledge/adr/2026-04-16-2200-tddd-type-graph-view.md §Phase 2 Scope Update §S4] [tasks: T006]
...
- State machine view (stateDiagram-v2 で enum state machine を描画する機能) は TypeGraph とは別のデータソースを必要とするため本 track では扱わない。別 ADR 案件として将来的に検討する [source: knowledge/adr/2026-04-16-2200-tddd-type-graph-view.md §Phase 2 Scope Update §S3.1]
- architecture-rules.json の module 粒度拡張 (例: domain::guard may_depend_on domain::shared) は別 ADR 案件として扱う [source: knowledge/adr/2026-04-16-2200-tddd-type-graph-view.md §Phase 2 Scope Update §S3.2]
...
- Backward compat なし clean break (TDDD-BUG-02): check_type_signals の signature 変更は破壊的に行う (TDDD-BUG-02 の修正案として ADR §D9 に明記) [source: knowledge/adr/2026-04-16-2200-tddd-type-graph-view.md §D9 TDDD-BUG-02]
...
- [ ] verification.md に deduplicate_typestate_edges default の実測判断が記録されている (ADR Open Questions §4 への実測解答) [source: knowledge/adr/2026-04-16-2200-tddd-type-graph-view.md §Open Questions §4, knowledge/adr/2026-04-16-2200-tddd-type-graph-view.md §Phase 2 Scope Update §S4] [tasks: T006]
- [ ] ADR 2026-04-16-2200 の §Phase 2 Scope Update に scope (K) 縮小と DRIFT-01 / entry-point / auto-render / orphan 延期の根拠が記録されている (track T006 の acceptance として ADR を参照) [source: knowledge/adr/2026-04-16-2200-tddd-type-graph-view.md §Phase 2 Scope Update §S2] [tasks: T006]
