...
decisions:
  ...
  - id: D3
    ...
    status: proposed
  - id: D4
    user_decision_ref: "chat_segment:baseline-hash-freshness-decision:2026-08-01; chat_segment:phase0-converged-adr-approval:2026-08-01; chat_segment:authority-availability-boundary-directive:2026-08-05"
    status: proposed
---
...
### D4: type-signals の新鮮度判定に baseline hash を含め、検証範囲を権威の可用性で決める

現在アクティブなトラックを対象として、type-signals cache に `baseline_hash` を記録し、catalogue declaration hash、implementation input hash、baseline hash の 3 値がすべて一致する場合に限り既存評価を再利用する。いずれかの不一致、cache の不在、decode 不良は再評価とし、成功した場合だけ評価結果と現在の 3 hash で cache を原子的に置き換える。

検証の範囲は権威の可用性で決める。3 hash の完全検証と、baseline の欠損・読み取り失敗を fail-closed とする扱いは、権威がそろうローカルの pre-commit workspace に適用し、そこを baseline 鮮度の唯一の強制点とする。権威が構造的に不在のコンテキスト（fresh な CI checkout、merge gate）は committed な権威のみで検証する。判定は非対称であり、**存在する権威が読めない・一致しない場合は常に fail-closed**、検査を降格してよいのは構造的不在の場合だけである。信号機評価の失敗と cache 書き込みの失敗も fail-closed とする。

inactive・archived track の既存成果物は走査・移行・検証しない。type-signals の手動削除を baseline 再取得の通常手順には含めない。
...
### B: baseline hash の committed attestation artifact を導入する

merge gate でも baseline 鮮度を照合できるよう、baseline hash を宣言する成果物を commit 対象として新設する案。新しい artifact lifecycle の設計拡張であり本決定の範囲を超えるため却下する（独立した決定として再検討の余地は残る）。
