...
decisions:
  ...
  - id: D3
    user_decision_ref: "handoff:tmp/handoff/2026-08-02-lane-d-delta-adjudication.md#裁定1:admit; chat:2026-08-02 ユーザー確認「OK。進めて。」"
    review_finding_ref: "pr-236-review:cleanup-view-staleness:2026-08-11"
    status: accepted
---
...
### D3: clean merge の後始末は Views → Baseline → ViewsAfterBaseline → SyncBaseStamp の順でのみ完了する

clean merge の後始末は Views → Baseline → ViewsAfterBaseline → SyncBaseStamp の順を維持する。
Baseline の publish が成功した後、影響を受ける rendered view を新たに publish された baseline に対して
再生成する。sync-base stamp は Views、Baseline、ViewsAfterBaseline のすべてが成功した後にだけ書き込む。
これにより、superseded baseline に対して評価された signal を tracked view に残したまま完了を報告しない。
ViewsAfterBaseline の失敗は既存 stage と同じく fail-closed とし、rendered view の prior state を維持する。
いずれかの段階が失敗した場合、merge 操作は後始末を含む完了を報告してはならない。conflict outcome
では、この clean-merge cleanup を実行しない。
