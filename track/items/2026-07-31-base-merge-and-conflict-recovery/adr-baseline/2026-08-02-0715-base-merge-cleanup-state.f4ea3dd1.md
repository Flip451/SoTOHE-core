...
decisions:
  ...
  - id: D2
    ...
    status: accepted
  - id: D3
    user_decision_ref: "chat:2026-08-12-strip-excess-decisions; chat:2026-08-12-baseline-loss-manual-recapture"
    status: accepted
---
...
競合した merge では Baseline 段階だけを実行し、供給源には競合中の作業ツリーではなく、ガード付き merge コマンドが実際に取り込んだ base commit を用いる。

Views と SyncBaseStamp は競合なしの merge に限り、競合した merge では半 merge 状態から表示用ファイルを生成せず、完了も記録しない。
...
- Bad: 後始末の失敗後は operator が baseline を手動で再取得する。
