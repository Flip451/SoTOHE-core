...
decisions:
  - id: D1
    user_decision_ref: "chat_segment:2026-08-24:adr-concise-rewrite"
    status: proposed
---
...
`catalogue-lint` は有効な型・トレイト宣言の集合を判定根拠とし、`TypeRef` 内の全カタログ内参照を一意に解決する。未解決・曖昧な参照がある場合は対象と候補の完全修飾パスを示して fail-closed とする。

`catalogue-lint` が成功できるのは、`TypeRef` 全体の検査完了を確認できた場合だけである。未対応構文、解析不能、深さ・資源上限、またはカタログ内参照と外部参照の分類不能があれば、その箇所を示して fail-closed とする。分類不能な参照を外部参照と推測してはならない。
