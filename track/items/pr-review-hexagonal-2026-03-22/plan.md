...
- [ ] resolve_reviewer_provider(&Path) → resolve_reviewer_provider(&str) に変更。PrReviewError から Io/ProfilesNotFound を削除。テスト書き換え。
...
- [ ] CLI 側 (pr.rs) でファイル読み込みを行い &str を渡す。ファイル不存在時の fail-closed テスト追加。CI 全通し + usecase-purity warning ゼロ確認。
