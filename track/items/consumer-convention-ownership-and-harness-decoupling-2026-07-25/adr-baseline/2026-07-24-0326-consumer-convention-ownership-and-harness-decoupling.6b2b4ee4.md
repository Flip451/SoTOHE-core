...
この決定は、conventions をファイル単位の `include` / `exclude` に分類する
`2026-07-23-0117-export-surface-minimization.md` D1 の方向を、所有権に基づく
directory `exclude` + overlay へ置き換える。既存 ADR 自体は歴史記録として変更しない。
...
読み取り専用コマンド
`bin/sotp conventions resolve --capability <capability-id>` を導入する。
resolver は `knowledge/conventions/**/*.md` を走査し、指定された capability ID と
`required_for` の要素が完全一致する文書の repository-relative path を、重複なく安定順で
返す。機械利用できる出力形式を正本とし、人間向け表示を持つ場合も同じ解決結果から
レンダーする。

resolver は文書の作成、更新、削除、索引生成を行わない。既存の
`conventions add` / `update-index` / `verify-index` の存廃・仕様変更は本 ADR の対象外である。

構造上の異常は fail-closed とする。

- frontmatter が YAML として解析できない。
- `required_for` が文字列配列ではない。
- `required_for` に空文字または空白だけの capability ID がある。
- 解決対象が `knowledge/conventions/` の外へ逸脱する。
- 対象文書を読み取れない。

frontmatter がない文書、`required_for` がない文書、指定 capability に一致する文書が
ゼロ件である状態は正常とする。
...
`orchestrator-output` capability は必ず
`bin/sotp capability exec <capability> --host <host> --briefing-file <path>` を経由する。
dispatcher は provider を分岐する前に、実行対象の capability ID を使って D4 の resolver を
呼ぶ。解決結果の引き渡しは dispatcher outcome ごとに次のように行う。

- `CAPABILITY_EXEC_OUTCOME: executed`: 解決した規約 path と全文書を読む義務を、
  dispatcher が provider prompt／実行入力へ注入してから外部 provider を実行する。
- `CAPABILITY_EXEC_OUTCOME: delegate-in-host`: 解決した規約 path と全文書を読む義務を、
  dispatcher が返す delegation payload の `discipline` に含める。host orchestrator は
  `briefing_file` と `discipline` をそのまま provider-native subagent の依頼へ渡す。

Claude host と Claude provider の組合せで Claude Code の subagent を使う場合も、先に
`capability exec` が `delegate-in-host` を返し、その指示を host orchestrator が Agent
呼び出しへ運ぶ。host が resolver を再実装したり、規約本文を独自に要約したりしない。
dispatcher を経由しない Agent／skill の直接起動は supported entrypoint とせず、同等
preflight を別実装する経路も設けない。

これにより `track:adr2pr` 自体が規約一覧を管理しなくても、そこから
`capability exec` で起動される各 capability が自身の必読規約を canonical dispatch
境界で取得できる。

`reviewer`、`review-fix-lead`、`dry-fix-lead` 等、専用 CLI が prompt 構築と provider
dispatch を所有する `typed-pipeline` capability は本決定の対象外とする。特に review は
scope ごとに必読規約の集合が異なるため、単一 capability ID による解決をそのまま適用
しない。
...
これは export 境界の分類に束縛された構造検査である。
workflow／capability／entry document の本文から convention path を抽出して dead reference を検出する検査は置かない。
散文への参照抽出は機械可読な契約に束縛されず、そのための新たな契約や再発検出機構も導入しない。
移動漏れは同一変更集合内の live surface 更新 (D9) と review で担保する。

移動前 path の stub、redirect document、alias は作らない。`track/items/**` と
`track/archive/**` は過去時点の判断・参照を記録する歴史成果物なので遡及更新せず、
そこに残る旧 path は dead live reference として扱わない。既存 ADR や research note に
過去状態の説明として残る path も、現行手順として参照していない限り歴史記録として扱う。
...
- `capability exec` の `executed`／`delegate-in-host` 双方で、prompt／discipline 注入の
  適合テストが必要になる。
