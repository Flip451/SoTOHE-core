...
- [x] **T001**: Change `llvm-cov-local` task args in `Makefile.toml` from `["llvm-cov", "--html", "--all-features"]` to `["llvm-cov", "nextest", "--html", "--all-features", "--locked"]`, aligning the coverage harness with `cargo make test` (nextest). No other files are modified.
