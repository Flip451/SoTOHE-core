...
- [GO-02] Codex root orchestrator が SoTOHE の既存 `/track:*` workflow、phase ownership、review、DRY gate、hook policy を維持したまま運用できる project config、custom agents、skills、rules、hooks、運用文書を追加する。また、利用者の provider/agent 設定を CI で強制せず提供物と docs に留めることを責任分界点として確立する。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D1, knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D2, knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D4, knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D5, knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D6]
...
- [IN-03] Codex custom agent definitions を `.codex/agents/*.toml` に追加する。対象は `orchestrator`、`spec-designer`、`impl-planner`、`type-designer`、`adr-editor`、`review-fix-lead`、`dry-fix-lead` とし、agent TOML は role、model、所有 artifact、境界、status contract を薄く定義する。per-agent sandbox は Codex 現行 schema が agent role file で受け付けないため agent TOML には設定しない。sandbox は `.codex/config.toml` の session-level `sandbox_mode` で与える。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D2] [tasks: T002]
- [IN-04] specialist workflow は provider 非依存の単一 SSoT として `.harness/capabilities/<name>.md` に置く。`spec-designer`、`impl-planner`、`type-designer`、`adr-editor` の各 capability において、Codex skill (`.agents/skills/<name>/SKILL.md`) と Claude subagent (`.claude/agents/<name>.md`) はどちらもこの共有 SSoT を参照する薄い wrapper とする（DRY）。agent TOML だけを置いて specialist workflow が空になる状態を禁止する。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D2] [tasks: T002]
...
- [IN-06] Codex hooks coverage を `.codex/config.toml` から `.codex/hooks/sotp-hook.sh` へ接続し、hook adapter から既存 `bin/sotp hook dispatch <hook-id>` を呼ぶ。policy 本体は既存 `sotp hook dispatch` 側に残す。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D4] [tasks: T001]
...
- [IN-08] `verify-orchestra` を全廃し、`.claude` / `.codex` の利用者設定（permission allow/deny、config 値、ファイル存在、散文文言）を CI で強制しない。設定は提供物 + docs に留め、正しさは review に委ねる（責任分界点）。削除対象は `libs/infrastructure/src/verify/orchestra/` モジュール、CLI の `verify orchestra` subcommand、`cargo make verify-orchestra` task、CI からの呼び出しである。Codex surface の設定ファイル自体（`.codex/*`、`.agents/skills`、`.harness/capabilities`、sample profile）は提供物として残す。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D6] [tasks: T004, T006]
...
## Constraints
- [CN-01] `.codex/config.toml` は project-local config として安全に共有できるキーだけを使う。provider auth、base URL、machine-local notification/telemetry、profile selector など project-local config で無視されるか個人環境に属する設定は入れない。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D1] [tasks: T001]
- [CN-02] `.codex/rules/default.rules` は least-privilege にする。`cargo make ci`、`cargo make track-commit-message`、`cargo make track-pr*` など SoTOHE canonical wrapper と read-only Git inspection は allow 対象にできるが、direct `git add` / `git commit` / `git push` / `git reset`、destructive shell、broad `bash -lc` allow は許可しない。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D1, knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D4] [tasks: T001]
- [CN-03] Codex custom agent TOML と Claude subagent は薄い wrapper に限定し、詳細 workflow は provider 非依存の単一 SSoT である `.harness/capabilities/<name>.md` に置く。`spec-designer`、`impl-planner`、`type-designer`、`adr-editor` の各 capability において、共有 SSoT (`.harness/capabilities/<name>.md`) と両 wrapper (`.agents/skills/<name>/SKILL.md` および `.claude/agents/<name>.md`) の三点セットをすべて必須とする。workflow を Claude 側と Codex 側へ二重に複製しない（DRY）。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D2] [tasks: T002]
...
- [CN-05] Codex hook adapter は既存 `bin/sotp hook dispatch` へ委譲する。direct git operation 抑止、test file deletion 抑止、skill compliance などの policy logic を Codex hook script 側へ重複実装しない。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D4] [tasks: T001]
- [CN-06] 利用者の provider/agent 設定（`.claude` / `.codex` の config 値・ファイル存在・permission allow/deny エントリ・散文文言）を CI で hard-fail させる verifier を SoTOHE に追加しない（責任分界点 convention）。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D6] [tasks: T004, T006]
...
- [ ] [AC-02] `.codex/rules/default.rules` が SoTOHE canonical wrapper の command prefix を allow し、direct git mutation と broad/destructive shell command を allow していないことを review で確認できる。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D1] [tasks: T001]
- [ ] [AC-03] `.harness/capabilities/{spec-designer,impl-planner,type-designer,adr-editor}.md` が tracked repository files として存在する。各 capability において `.claude/agents/<name>.md` と `.agents/skills/<name>/SKILL.md` の両方が存在し、かつ両 wrapper が対応する共有 SSoT (`.harness/capabilities/<name>.md`) を参照していること（workflow が空洞化しておらず、二重複製もない）を review で確認できる。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D2] [tasks: T002]
...
- [ ] [AC-06] Codex hook adapter の各 configured hook が `.codex/hooks/sotp-hook.sh` から `bin/sotp hook dispatch` へ接続している。hook adapter の smoke test で `block-direct-git-ops`、`block-test-file-deletion`、`skill-compliance` の接続を確認できる。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D4] [tasks: T001]
- [ ] [AC-07] `verify-orchestra` モジュール (`libs/infrastructure/src/verify/orchestra/`)、CLI の `verify orchestra` subcommand、`cargo make verify-orchestra` task、および CI からの呼び出しが削除され、`cargo make ci` が pass する。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D6] [tasks: T004, T006]
...
- [ ] [AC-09] 最終的に `cargo make ci`、`bin/sotp review check-approved`、`bin/sotp dry check-approved` が pass し、review/DRY/PR workflow が既存 SoTOHE gate を通過する。 [adr: knowledge/adr/2026-06-13-0002-codex-orchestrator-settings-addition.md#D5] [tasks: T006, T007]
