...
decisions:
  - id: D1
    user_decision_ref: "chat_segment:session-012hW6t9KKjqZBWQeUVBLE6Z:2026-07-29"
  ...
  - id: D3
    ...
    status: proposed
  - id: D4
    user_decision_ref: "chat_segment:session-012hW6t9KKjqZBWQeUVBLE6Z:2026-07-28"
    status: proposed
  - id: D5
    user_decision_ref: "chat_segment:session-012hW6t9KKjqZBWQeUVBLE6Z:2026-07-29"
    status: proposed
...
独立 review に分割する機構は存在しない（review の hash / verdict は scope 全体を単位とする）。
したがって強制が意味を持つのは「diff がまだ存在しない、または止めれば累積を防げる」地点、
すなわち**実装開始前**に限られる。
...
- **(ii) 実測ガード**: 候補タスクが見積りを申告する scope ごとに、先行寄与
  ...
  **先行寄与が 0 の scope では候補は 1 件目の寄与者であり、見積りの大小によらず admit
  ...
  既超過は遷移を妨げない。commit で baseline は 0 へ戻り、残る member は後続の batch
  として続行する — **runtime に許されるのは宣言 batch の分割のみで、合併は許されない**
