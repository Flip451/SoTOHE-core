...
- [x] **T021**: `libs/domain/src/batch_plan/{mod,gate,admission}.rs`: check_batch_plan に scope 名の設定照合検査を追加、BatchPlanGateViolation と AdmissionEvaluationError に UnknownMainScopeName を追加、collect_task_set_violations の UnplannedTask 報告域を限定（IN-03、IN-07、IN-11、IN-21、AC-07、AC-08、AC-23、AC-24、AC-25）。`libs/domain/src/impl_plan.rs`: ImplPlanDocument に transition_for を追加。`libs/usecase/src/{batch_plan/output,task_admission,task_ops}.rs`: BatchPlanViolationOutput の鏡像 variant と写像、admission error の写像、judge_start_of_work の判定対象条件を transition_for 経由へ差し替え（IN-09、AC-26、AC-27）。`apps/cli-driver/src/batch_plan.rs`: 描画 arm を追加（AC-07）。 (`38c1afc96413621f37961bbe777f3cf8d6edafb2`)
- [x] **T022**: `libs/domain/src/batch_plan/mod.rs`: membership_rejection の current_batch 不在経路を AdmissionRejection::NoCurrentBatch として報告し、同経路の doc comment を更新（IN-09、AC-27）。`libs/usecase/src/task_admission.rs`: map_rejection と Display 写像に AdmissionRejectionOutput の対応 variant を追加（AC-27）。`libs/domain/src/batch_plan/tests.rs`: 全 batch settle 済みの plan に対する Reopen 候補の NoCurrentBatch 拒否と、現在 batch が残る場合との対比を試験する（IN-09、AC-27）。`libs/usecase/src/task_ops.rs`: interactor 段で、全 member settle 済みの宣言に対する reopen の拒否・鏡像 variant 経由の描画・状態非書き込みを試験する（IN-09、AC-27）。`apps/cli/tests/transition_integration.rs`: 新しい拒否経路に合わせて round-trip fixture を修復する（AC-27）。 (`a4c281760275c4f4d95b5b7e4ae570ebd8e43b97`)
- [x] **T023**: `libs/infrastructure/src/dry_check/commit_hash_store.rs`: ancestry 検査の repo 解決を SystemGitRepo::discover() から discover_from(self.trusted_root) へ差し替え、CWD が別 repo のときの一貫性試験を追加する（CN-05、IN-14）。 (`a4c281760275c4f4d95b5b7e4ae570ebd8e43b97`)
- [x] **T024**: `libs/domain/src/batch_plan/estimate.rs`: TaskEstimate::new に空 scope_estimates の拒否を追加し doc comment を更新する（IN-02、AC-04）。`libs/domain/src/batch_plan/tests.rs`: 同経路の試験を追加する（IN-02、AC-04）。`libs/infrastructure/src/batch_plan_codec.rs`: 空 scope_estimates を前提にしていた fixture を差し替え、decode 経路の試験を追加する（AC-04）。 (`5d7e5395de8c56ae09cef59d4e621b74aad99e6f`)
- [x] **T025**: `libs/infrastructure/src/scope_diff_measure.rs`: untracked_additions の guard 失敗分岐と count_file_lines の全分岐で message の絶対パス埋め込みを repo-relative FilePath 報告 + sanitized reason に差し替え、count_file_lines のシグネチャを調整する（IN-14、`knowledge/conventions/security.md` §Error Messages）。同ファイルの symlink 拒絶テストに message 内容の assertion を追加する（IN-14）。 (`3909a21401933d85f260e74922f63904e0bfe19b`)
- [x] **T026**: `libs/usecase/src/task_ops/` の ports module: CommitRecordVerifierPort と CommitRecordVerifyError を定義する（IN-22、AC-28）。`libs/usecase/src/task_ops.rs`: port 照合と TaskOperationError への mapping を行う usecase 内部 helper を追加する（呼び出し側への挿入と署名変更は含まない）（IN-22、AC-28）。同ファイルの試験: stub port で mapping の 3 failure lane を function 段で試験する（AC-28）。 (`30456dddcd179b7597502133b12bff78784dd63b`)
- [x] **T027**: `libs/domain/src/track.rs`: TaskTransition に recorded_commit_hash accessor を追加し、hash 記録を伴う遷移の判定を単一定義に集約する（IN-22）— usecase の検証レーンはこの accessor を経由して対象遷移を識別する。`libs/infrastructure/src/commit_record_verifier.rs`: T026 が定義した CommitRecordVerifierPort を実装する GitCommitRecordVerifier を追加し、実 git fixture で ancestry / not-found / unreadable の各分岐を試験する（IN-22、AC-29）。`libs/usecase/src/task_ops.rs`: TaskOperationInteractor::new を 6 collaborator 化し、T026 の helper を hash 書き込み遷移 2 経路（Complete-with-hash / BackfillHash）の apply 前に挿入する（IN-22、AC-28、AC-29）。同ファイルの試験: transition 段で 3 failure mode × 同 2 経路 × 永続化された impl-plan.json の byte 不変性、および success 経路（受理された hash の永続化）と bypass 経路（hash なし完了）を試験する（AC-28、AC-29）。`apps/cli-composition/src/track/mod.rs`: task-ops composition root の build 関数に 6 番目の collaborator として同 adapter を配線する（IN-22）。`apps/cli/tests/transition_integration.rs`: 配線後に影響する fixture を確認し必要なら更新する（AC-28）。 (`30456dddcd179b7597502133b12bff78784dd63b`)
