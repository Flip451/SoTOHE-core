...
- [ ] **T001**: Update `bin/sotp track type-graph` Rust CLI: change cluster-mode output directory from `<layer>-graph/` to `<layer>-graph-d<depth>/`, preserve flat-mode output as `<layer>-graph.md`, add unit tests for new path logic, and add `track/items/**/*-graph-d*/` to `.gitignore`
