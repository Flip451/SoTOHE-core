...
- [x] Domain 層: TrackMetadata::add_task() + next_task_id() メソッド追加
...
- [x] Usecase 層: AddTaskUseCase（atomic RMW でタスク追加）
- [x] Usecase 層: SetOverrideUseCase（atomic RMW でオーバーライド設定/解除）
...
- [x] CLI 層: sotp track add-task サブコマンド（--section/--after オプション + ブランチガード）
- [x] CLI 層: sotp track set-override / clear-override サブコマンド（ブランチガード付き）
- [x] CLI 層: sotp track next-task サブコマンド（plan section 順 + in_progress 優先 + JSON スキーマ準拠）
- [x] CLI 層: sotp track task-counts サブコマンド（total/todo/in_progress/done/skipped の JSON スキーマ準拠）

## Makefile 統合 + bin/sotp 更新

Makefile.toml に wrapper タスクを追加し、bin/sotp をリビルド。

- [x] Makefile 統合: wrapper タスク追加 + bin/sotp リビルド
...
- [x] Python 削減: track_state_machine.py の Python フォールバック削除 + sotp 必須化
- [x] ドキュメント更新: deprecated マーキング + /track:plan コマンド参照更新
