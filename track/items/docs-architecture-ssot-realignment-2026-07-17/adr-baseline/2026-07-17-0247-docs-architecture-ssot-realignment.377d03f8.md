...
- **Usecase Layer Purity Rules**（std I/O モジュール・暗黙的外部依存・出力マクロの禁止表と rationale、および usecase の Good/Bad コード例）は `knowledge/conventions/coding-principles.md` へ節として移設する。機械強制は従来どおり `sotp verify usecase-purity` が担い、移設節には強制強度（現状 CI blocking であること、緩和は採用者判断であること）を consumer 向けの表現で明記する（監査で指摘された、強制強度が内部チケット参照でしか読めない問題の同時解消）。
...
- `sotp verify usecase-purity` の CI blocking を緩和するとき（coding-principles.md へ移設した purity 節の強制強度記述を同時に更新する）。
