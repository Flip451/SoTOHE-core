...
- **role × layer の配置（ポート配置を含む）**: `knowledge/conventions/type-designer-kind-selection.md` の R1 マトリクス。draft 段階で違反を却下する準機械ルールであり、SecondaryPort（domain / usecase）・Repository（domain / usecase）・ApplicationService（usecase のみ）・SecondaryAdapter（infrastructure のみ）・CompositionRoot（cli_composition のみ）・PrimaryAdapter（cli_driver のみ）を既に規定している。
