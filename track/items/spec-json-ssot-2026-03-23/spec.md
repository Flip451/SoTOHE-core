...
- sync_rendered_views への spec.md 統合 [source: convention — hexagonal-architecture.md]
...
- [ ] sotp verify spec-frontmatter が spec.json 存在時に spec.json スキーマ検証に委譲する [source: convention — hexagonal-architecture.md]
- [ ] sotp verify latest-track が spec.json または spec.md の存在をチェックする (spec.json 優先、fallback で spec.md) [source: convention — hexagonal-architecture.md]
- [ ] spec.json なし旧 track で spec-signals, spec-states, spec-attribution, spec-frontmatter, latest-track の各 verifier が spec.md fallback 動作する [source: convention — hexagonal-architecture.md]
- [ ] sync_rendered_views が spec.md を plan.md と並行して生成する [source: convention — hexagonal-architecture.md]
