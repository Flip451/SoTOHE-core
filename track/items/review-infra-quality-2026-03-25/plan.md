...
- [ ] GitDiffScopeProvider contract tests — tempdir git fixture covering merge-base diff, staged changes, unstaged worktree changes, untracked files, renames, deletes, and error propagation
...
- [ ] codex-reviewer agent tools: restriction verification — confirm Bash(cargo make track-local-review:*) actually limits tools, fix if not
...
- [ ] review.md --auto-record full migration — update /track:review to always pass --auto-record flags, validate via this track's own review cycle
...
- [ ] /track:plan skill に spec.json 生成後の信号機評価ステップを追加 — sotp track signals を自動実行して signals フィールドを埋める
