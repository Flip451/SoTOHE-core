...
- [x] 中優先度: track-branch-create/switch/activate の command 直接呼び出し化
- [x] 中優先度: track-pr-* 系の command 直接呼び出し化
- [x] 中優先度: track-plan-branch を sotp track branch plan サブコマンドとして新設
- [x] 中優先度: 残りの薄い shell ラッパー (track-local-review, track-resolve 等) の command 直接呼び出し化

## Phase 4: -exec daemon ラッパー統一

- [-] -exec daemon ラッパー: sotp make exec サブコマンドで WORKER_ID 処理を Rust 統一 [skipped: run --rm で十分、daemon 常駐の投資対効果が低い]
- [-] -exec タスク群の Makefile.toml command 形式への変換 [skipped: T011 と同理由]

## Phase 5: ドキュメント・CI 更新

- [x] Makefile.toml の最終整理と script_runner=@shell 残存の監査
- [x] ドキュメント更新: track/workflow.md, .claude/rules/07-dev-environment.md, DESIGN.md
