...
- [x] **T005**: 型カタログドキュメントに spec_refs: Vec<SpecRef> + informal_grounds: Vec<InformalGroundRef> field を追加。libs/domain/src/track.rs の TrackMetadata から tasks / plan を削除し identity-only 化 (schema migration 戦略は実装者判断、既存 v3 との並立対応を含む)。両方の codec 更新 + tests (`6ddaabcd78242838a4633d9a8ae9e251aff520b4`)
...
- [x] **T006**: libs/infrastructure/src/verify/ の verify-track-metadata-local / verify-latest-track-local を file 存在ベースに改訂 (metadata = identity のみ検証、latest-track = impl-plan.json 存在条件で task 項目チェック)。sotp track type-signals / baseline-capture / 関連 verify から「空カタログ拒否」ロジックを撤廃し、エントリ 0 件の空カタログを有効状態として受け入れ。tests 更新 (`6ddaabcd78242838a4633d9a8ae9e251aff520b4`)
- [x] **T007**: libs/usecase/src/task_completion.rs の check_tasks_resolved_from_git_ref を metadata.json から impl-plan.json 読み替え。TrackBlobReader port の read_track_metadata を read_impl_plan に置換 or 追加、apps/cli/src/commands/pr.rs 呼び出し側も改修。K1-K7 MockReader tests を新 port で書き換え (`6ddaabcd78242838a4633d9a8ae9e251aff520b4`)
...
- [x] **T008**: libs/infrastructure/src/track/render.rs の plan.md / spec.md renderer を集約形式に変更 (plan.md = metadata.json + impl-plan.json、spec.md = spec.json + task-coverage.json)。tests 更新 (`6ddaabcd78242838a4633d9a8ae9e251aff520b4`)
- [x] **T009**: Makefile.toml から cargo make spec-approve タスクを削除し、関連する apps/cli/src/commands/ の spec approve コマンドを廃止。spec-signals ツールおよび spec schema 参照コードから status / approved_at / content_hash 関連の依存を除去。tests 整理 (`93a3cc429b42e67055246dc521f465dd8c51d22a`)
...
- [x] **T010**: apps/cli/src/commands/verify/plan_artifact_refs.rs を新設し sotp verify plan-artifact-refs subcommand として公開。各 ref field (adr_refs / convention_refs / spec_refs / related_conventions / informal_grounds) の走査、schema validation、file 存在チェック (file ベース ref のみ)、SpecRef.anchor 解決 (spec 要素 id lookup)、SpecRef.hash 照合 (canonical JSON subtree SHA-256)、AdrAnchor / ConventionAnchor の newtype loose validation、InformalGroundRef の newtype validation (kind variant / summary 非空、file resolution なし) を実装。unit + integration tests (`99c5ecdfea923ab3b9f030fb507e980bdd1ae457`)
- [x] **T011**: T010 の CLI に task-coverage.json の coverage 強制 + referential integrity 検査 (現行 spec_coverage::verify 踏襲) を追加し、canonical block 疑惑検出 (ADR 例示マーカー除外 / spec フィールド内 10 行超コードブロック警告) を実装。cargo make ci / Makefile.toml に組み込み、CI で fail-closed 動作を検証 (`7cb345e4cd1f16f965a5912774b19ec0b6fb9462`)
- [x] **T012**: 既存 libs/infrastructure/src/verify/spec_coverage.rs の spec_coverage::verify を新 CLI (plan-artifact-refs) に統合し、旧呼び出し経路 (Makefile.toml / scripts/ / 関連 verify-*) を新 CLI 経由に切り替え、不要コードを削除。tests 整理 (`70fb44ee5f3a72683c77e032ddb112566b9a5412`)

### S7 — S7 — PR-review cycle fix: status 単一 SSoT 化 + activation 不変条件

> metadata.json から status field を除去し identity-only 化 (schema_version v4 → v5)
> track status は domain::derive_track_status(impl_plan, status_override) で都度導出する single SSoT に統一
> activation 不変条件を domain::TrackMetadata::is_activated() ≡ branch.is_some() で codify
> impl-plan.json 存在条件を domain::check_impl_plan_presence(track, impl_plan) で検証 (is_activated → impl-plan 存在、一方向: ブランチ有りのとき impl-plan が必ず存在する)
> 全 CLI 呼び出し側 (make.rs / state_ops.rs / activate.rs / resolve.rs / tddd/graph.rs / tddd/signals.rs x2 / tddd/contract_map.rs) を新 API に置き換え ad-hoc 判定を削除
> ADR 2026-04-19-1242 §D1.4 Follow-up に経緯を記録 (PR #107 の atomic-write P0 findings 由来)

- [~] **T013**: PR-review cycle fix: metadata.json から status 除去し、track status は impl-plan.json + status_override から derive する domain::derive_track_status に一本化。activation 不変条件を domain::TrackMetadata::is_activated() + domain::check_impl_plan_presence() で codify し、全 CLI 呼び出し側の ad-hoc 判定 (branch.is_some() || derived_status != Planned) をこの single source of truth に置き換え。schema_version を v4 → v5 に bump (identity-only)。ADR 2026-04-19-1242 §D1.4 Follow-up で経緯を記録。Codex Cloud PR #107 の atomic-write P0 findings に構造的対応
