...
- `--scope <name>` の値は query の前に `ScopeName` として検証し、invalid scope 名を拒否する。
...
理由は、列挙範囲を selection type の variant に対応させることで、全体表示と単一 scope query の
意味を混在させず、競合入力と未知入力を query 実行前に fail-closed で排除できるためである。
