...
---
status: approved
approved_at: "2026-04-15T12:01:00Z"
version: "1.0"
...
- sync_rendered_views を multi-layer 対応に拡張する — architecture-rules.json の tddd.enabled=true 全 layer を iterate し、各 <layer>-types.md を一括生成する。resolve_tddd_layer_bindings を render.rs 内の private helper として新設 (apps/cli::resolve_layers は layer dependency direction により reach 不可)。既存 is_done_or_archived / rendered_matches / TypeCatalogueCodecError::Json warn-and-continue の 3 pattern は保持する。 [source: libs/infrastructure/src/track/render.rs §sync_rendered_views line 568-606 (現状 domain-only loop), architecture-rules.json §layers tddd.enabled, apps/cli/src/commands/track/tddd/signals.rs §resolve_layers (参考実装), feedback — ユーザー指摘: sync_rendered_views が domain_types.md だけをレンダーするのはバグ (2026-04-15)] [tasks: T004]
...
- resolve_tddd_layer_bindings を apps/cli::resolve_layers と共通化して shared module (例: infrastructure::verify::architecture_rules) に抽出する (本 track では render.rs 内の private helper として独立実装し、重複を許容する。共通化は 3 箇所目の resolver が必要になった時点で別 track で実施) [source: apps/cli/src/commands/track/tddd/signals.rs §resolve_layers, convention — .claude/rules/10-guardrails.md §Small task commits 原則]
...
- layer 依存方向を維持: signals.rs の guard は apps/cli 層で infrastructure の track metadata loader を呼ぶ。sync_rendered_views の multi-layer loop は libs/infrastructure 層で architecture-rules.json を直接 parse する (apps/cli からの import は依存方向違反のため不可) [source: architecture-rules.json, knowledge/conventions/hexagonal-architecture.md]
