...
- [x] **T004**: Complete `.harness/config/contract-map-style.toml` and update `RoleKind` in `libs/domain/src/tddd/catalogue_linter_role.rs` plus the contract-map renderer result flow in `libs/domain/src/tddd/contract_map_renderer.rs`, `libs/usecase/src/contract_map_workflow.rs`, and `libs/infrastructure/src/tddd/contract_map_renderer_adapter/`. Add renderer and workflow regression coverage for AC-04. IN-04, CN-04, AC-04. (`87178dce272cdebef44725695f891b143c012515`)
