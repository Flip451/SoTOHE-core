...
境界は検証済み Command 型を受け取り、string からのパースは usecase 境界で一度だけ行う（port 粒度 ADR の D2 の様式を、新規コード全体の一般規則へ昇格する）。domain enum の鏡像を cli 側に定義する様式は廃止し、境界語彙は usecase 所有の boundary 型に一本化する。cli が domain を知らないという既存原則は維持する。
