<!-- Generated from domain-types.json — DO NOT EDIT DIRECTLY -->

## Enums

| Name | Kind | Action | Details | Signal | Cat-Spec |
|------|------|--------|---------|--------|----------|
| CatalogEntryKind | enum | add | Struct, Enum, TypeAlias, Trait, Function | 🔵 | 🔵 |
| CatalogImportAction | enum | add | Reference, Modify, Delete | 🔵 | 🔵 |
| DeletionRecord | enum | add | Type, Trait, Function | 🔵 | 🔵 |

## Value Objects

| Name | Kind | Action | Details | Signal | Cat-Spec |
|------|------|--------|---------|--------|----------|
| CatalogEntryName | value_object | add | — | 🔵 | 🔵 |
| DocString | value_object | add | — | 🔵 | 🔵 |
| DraftHole | value_object | add | — | 🔵 | 🔵 |
| DraftHolePath | value_object | add | — | 🔵 | 🔵 |
| FunctionEntry | value_object | modify | — | 🔵 | 🔵 |
| FunctionInfo | value_object | modify | — | 🔵 | 🔵 |
| ImplInfo | value_object | modify | — | 🔵 | 🔵 |
| InherentImplDeclV2 | value_object | reference | — | 🔵 | 🔵 |
| TodoInstruction | value_object | add | — | 🔵 | 🔵 |
| TraitEntry | value_object | modify | — | 🔵 | 🔵 |
| TraitImplDeclV2 | value_object | reference | — | 🔵 | 🔵 |
| TypeEntry | value_object | modify | — | 🔵 | 🔵 |
| TypeInfo | value_object | modify | — | 🔵 | 🔵 |

## Domain Services

| Name | Kind | Action | Details | Signal | Cat-Spec |
|------|------|--------|---------|--------|----------|
| CatalogueDocument | domain_service | modify | — | 🔵 | 🔵 |

