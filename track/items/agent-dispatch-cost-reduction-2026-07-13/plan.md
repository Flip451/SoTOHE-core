...
- [x] **T002**: Modify `libs/infrastructure/src/agent_profiles.rs` (`AgentProfiles`, `AgentProfilesError` error mapping, `CapabilityConfigDto`, `ExecutionModeDto`, `ResolvedExecution`, and routing DTOs), `libs/infrastructure/src/capability_exec/agent_profiles.rs` (`AgentProfilesCapabilityAdapter::resolve`), `libs/infrastructure/src/ref_verify/process_runner.rs` (the Claude, Codex, and Gemini ref-verifier argument builders), `libs/infrastructure/src/review_v2/review_fix_runner/{env,mod}.rs` (`CodexReviewFixRunner`), `libs/usecase/src/review_v2/run_review_fix.rs` (`RunReviewFixError`, `SubagentDispatchInstruction`, and `SubagentName`) for the typed dispatch instruction, the `ReviewService`/`RunReviewFixCommand`/`ReviewRunFixInput` review-fix usecase lane, the `ReviewFixDriver`/`ReviewInput` CLI-driver execution-and-rendering lane, and the review-fix effort wiring through the `ReviewFixRunner` carrier in `apps/cli-composition/src/review_v2/{mod,run_fix,shared}.rs`. Update `.harness/config/agent-profiles.json` and `.harness/config/samples/agent-profiles.default.json`, their fixtures, and `track/items/agent-dispatch-cost-reduction-2026-07-13/test-bindings.json`. IN-01, IN-02, CN-01, AC-01, AC-02. (`fc39a2ec`)
