...
- [x] domain: SpecRequirement に task_refs: Vec<TaskId> 追加 + テスト
- [x] domain: CoverageResult 型 + SpecDocument::evaluate_coverage() + テスト
...
- [x] infra: spec/codec.rs の DTO に task_refs 追加 + round-trip テスト
- [x] infra: spec/render.rs に task_refs 表示追加 + テスト
- [x] infra: verify/spec_coverage.rs CI ゲート実装 + テスト
...
- [x] CLI: sotp verify spec-coverage コマンド追加
- [x] Makefile: verify-spec-coverage を CI gate に追加 + /track:plan skill の spec.json 生成に task_refs を含める
...
- [x] 統合テスト + ドキュメント更新 (DESIGN.md, TRACK_TRACEABILITY.md)
