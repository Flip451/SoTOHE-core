...
decisions:
  ...
  - id: D5
    ...
    status: proposed
  - id: D6
    user_decision_ref: "chat_segment:session-01GzQfyJu34bkRqJ7tWxRJtC:2026-07-19 provision の --project-root 省略時は invocation directory から git root を発見して配備し、明示指定を優先し、リポジトリ外は typed failure とする裁定（PR #202 round 5 外部 reviewer P1 起点）"
    candidate_selection: "from:[default-to-cwd,default-to-discovered-git-root,reject-non-root-invocation] chose:default-to-discovered-git-root"
    status: proposed
---
...
### D6: provision の既定 project root は invocation directory から発見した Git root とする

`sotp codex-runtime provision` は、明示的な `--project-root` があればその値を優先する。
指定がなければ invocation directory から Git root を探索し、発見した root を配備先の
project root とする。Git repository 外で探索に失敗した場合は、成功として扱わず、
`CommandOutcome` を通じて typed failure を報告する。
...
### H. provision の既定 project root を invocation directory そのものとする

repository の subdirectory から実行するとリンクがその subdirectory 配下へ配備され、Git root
を基準に解決する reviewer / fixer から見えない。配備成功の報告と spawn 時の解決結果が
食い違うため却下。

### I. Git root 以外からの provision 実行を拒否する

呼び出し側に repository root への移動を要求すれば配備先は一意になるが、任意の subdirectory
から実行できる CLI 契約を失い、不要な利用者負荷を生む。invocation directory から Git root
を発見すれば同じ一意性を自動で得られるため却下。
