...
- [ ] **T001**: libs/usecase/src/git_workflow.rs — add DiagnosticText; update error variant payloads and diagnostic construction sites in validate_stage_path_entries / verify_explicit_track_branch / verify_auto_detected_branch. libs/infrastructure/src/git_cli/workflow_adapter.rs — update DiagnosticText construction sites. Add DiagnosticText display tests (IN-06 / CN-02).
- [ ] **T002**: libs/usecase/src/git_workflow.rs — add the git primitive-operation method surface (current_branch / sync_current_branch / switch_branch / create_branch / branch_exists / move_path / fetch_branch / show_file_at_ref / resolve_commit / stage_all / stage_from_file / commit_from_message_file / note_from_file / unstage / read_explicit_track_branch -> ExplicitTrackBranch / collect_track_branch_claims -> Vec<TrackBranchClaim>); add TrackArchiveFsPort with path_is_dir / path_exists / create_dir_all / rename_path / list_dir_file_names / remove_dir; add dyn-safety compile test (IN-08).
...
- [ ] **T004**: libs/infrastructure/src/git_cli/mod.rs — add SystemGitRepo::sync_current_branch and sync tests. libs/infrastructure/src/git_cli/workflow_adapter.rs — add the git primitive-method adapter implementations; add FsWorkspaceAdapter implementing TrackArchiveFsPort; map sync / fs failures through usecase error surfaces. Add adapter and fs-port tests (IN-02 / CN-02 / AC-08).
...
- [ ] **T010**: Git-debt review repayment round 2. libs/usecase/src/git_workflow.rs — add GitWorkflowError::SwitchFailed { branch: DiagnosticText, exit_code: i32 } and update variant/display tests. libs/infrastructure/src/git_cli/workflow_adapter.rs — update FsGitWorkflowAdapter switch_branch and sync_current_branch error mapping. libs/infrastructure/src/git_cli/mod.rs — add SyncError::Spawn { detail: DiagnosticText }, extract classify_sync_failure, and add classification regression tests for UpstreamNotSet / NonFastForward / WorktreeUnresolved / Spawn. apps/cli-composition/src/review_v2/commit_hash.rs — demote persist_commit_hash_for_track to pub(crate). Verify track switch-base typed failure behavior through existing branch_strategy tests. Update touched-surface tests (IN-06 / IN-07 / CN-02 / CN-05 / AC-02 / AC-04 / AC-08).
- [ ] **T011**: Git-debt review repayment round 2. libs/usecase/src/git_workflow.rs — add GitPrimitivePort::resolve_repo_root(&self, project_root: Option<&std::path::Path>) -> Result<std::path::PathBuf, GitWorkflowError>. libs/infrastructure/src/git_cli/workflow_adapter.rs — implement resolve_repo_root on FsGitWorkflowAdapter. libs/usecase/src/telemetry.rs — add ArchivedTelemetryFactoryPort; add TelemetryReportInteractor (report) and TelemetryEmitInteractor (emit_archived) sub-interactors; update TelemetryAggregateInteractor to compose the two sub-interactors; add mock-port unit tests. libs/infrastructure/src/telemetry/archived_track.rs — add FsArchivedTelemetryFactoryAdapter. apps/cli-composition/src/telemetry.rs and apps/cli-composition/src/telemetry_wiring.rs — remove archived-telemetry emit orchestration from composition and wire TelemetryAggregateInteractor / FsArchivedTelemetryFactoryAdapter only. Verify telemetry_driver interactor wiring through touched-surface tests. Update touched-surface tests (IN-08 / IN-10 / CN-03 / AC-09).
