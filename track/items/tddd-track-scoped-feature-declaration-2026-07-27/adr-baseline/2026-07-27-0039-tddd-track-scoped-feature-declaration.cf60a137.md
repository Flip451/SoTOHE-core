...
decisions:
  ...
  - id: D2
    ...
    status: proposed
  - id: D3
    user_decision_ref: "chat_segment:current-task:2026-07-27:tddd-track-scoped-feature-declaration-phase0-approval"
    status: proposed
  - id: D4
    user_decision_ref: "chat_segment:current-task:2026-07-27:tddd-track-scoped-feature-declaration-hearing"
    status: proposed
...
baseline 取得と実測取得は、同一の宣言内容を観測しなければならない。同じパスを読むことだけではこの要件を満たさない。両取得を同一内容へ拘束する具体的な方法は本 ADR では固定せず、下流の型設計に委ねる。
...
- baseline 取得と実測取得を同一の宣言内容へ拘束するため、両者の feature 集合が乖離しなくなる。
