...
- [x] **T004**: Declare unchanged `SchemaExporter` and `SchemaExporterPort` as `reference` referents required to express the `RustdocSchemaExporter` change; this task does not modify either interface. Modify `RustdocCratePort` in `libs/domain/src/tddd/catalogue_v2/catalogue_impl_signals_ports.rs` and `CatalogueImplSignalsInteractor` / `CatalogueImplSignalsError` in `libs/usecase/src/catalogue_impl_signals/{interactor.rs,service.rs}`; update `RustdocCrateAdapter` in `libs/infrastructure/src/tddd/rustdoc_crate_adapter.rs` and `RustdocSchemaExporter` in `libs/infrastructure/src/schema_export.rs`; inject `TdddActualFeatureDeclarationPort` through `FsTdddFeatureDeclarationAdapter` at `apps/cli-composition/src/track/tddd.rs`; and add actual-capture tests (IN-03, IN-05, CN-01, CN-02, CN-03, CN-05, AC-03, AC-04, AC-05, AC-06, AC-08).
- [x] **T006**: Modify `TypeSignalsInteractor`, `TypeSignalsError`, and `TypeSignalsExecutorPort` in `libs/usecase/src/type_signals/{interactor.rs,service.rs,ports.rs}`, `TypeSignalsExecutorAdapter` in `libs/infrastructure/src/tddd/type_signals_executor_adapter.rs`, and `execute_type_signals_for_layer` in `libs/infrastructure/src/tddd/type_signals_evaluator.rs`; inject `TdddActualFeatureDeclarationPort` through `FsTdddFeatureDeclarationAdapter` at `libs/infrastructure/src/signal.rs::SystemSignalCommandAdapter::calc_impl` and `apps/cli-composition/src/track/tddd.rs`; and add canonical signal-calculation tests (IN-03, CN-01, AC-03, AC-04).
