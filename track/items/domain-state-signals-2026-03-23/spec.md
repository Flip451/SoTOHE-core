...
| State | Description |
|-------|-------------|
| DomainStateEntry | spec.json の各状態エントリ。name + description + transitions_to を保持 |
| DomainStateSignal | per-state の信号判定結果: state_name + signal (Blue/Yellow/Red) + found_type (bool) + found_transitions (Vec) + missing_transitions (Vec) |
| DomainStateSignalCounts | domain_state_signals の集計: blue + yellow + red |
| CodeScanResult | syn スキャン結果: 検出された型名 Set + 遷移関数マップ (from_state → to_states) |
