...
- [ ] GITIGNORED_RENDERED_VIEWS 定数 + is_gitignored_rendered_view() ヘルパーを追加
- [ ] activation_commit_paths() を抽出し、persist_activation_commit() でフィルタ適用
- [ ] activation_artifact_paths() から registry.md 削除、spec.md 追加
- [ ] 既存テスト更新 + 回帰テスト追加（gitignored path 除外の確認）
- [ ] sync_rendered_views() にドキュメントコメント追加（VCS フィルタは呼び出し側の責任）
...
- [ ] sotp plan codex-local サブコマンド実装（CLI に Plan サブコマンド追加 + --briefing-file / --prompt、codex subprocess spawn、stdout 出力）
- [ ] Makefile.toml に track-local-plan タスク追加 + permissions.allow 登録
- [ ] .claude/rules/02-codex-delegation.md と 10-guardrails.md のドキュメント更新（briefing file パターン正式化）
- [ ] scripts/test_make_wrappers.py に track-local-plan ラッパーの回帰テストケースを追加
