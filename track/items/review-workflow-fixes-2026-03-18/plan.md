...
Extend poll_review and poll_review_for_cycle to check issues/{pr}/reactions API for a bot thumbs-up (+1) reaction as the zero-findings completion signal. Confirmed behavior from PR #2 and #36: findings-none -> bot posts thumbs-up reaction on the issue + 'Didn't find any major issues' text comment; findings-present -> bot posts a COMMENTED review with inline findings. Add list_reactions method to GhClient trait.

- [ ] Add thumbs-up reaction detection to poll_review: check issues/{pr}/reactions API for bot +1 as zero-findings completion signal + tests
