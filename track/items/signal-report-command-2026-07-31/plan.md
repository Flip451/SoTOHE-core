...
- [x] **T006**: After T001, T002, and T004, preserve and confirm the existing `SignalDriver` reference edge while adding and independently verifying `SignalCompositionRoot::signal_report_driver` wiring in `apps/cli-composition/`, before CLI report dispatch consumes it. CN-01, AC-01, AC-04. (`cc5ef158545673e58f2ead2999f6e41a637d1a74`)
- [x] **T005**: After T003, T004, and T006, add the parsing and command shape that makes `sotp signal report` parseable in `apps/cli/src/commands/signal/mod.rs`: `SignalReportChainArg`, `SignalReportOnlyArg`, `SignalReportArgs`, and `SignalCommand::Report`; add same-module parsing tests. IN-01, IN-03, AC-03. (`965b32d453149168757fbca0b858d4448592293c`)
- [x] **T008**: After T005, T004, and T006, add the signal-report branch of `execute` in `apps/cli/src/commands/signal/mod.rs`, constructing and using the composition-root report driver while propagating output and exit status; add focused dispatch tests. IN-01, AC-01. (`965b32d453149168757fbca0b858d4448592293c`)
- [x] **T007**: After T008, add `sotp track resolve` regression tests in `apps/cli/src/commands/track/resolve.rs`. OUT-01, AC-05. (`965b32d453149168757fbca0b858d4448592293c`)
