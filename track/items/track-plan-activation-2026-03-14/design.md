...
また、`tmp/TODO.md` の `STRAT-04` (`track/registry.md` の非 Git 化 / 完全生成ビュー化) は
このトラックの外に置く。ここでは registry を git-tracked な generated view として扱い続け、
plan-only / activate 導線に必要な最小限の routing / copy / rendering 変更だけを入れる。
repo-wide な generated view 方針の再設計は follow-up として切り分ける。
...
このトラックは収束を優先する。
canonical な public contract は Rust 実装と rendered metadata behavior に置き、
Python helper 群は移行完了までの compatibility layer とみなす。

したがって、このトラックの review / fix loop では次を徹底する。

- Python に対する新規 feature 投資や parity 目的の深追いはしない
- Python 側を触るのは、既存必須 verification が壊れる、public contract と矛盾して crash する、
  もしくは canonical Rust path の利用者導線を誤誘導する場合だけに限る
- `takt` / `/track:full-cycle` は transitional compatibility surface として guard のみ維持し、
  UX や追加 capability の整備対象にしない
