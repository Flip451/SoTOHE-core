...
計画を実装とは別のブランチでレビューしてから main に合流させたい場合の手順は次のとおり。

1. `/track:plan-only <feature>` — `plan/<id>` ブランチを main から作成し、planning artifacts を作成
2. `plan/<id>` ブランチを push して PR を作成、計画をレビュー
3. PR マージ → planning artifacts が main に合流（`metadata.json` の `branch` は `null` のまま）
4. `/track:activate <track-id>` — main 上の planning-only track から `track/<id>` 実装ブランチを作成
5. `/track:implement`
6. `/track:review` または `/track:pr-review`
7. `/track:ci`
8. `/track:commit <message>`

`plan/<id>` ブランチは計画レビュー専用の一時ブランチであり、
`metadata.json.branch` には記録しない（実装ブランチではないため）。
PR マージ後に `plan/<id>` ブランチは不要になる。

activation 前の planning-artifact flow（review / commit）は `plan/<id>` ブランチ上で行う。
planning-only とみなす diff は固定 allowlist に限る。
`track/items/<id>/` 配下、`track/registry.md`、`track/tech-stack.md`、
...
`/track:plan-only` は計画レビュー用の一時 branch `plan/<id>` を作成し planning artifact をその上に置く path を持ち、
