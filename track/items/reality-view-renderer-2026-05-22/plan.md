...
- [x] **T018**: render サブモジュール module 分割 (機能不変 refactor) — render/mod.rs (production 1391 行) / render/impl_processor.rs (production 975 行) / render/entry_subgraph.rs (production 708 行) を責務単位で分割し、各 production ファイルの行数を 700 行以下にする。分割境界の例: overview render / cluster render / edge collector / cluster key / inherent method / trait index / impl edge / struct subgraph / enum subgraph 等 (実際の境界は implementer が判断)。BaselineGraphRendererAdapter の pub(super) API および catalogue 宣言型 (BaselineGraphRendererAdapter) は不変。テストは移動先モジュールに追従する。cargo make ci (fmt-check + clippy + nextest + deny + check-layers + verify-*) が全 pass することを確認する (04-coding-principles.md 700 行上限、PR #136 finding 1-3 対応) (`5cd7b5255c3c2e82e6ac1138e48da952297f3383`)
