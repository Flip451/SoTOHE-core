...
T009 corrects the evaluation construction boundary so one hasher determines all fulfillment-cache key hashes. GO-01.
...
- [x] **T003**: Update `ObligationsDocument`, `ObligationCheckError`, and `CheckTestObligationsInteractor`; use `TestBindingConsistencyError`; add check finding/exit, duplicate-current-key, historical-row, and binding-consistency regressions. IN-01, IN-02, IN-03; OUT-03; CN-01, CN-02, CN-03; AC-01, AC-02, AC-03, AC-06. (`062ed4ea`)
...
- [x] **T004**: Update `EdgeResolutionOutcome`, `EdgeVerdictRecord`, and `TestObligationResultsInteractor`; migrate `EvaluateTestObligationsInteractor` constructors; add result fixtures. IN-04; CN-04; AC-04. (`062ed4ea`)
...
- [x] **T005**: Update `ObligationEvaluateError` and `EvaluateTestObligationsInteractor`; add evaluation fixtures. IN-01, IN-04, IN-05; OUT-04; CN-04; AC-04, AC-05, AC-07. (`062ed4ea`)
...
- [x] **T006**: Update `ObligationFulfillmentCacheEntryDto` and in-memory `JsonObligationFulfillmentCacheCodec` mapping; add mapping fixtures. IN-05; OUT-04; CN-04; AC-05. (`062ed4ea`)
...
- [x] **T007**: Update `JsonObligationFulfillmentCacheCodec` cache-load and legacy-decode paths; add decode regressions. IN-05; OUT-04; CN-04; AC-05. (`062ed4ea`)
...
- [x] **T008**: Update the `JsonObligationFulfillmentCacheCodec` cache-file write path and `fulfillment_cache_io`; add persistence fixtures. IN-05; OUT-04; CN-04; AC-05. (`062ed4ea`)

### S9 — Batch T009 — unified evaluation hashing

> Make the evaluation interactor internally construct its resolved-bound-tests resolver from the supplied source scanner and same hasher; migrate source and composition call sites, and add a regression that makes a distinct bound-test hashing strategy unconfigurable. IN-01; AC-06.
> Expected review scope: usecase and cli composition only, below the 500-line ceiling.

- [x] **T009**: Bind `EvaluateTestObligationsInteractor` resolved-bound-test hashing to its supplied `ContentHasherPort`: remove the separately supplied resolver from construction, construct the resolver internally from the supplied source scanner and same hasher, migrate source and composition call sites, and add regressions proving callers cannot configure a distinct hashing strategy for bound tests. IN-01; AC-06.
