...
- [x] Add CI test verifying .git is read-only inside tools container (docker compose exec git status should fail to write)
- [x] Add CI test verifying private/ and config/secrets/ are empty/inaccessible inside container

## Hook Behavior Tests

Verify fail-closed error handling in security-critical hooks.

- [x] Add hook selftest for fail-closed behavior in block-direct-git-ops.py error paths

## Concurrency Tests

Verify file locking prevents metadata.json corruption under concurrent access.

- [x] Add selftest for filelock-based metadata.json locking (concurrent write detection)

## CI Integration

Wire all new tests into the cargo make ci pipeline.

- [x] Integrate new security tests into cargo make ci pipeline
