...
- 実装フットプリントは workflow SSoT（`.harness/workflows/track/adr2pr.md`）、両 adapter（`.claude/commands/track/adr2pr.md` / `.agents/skills/track-adr2pr/SKILL.md` の報告形式）、D3 の一文追記（`knowledge/conventions/pre-track-adr-authoring.md` の機構整合節）、および Claude Code 経路の自律実行に必要な template-shipped recommended default allowlist（`.claude/settings.json` の `Bash(gh pr view:*)` / `Bash(gh pr comment --body-file:*)`）である。この allowlist は consumer-owned な permission guidance であり、CI enforcement ではないため、Rust / CI 設定に変更はない。reason の書式は自由記述のまま — D3 は含めるべき情報を定めるのであって、構造化 schema を課すのではない。
