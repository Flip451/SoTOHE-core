...
- 実装フットプリントは workflow SSoT（`.harness/workflows/track/adr2pr.md`）、両 adapter（`.claude/commands/track/adr2pr.md` / `.agents/skills/track-adr2pr/SKILL.md` の報告形式）、D3 の一文追記（`knowledge/conventions/pre-track-adr-authoring.md` の機構整合節）、`gh pr comment --body-file <path>` 以外を拒否する argv 検証 wrapper（`Makefile.toml` と consumer scaffold 用 `overlay/Makefile.toml` の `pr-audit-comment` task）、exported scaffold の task 集合と wrapper 挙動（root task との drift、invalid argv の拒否、valid invocation から `gh` への 1 回到達）を検証する Rust test（`apps/cli/tests/consumer_scaffold_host_first.rs`）の追随、および両 host の自律実行 permission wiring である。permission wiring は Claude Code の template-shipped recommended default allowlist（`.claude/settings.json` の `Bash(gh pr view:*)` / `Bash(cargo make pr-audit-comment:*)`）と Codex の prefix rules（`.codex/rules/default.rules` の `gh pr view` / `cargo make pr-audit-comment`）から成る。wrapper は posting task であって CI gate ではないため、production Rust behavior / CI gate wiring に変更はないが、Rust test は expected-task set と wrapper の behavioral / drift coverage を更新する。reason の書式は自由記述のまま — D3 は含めるべき情報を定めるのであって、構造化 schema を課すのではない。
