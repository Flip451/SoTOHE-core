...
ゲート・検証系タスク(テスト実行・義務評価・コミットゲート等)の標準出力の契約を、判定(PASS/FAIL)・フルログのファイルパス・失敗時のみの抜粋(失敗項目と短い理由)に統一する。フルログは `tmp/gate/` 配下に残し、調査時に開く。成功時に個別 PASS 行・内部レコードの Debug 表現を stdout に出さない。
