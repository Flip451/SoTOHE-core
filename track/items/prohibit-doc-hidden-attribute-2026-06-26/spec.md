...
- [IN-03] `#[cfg(test)]` または `#[test]` で gated された item のスキャン除外。既存 `syn_helpers.rs` の `has_cfg_test_attr` を流用し、テストコードは検出対象としない。 [adr: knowledge/adr/2026-06-26-0810-prohibit-doc-hidden-attribute.md#D2, knowledge/adr/2026-06-26-0810-prohibit-doc-hidden-attribute.md#D1] [tasks: T001]
- [IN-04] `libs/infrastructure/src/verify/` 配下に共通 syn 走査ヘルパ（`.rs` ファイル再帰発見 → `syn::parse_file` → `#[cfg(test)]` / `#[test]` 除外 → コールバック visitor による判定）を新設する。既存 `syn_helpers.rs` の `has_cfg_test_attr` を再利用し、判定ロジックはコールバックで受ける設計とする。後続の syn ベース lint（例: `Result<_, String>` 禁止）でも再利用できる粒度とする。 [adr: knowledge/adr/2026-06-26-0810-prohibit-doc-hidden-attribute.md#D1] [tasks: T001]
- [IN-05] ゲートの有効化スコープを `architecture-rules.json` の `layers[]` から実行時に取得する。per-layer フラグや特定 crate パスのハードコードは持たず、`layers[]` に新 crate が追加されれば自動的に対象に含まれる。 [adr: knowledge/adr/2026-06-26-0810-prohibit-doc-hidden-attribute.md#D2] [tasks: T001]
...
- [CN-02] 検出は可視性（`pub` / `pub(crate)` / 非 pub）を問わず全 item に適用する。有効可視性の機械解析は行わず、属性の有無のみで判定する。 [adr: knowledge/adr/2026-06-26-0810-prohibit-doc-hidden-attribute.md#D2] [tasks: T001]
- [CN-03] ゲートのスコープは `architecture-rules.json` の `layers[]` を実行時に参照して決定する。特定 crate のパスや名前をコードにハードコードしない。`layers[]` に新 crate が追加されれば自動的にスコープに含まれる（per-layer フラグなし）。 [adr: knowledge/adr/2026-06-26-0810-prohibit-doc-hidden-attribute.md#D2] [tasks: T001, T002]
...
- [ ] [AC-02] `layers[]` のいずれかの crate の非テストソースに `#[doc(hidden)]` 相当の属性が存在する場合、ゲートが exit non-zero を返し、`#[doc(hidden)]` 属性の存在をファイルパス・行情報とともに示す finding を出力する。 [adr: knowledge/adr/2026-06-26-0810-prohibit-doc-hidden-attribute.md#D2] [tasks: T001]
- [ ] [AC-03] `#[cfg(test)]` または `#[test]` で gated された item 内の `#[doc(hidden)]` はスキャン対象から除外され、finding を出力せずにゲートが pass する。 [adr: knowledge/adr/2026-06-26-0810-prohibit-doc-hidden-attribute.md#D2] [tasks: T001]
...
- [ ] [AC-07] IN-04 で導入した共通走査ヘルパが、後続の syn ベース lint（例: `Result<_, String>` 禁止）から再利用できる設計になっている。具体的には、ファイル再帰発見・`syn::parse_file`・cfg(test) 除外を共通基盤とし、判定ロジックのみをコールバックで差し替えられること。 [adr: knowledge/adr/2026-06-26-0810-prohibit-doc-hidden-attribute.md#D1] [tasks: T001]
