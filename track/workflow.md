...
| Phase 0 (初期化) | `/track:init` (新設予定) | `track/items/<id>/metadata.json` (identity のみ) | main | ADR 存在確認 | — |
| Phase 1 (振る舞い契約) | `/track:spec` (新設予定) | `track/items/<id>/spec.json` | spec-designer | 仕様書 → ADR signal (🔵🟡🔴) | ADR + convention (SoT Chain ①) |
| Phase 2 (型設計) | `/track:design` (既存) | `track/items/<id>/<layer>-types.json` | type-designer | 型契約 → 仕様書 signal (signal 未実装期は schema 存在のみ) | spec (SoT Chain ②) |
| Phase 3 (実装計画) | `/track:impl-plan` (新設予定) | `track/items/<id>/impl-plan.json` + `track/items/<id>/task-coverage.json` | impl-planner | task-coverage binary pass/fail | 型カタログ + spec |
| Phase 4 以降 (実装) | `/track:implement` | 実装コード | 実装者 | 実装 → 型契約 signal (rustdoc 突合) | 型カタログ (SoT Chain ③) |

- `/track:plan` は Phase 0-3 を順次 invoke する orchestrator (独立 phase コマンドが揃うまでは `/track:plan` が暫定的に全 phase を扱う)
...
`/track:plan <feature>` は Phase 0-3 を順次 invoke して `track/items/<id>/` にトラック成果物を作成する (独立 phase コマンドが揃うまでは `/track:plan` が全 phase を暫定的に扱う)：
