...
- 自動生成ビュー: `plan.md` (`cargo make track-sync-views` で `metadata.json` から生成)
...
- `track/registry.md` — `.gitignore` 済み。`cargo make track-sync-views` で再生成される
...
`track/registry.md` は以下のタイミングで `metadata.json` から自動再生成される (生成ロジックは `cargo make track-sync-views` 経由の `sotp track views sync`)。
