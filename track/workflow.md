...
Specialist capability の実体は `.harness/config/agent-profiles.json` で決まる。既定では `orchestrator` / `spec-designer` / `type-designer` / `impl-planner` / `adr-editor` / `implementer` が Claude Code、`reviewer` が Codex、`researcher` が Gemini に割り当てられる。
