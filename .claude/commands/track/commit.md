...
- If empty, do not commit immediately. Inspect the latest track, current diff, and recent task context, then propose 2-3 concrete commit message candidates and ask the user to choose one or edit one.
...
1. Inspect the latest active track under `track/items/` when present.
2. Read available context from `spec.md`, `plan.md`, `verification.md`, and current changed files.
3. Propose 2-3 commit message candidates that follow the repository's current style and reflect the actual change scope.
4. Stop after presenting the candidates. Do not run `cargo make track-commit-message` yet.
...
3. If the latest active track exists, read `track/registry.md` and confirm it reflects the current track status, next command, and completion state.
4. If `track/registry.md` is stale for the current track, update and stage it as part of this same commit (pre-commit step, not post-commit):
...
1. Find the latest track under `track/items/` (if any exist).
