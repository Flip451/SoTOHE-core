...
- After writing spec.json (with `status: "draft"`), evaluate signals and then approve:
  1. Run `cargo make track-signals <track-id>` to compute confidence signals from source tags.
     If the command succeeds, `spec.json` `signals` field is updated in-place.
     If the command fails, check whether `spec.json` was partially updated (signals may have been written before the failure). If so, the file is still valid — proceed to step 2 to approve the current state. If `signals` remains `null`, also proceed to step 2.
  2. Run `cargo make spec-approve <track-dir>` to set `status=approved` with `approved_at` and `content_hash`.
     **Important**: `spec-approve` must always be the **last** step that modifies `spec.json`. If signals are re-evaluated later (e.g., after adding source tags), re-run `cargo make spec-approve` to update `content_hash`.
  3. Run `cargo make track-sync-views` to generate `spec.md` as a read-only rendered view from spec.json. Do NOT write `spec.md` directly — it is generated by `render_spec()`.
...
- **`sources` format**: Store raw reference strings WITHOUT the `[source: ...]` wrapper. The renderer adds the wrapper automatically. Example: `"sources": ["knowledge/strategy/vision.md §7"]` not `"sources": ["[source: knowledge/strategy/vision.md §7]"]`.
- **`domain_states` descriptions**: Avoid `|` characters in description text — they break the rendered Markdown table. Use `,` or `/` as separators instead.
