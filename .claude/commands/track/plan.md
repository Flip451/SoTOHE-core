...
3. Apply the loop rule:
   - 🔵 (all elements blue): run the semantic review as the phase first-line defense — `bin/sotp ref-verify run` (see `/track:ref-verify`; the scope derives from track artifact existence, so at this phase only Chain① — spec → ADR, judged with `.harness/prompts/ref-verifier-chain1.md` — contributes pairs because no catalogue exists yet). On `[BLOCKED]`, treat the failing pair like a 🔴 below (route to adr-editor or re-invoke `/track:spec-design` depending on which side of the pair is wrong). On `[ESCALATE]`, report to the user and stop (do not retry). On `[OK]`, mark the Phase 1 task `completed` and proceed to Phase 2.
...
3. Apply the loop rule:
   - 🔵 across all processed layers: run the semantic review as the phase first-line defense — a single `bin/sotp ref-verify run` invocation (see `/track:ref-verify`; the scope derives from track artifact existence, so with all catalogues now present both chains are covered in one run — Chain② judges catalogue → spec with `.harness/prompts/ref-verifier-chain2.md`, and any Chain① pairs invalidated by spec edits during this phase are re-verified in the same run; the differential cache keeps unchanged pairs out of re-review). On `[BLOCKED]`, route by the owning side and apply the **Phase 2 retry counter**: for catalogue-side failures (catalogue claim does not match spec evidence), invoke `type-designer`, then re-run the semantic review; for spec-side failures (spec text is wrong), treat as a Phase 2 🔴 and follow the full 🔴 path below (re-invoke `/track:spec-design`, re-evaluate the Phase 1 gate including its semantic review, then re-invoke `/track:type-design` and re-run the semantic review). **max_retry guard**: each BLOCKED → fix → re-run iteration counts against the Phase 2 retry counter, identical to the 🔴 branch; on overflow, stop and present options. On `[ESCALATE]`, report to the user and stop (do not retry). On `[OK]`, mark the Phase 2 task `completed` and proceed to Phase 3.
...
- `knowledge/conventions/prefer-type-safe-abstractions.md` — enum-first / typestate / newtype principles
