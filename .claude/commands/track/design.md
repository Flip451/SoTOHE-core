...
1. Run `sotp track baseline-capture <id> [--layer <layer_id>] [--force]` to snapshot the current TypeGraph as `<catalogue-stem>-baseline.json`. When `--layer` is omitted here, the command iterates all enabled layers in order — equivalent to calling it once per layer. The baseline filename is derived from the layer's `catalogue_file` (e.g., `domain-types-baseline.json`, `usecase-types-baseline.json`). The capture is idempotent — if a baseline already exists it is skipped (use `--force` to regenerate).
