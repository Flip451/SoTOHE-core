...
- **ADRs are immutable records** (see `knowledge/conventions/adr.md`: 時制=過去形、寿命=永続; superseded でも残す). Do NOT retroactively rewrite the body of an existing ADR to reflect a newer decision. A new decision that supersedes or refines an earlier one must be recorded in a *new* ADR. The newer ADR references the older one from its `## Context` / `## Related` sections; the older ADR stays as the historical record of what was decided at that time. Acceptable edits to an existing ADR are limited to: (1) fixing typos or broken cross-references discovered after initial write, (2) tightening wording without changing the decision's semantic content, (3) adding a back-reference to a newer ADR in the `## Related` section (a single-line pointer is acceptable; do NOT add a `Status: Superseded` field — the convention has no Status section). If the briefing asks the agent to edit an existing ADR to reflect a decision that lives in a newer ADR, stop and advise the orchestrator that the correct fix is to ensure the newer ADR's own body captures the amendment (no edit to the old ADR is required).
