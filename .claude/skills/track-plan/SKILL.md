...
/track:activate <track-id> ← planning-only track を materialize
    ↓
/track:full-cycle           ← タスクごとの implement → review → commit ループ
    または
/track:implement           ← 並列実装（手動で /track:review → /track:commit を繰り返す）
```
...
- **Claude (default profile)**: Agent tool で custom subagent `planner` を起動する。
  `subagent_type: "planner"` を指定することで、`.claude/agents/planner.md` の
  `model: opus` frontmatter が効いて Claude Opus が自動選択される。
  briefing file の内容を subagent のプロンプトに含める。メインコンテキストの文脈を
  引き継げるため、外部プロセス (`-p`) より設計レビューに適している。
  (旧: built-in `subagent_type: "Plan"` + `model: "opus"` 指定。custom agent
  への移行で model 指定忘れ時の Sonnet フォールバックを防止する。)
...
Teammate 2 — Planner / Architect capability（provider は active profile で決まる）:
  ...
  - Provider-specific invocation:
    - Claude (default): Agent tool (subagent_type: "planner") で起動 (frontmatter で Opus 固定)
...
### 次のステップ
- この計画で進めてよろしいですか？
- 承認後: `cargo make track-signals <track-id>` → `cargo make spec-approve <track-dir>` → `cargo make track-sync-views` の順で仕様を確定
- standard lane なら `/track:full-cycle` または `/track:implement`、plan-only lane なら `/track:activate <track-id>` を経由して実装を開始
```
