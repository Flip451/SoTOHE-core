...
class AgentProfilesTest(unittest.TestCase):
    ...
    def test_provider_label_and_example_follow_active_profile(self) -> None:
        ...
        self.assertEqual(
            agent_profiles.takt_host_model(profiles=profiles), "claude-opus-4-6"
        )
        self.assertEqual(
            agent_profiles.takt_host_label(profiles=profiles), "Claude Code"
        )
    ...
    def test_load_profiles_rejects_unsupported_takt_host_provider(self) -> None:
        def mutator(profiles: dict) -> None:
            profiles["profiles"]["default"]["takt_host_provider"] = "gemini"
    ...
    def test_load_profiles_rejects_missing_takt_host_model(self) -> None:
        def mutator(profiles: dict) -> None:
            del profiles["profiles"]["default"]["takt_host_model"]

        path = self.write_mutated_profiles(mutator)

        with self.assertRaisesRegex(
            agent_profiles.AgentProfilesError, "takt_host_model"
        ):
            agent_profiles.load_profiles(path)
