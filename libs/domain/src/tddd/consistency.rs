...
enum TypeSignalGate<'a> {
    Clean,
    Red(Vec<&'a str>),
    Yellow(Vec<&'a str>),
}

fn classify_type_signal_gate(signals_doc: &TypeSignalsDocument) -> TypeSignalGate<'_> {
    let mut red_names = Vec::new();
    let mut yellow_names = Vec::new();

    for signal in signals_doc.signals() {
        match signal.signal() {
            ConfidenceSignal::Red => red_names.push(signal.type_name()),
            ConfidenceSignal::Yellow => yellow_names.push(signal.type_name()),
            ConfidenceSignal::Blue => {}
        }
    }

    if !red_names.is_empty() {
        TypeSignalGate::Red(red_names)
    } else if !yellow_names.is_empty() {
        TypeSignalGate::Yellow(yellow_names)
    } else {
        TypeSignalGate::Clean
    }
}
...
pub fn check_type_signals(
    signals_doc: &TypeSignalsDocument,
    strictness: Strictness,
) -> VerifyOutcome {
    match classify_type_signal_gate(signals_doc) {
        ...
        TypeSignalGate::Clean => VerifyOutcome::pass(),
        // Red check: applies to all signals (forward + reverse, all regions).
        TypeSignalGate::Red(all_red) => {
        ...
        // filter is needed).
        TypeSignalGate::Yellow(all_yellow) => {
            let message = format!(
                "{} type(s) have Yellow signal: {} — merge gate will block these until upgraded to Blue. Resolve each type (implement or remove per its declared action) and re-run `sotp signal calc-impl-catalog`.",
                all_yellow.len(),
                all_yellow.join(", ")
            );
            match strictness {
...
mod tests {
    ...
    fn test_check_type_signals_empty_signals_passes_per_adr_d64() {
        ...
        let outcome = check_type_signals(&doc, Strictness::Interim);
    ...
    fn test_check_type_signals_red_is_error_regardless_of_mode() {
        ...
        let outcome_interim = check_type_signals(&doc, Strictness::Interim);
        assert!(
            outcome_interim.has_errors(),
            "red in interim must be an error: {outcome_interim:?}"
        );
        let outcome_strict = check_type_signals(&doc, Strictness::Strict);
    ...
    fn test_check_type_signals_yellow_is_error_in_strict_mode() {
        ...
        let outcome = check_type_signals(&doc, Strictness::Strict);
    ...
    /// D4 regression: documents that `check_type_signals(signals_doc, Strictness::Interim)`
    ...
    ///   (`check_type_signals(doc, Strictness::Interim)`) unconditionally."
    ///
