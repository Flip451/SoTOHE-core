...
use crate::tddd::catalogue_v2::roles::Layer;
...
/// - `layer`: the architectural layer (`Domain` / `Usecase` / `Infrastructure`).
...
impl CatalogueDocument {
    ...
    #[must_use]
    pub fn new(schema_version: u32, crate_name: CrateName, layer: Layer) -> Self {
        Self {
            schema_version,
            crate_name,
            layer,
...
mod tests {
    ...
    fn test_catalogue_document_new_creates_empty_document() {
        ...
        let doc = CatalogueDocument::new(2, crate_name.clone(), Layer::Domain);
    ...
    fn test_catalogue_document_with_usecase_layer() {
        ...
        let doc = CatalogueDocument::new(2, crate_name.clone(), Layer::Usecase);
        assert_eq!(doc.layer, Layer::Usecase);
    }

    #[test]
    fn test_catalogue_document_with_infrastructure_layer() {
        let crate_name = CrateName::new("infrastructure").unwrap();
        let doc = CatalogueDocument::new(2, crate_name.clone(), Layer::Infrastructure);
        assert_eq!(doc.layer, Layer::Infrastructure);
    }
    ...
    fn test_catalogue_document_types_btreemap_stores_type_entry() {
        let crate_name = CrateName::new("domain").unwrap();
        let mut doc = CatalogueDocument::new(2, crate_name, Layer::Domain);
    ...
    fn test_catalogue_document_functions_btreemap_stores_function_entry() {
        let crate_name = CrateName::new("domain").unwrap();
        let mut doc = CatalogueDocument::new(2, crate_name.clone(), Layer::Domain);
        let fn_path =
    ...
    fn test_validate_filename_with_matching_stem_returns_ok() {
        let crate_name = CrateName::new("domain").unwrap();
        let doc = CatalogueDocument::new(2, crate_name, Layer::Domain);
