...
/// The crate scope a `trait_ref` resolves to (IN-17).
...
impl TraitImplDeclV2 {
    /// Creates a `TraitImplDeclV2` from its complete private state.
    #[must_use]
    pub fn from_parts(
        action: ItemAction,
        trait_ref: TypeRef,
        ...
        Self { action, trait_ref, for_type, impl_generics, impl_where_predicates }
    }
    ...
    pub fn new(trait_ref: TypeRef, for_type: TypeRef) -> Self {
        Self::from_parts(ItemAction::Add, trait_ref, for_type, vec![], vec![])
    }

    /// Reads the TDDD action for this impl declaration.
    #[must_use]
    pub fn action(&self) -> ItemAction {
        self.action
    }

    /// Reads the trait reference implemented by this declaration.
    #[must_use]
    pub fn trait_ref(&self) -> &TypeRef {
        &self.trait_ref
    }

    /// Reads the self type for this impl declaration.
    #[must_use]
    pub fn for_type(&self) -> &TypeRef {
        &self.for_type
    }

    /// Reads impl-block-level generic parameters.
    #[must_use]
    pub fn impl_generics(&self) -> &[MethodGenericParam] {
        &self.impl_generics
    }

    /// Reads impl-block-level where predicates.
    #[must_use]
    pub fn impl_where_predicates(&self) -> &[WherePredicateDecl] {
        &self.impl_where_predicates
    }

    /// Classifies the `trait_ref` as self-crate, workspace-member, or external
    /// for `trait_impl` obligation derivation (IN-17).
    ///
    /// Generics / turbofish are stripped first. A bare identifier is
    /// [`TraitRefScope::SelfCrate`]; a path whose first segment is a workspace
    /// member is [`TraitRefScope::Workspace`] (keyed by its last segment);
    /// anything else is [`TraitRefScope::External`]. Malformed identifiers
    /// fail-safe to `External`.
    #[must_use]
    pub fn trait_ref_scope(&self) -> TraitRefScope {
        let raw = self.trait_ref.as_str();
...
mod tests {
    ...
    #[test]
    fn test_trait_ref_scope_self_crate_strips_generics() {
        let local =
    ...
    #[test]
    fn test_trait_ref_scope_workspace_for_each_member_crate() {
        for crate_id in
            ["domain", "usecase", "infrastructure", "cli", "cli_driver", "cli_composition"]
        {
            let decl = TraitImplDeclV2::new(
                TypeRef::new(format!("{crate_id}::Bar")).unwrap(),
    ...
    #[test]
    fn test_trait_ref_scope_external_for_std_trait() {
        let external = TraitImplDeclV2::new(
            TypeRef::new("std::fmt::Display").unwrap(),
            TypeRef::new("Foo").unwrap(),
        );
        assert_eq!(external.trait_ref_scope(), TraitRefScope::External);
    }
    ...
    fn test_trait_ref_scope_external_for_malformed_path_segments() {
        for trait_ref in ["Foo::", "domain::", "::Foo", "domain::::Foo"] {
            ...
            assert_eq!(decl.trait_ref_scope(), TraitRefScope::External);
        }
    }
    ...
    fn test_trait_impl_decl_v2_modify_action_can_be_set() {
        let decl = TraitImplDeclV2::from_parts(
            ItemAction::Modify,
            TypeRef::new("std::fmt::Display").unwrap(),
            TypeRef::new("MyType").unwrap(),
            vec![],
    ...
    fn test_trait_impl_decl_v2_action_participates_in_equality() {
        let trait_ref = TypeRef::new("std::fmt::Display").unwrap();
        let for_type = TypeRef::new("MyType").unwrap();
        let base = TraitImplDeclV2::new(trait_ref.clone(), for_type.clone());
        let with_modify =
            TraitImplDeclV2::from_parts(ItemAction::Modify, trait_ref, for_type, vec![], vec![]);
    ...
    fn test_trait_impl_decl_v2_impl_generics_participates_in_equality() {
        ...
        let with_generics = TraitImplDeclV2::from_parts(
            ItemAction::Add,
            TypeRef::new("serde::Serialize").unwrap(),
            TypeRef::new("MyStruct").unwrap(),
    ...
    fn test_trait_impl_decl_v2_impl_where_predicates_participates_in_equality() {
        ...
        let with_where = TraitImplDeclV2::from_parts(
            ItemAction::Add,
            TypeRef::new("serde::Serialize").unwrap(),
            TypeRef::new("MyStruct").unwrap(),
            vec![],
