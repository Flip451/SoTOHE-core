...
/// Error returned by [`run_example_demo`].
#[derive(Debug, thiserror::Error)]
pub enum DemoRunError {
    /// Track creation or persistence failed.
    #[error("{0}")]
    Unavailable(String),
}
...
impl usecase::demo::DemoPort for FsDemoAdapter {
    fn run(&self) -> Result<String, usecase::demo::DemoPortError> {
        run_example_demo().map_err(|e| usecase::demo::DemoPortError::Unavailable(e.to_string()))
    }
}
...
/// # Errors
///
/// Returns [`DemoRunError`] if track creation or persistence fails.
pub fn run_example_demo() -> Result<String, DemoRunError> {
    ...
    save.execute(&track)
        .map_err(|e| DemoRunError::Unavailable(format!("failed to save example track: {e}")))?;
