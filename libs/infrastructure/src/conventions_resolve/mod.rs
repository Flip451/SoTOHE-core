//! Front-matter codec for convention documents (spec `IN-05`, `AC-06`,
//! `AC-07`, `AC-08`).
//!
//! This module is the serde boundary of convention resolution: it turns one
//! document's text into the declarations its front matter carries, and it is
//! the only site that decides the two decode conditions `AC-07` names — front
//! matter that is not parseable YAML, and a `required_for` value that is not an
//! array of strings. The remaining `AC-07` conditions belong elsewhere: the
//! path rule to [`usecase::conventions_resolve::ConventionDocumentPath`], the
//! unreadable document to the directory scan, and the empty identifier to
//! [`ConventionCapabilityId::try_new`], whose rejection this module only
//! translates.
...
use crate::capability_exec::read_front_matter;
...
pub struct ConventionFrontMatterDto {
    required_for: Vec<CapabilityIdField>,
}
...
/// notion of where a front-matter block starts and ends.
///
...
pub fn parse_convention_front_matter(
    ...
    // below attributable to `required_for` alone.
