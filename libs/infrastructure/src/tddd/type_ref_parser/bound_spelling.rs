...
/// Rustdoc emits `args: null` both for a path without generic arguments and
/// for an explicitly empty list (`Tr<>` vs `Tr`), and the converter likewise
/// collapses the empty list, so the notation difference would silently
/// compare equal. The explicit empty-list spelling is rejected.
#[derive(Default)]
struct EmptyGenericArgumentListVisitor {
    found: bool,
}

impl<'ast> Visit<'ast> for EmptyGenericArgumentListVisitor {
    fn visit_angle_bracketed_generic_arguments(
        &mut self,
        node: &'ast syn::AngleBracketedGenericArguments,
    ) {
        self.found |= node.args.is_empty();
        syn::visit::visit_angle_bracketed_generic_arguments(self, node);
    }
}

pub(super) fn reject_empty_generic_argument_lists_in_bound(
    syntax: &syn::TypeParamBound,
) -> Result<(), String> {
    let mut visitor = EmptyGenericArgumentListVisitor::default();
    visitor.visit_type_param_bound(syntax);
    reject_if_empty_generic_argument_list_found(visitor.found)
}

pub(super) fn reject_empty_generic_argument_lists_in_type(
    syntax: &syn::Type,
) -> Result<(), String> {
    let mut visitor = EmptyGenericArgumentListVisitor::default();
...
fn reject_if_empty_generic_argument_list_found(found: bool) -> Result<(), String> {
    if found {
        Err("explicitly empty generic argument lists are not supported by lexical type comparison"
            .to_owned())
    } else {
        Ok(())
    }
