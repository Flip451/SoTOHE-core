...
/// `<track_dir>/<layer_id>-graph-d<depth>/`.
///
/// Produces:
/// - `<layer_id>-graph-d<depth>/index.md` — cluster overview (`render_type_graph_overview`)
/// - `<layer_id>-graph-d<depth>/<cluster>.md` — per-cluster diagrams
...
///
/// The depth-suffix on the output directory ensures depth=1 and depth=2 outputs
/// coexist in independent paths and never overwrite each other (per ADR
/// `2026-04-25-0530-...` D2). Flat-mode output (`<layer_id>-graph.md`) is unchanged.
///
...
pub fn write_type_graph_dir(
    ...
    // Target directory: `<layer_id>-graph-d<depth>/`.
    // The depth-suffix on the directory name ensures depth=1 and depth=2 outputs
    // coexist in independent paths (per ADR `2026-04-25-0530-...` D2).
    let cluster_depth = opts.cluster_depth;
    let graph_dir = track_dir.join(format!("{layer_id}-graph-d{cluster_depth}"));
    ...
    //
    // Note: `<layer_id>-graph-d<depth>/` is a fully tool-managed output directory.
    ...
    // inside a `<layer_id>-graph-d<depth>/` directory.
    if graph_dir.is_dir() {
    ...
    written.push(format!("{layer_id}-graph-d{cluster_depth}/index.md"));
    ...
    for cluster_key in cluster_keys {
        ...
        written.push(format!("{layer_id}-graph-d{cluster_depth}/{filename}"));
    }
...
mod tests {
    ...
    /// and one file per cluster under `<layer_id>-graph-d<depth>/`.
    #[test]
    fn test_write_type_graph_dir_creates_expected_structure() {
        ...
        assert!(
            written.contains(&"domain-graph-d2/index.md".to_owned()),
            "must write index.md under depth-suffixed dir, got: {written:?}"
        );
        ...
        // Verify files exist on disk under depth-suffixed directory.
        let graph_dir = track_dir.join("domain-graph-d2");
        ...
        // Regression guard: legacy suffix-less directory must NOT be created.
        assert!(
    ...
    /// ADR `2026-04-25-0530-...` D2 — depth=1 and depth=2 outputs coexist in
    /// independent paths and never overwrite each other.
    #[test]
    fn test_write_type_graph_dir_depth1_and_depth2_coexist() {
        let tmp = TempDir::new().unwrap();
        let track_dir = tmp.path().join("track_dir");
        std::fs::create_dir_all(&track_dir).unwrap();
        ...
        // Run depth=1 first.
        let opts_d1 = TypeGraphRenderOptions { cluster_depth: 1, ..Default::default() };
        let r1 = write_type_graph_dir(&graph, "domain", &track_dir, tmp.path(), &opts_d1);
        assert!(r1.is_ok(), "depth=1 must succeed: {r1:?}");
        assert!(
            track_dir.join("domain-graph-d1").join("index.md").exists(),
            "depth=1 output must exist after first run"
        );

        // Then run depth=2 on the same track dir.
        let opts_d2 = TypeGraphRenderOptions { cluster_depth: 2, ..Default::default() };
        let r2 = write_type_graph_dir(&graph, "domain", &track_dir, tmp.path(), &opts_d2);
        assert!(r2.is_ok(), "depth=2 must succeed: {r2:?}");

        // Both depth outputs must coexist after the second run.
        assert!(
            track_dir.join("domain-graph-d1").join("index.md").exists(),
            "depth=1 output must remain after depth=2 run (no cross-depth overwrite)"
        );
        assert!(
            track_dir.join("domain-graph-d2").join("index.md").exists(),
            "depth=2 output must exist after second run"
        );
    }
