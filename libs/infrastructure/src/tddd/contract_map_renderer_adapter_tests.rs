...
/// Renders using the full style config and returns the bare mermaid string
/// (with the markdown fence + generated marker stripped).
///
/// The adapter wraps render output in a Markdown fenced-code-block plus a
/// generated marker (so viewers render the diagram instead of plain text — PR #133
/// review fix). Tests in this module continue to assert against the bare mermaid
/// payload because the wrapping is a presentation concern and the structural
/// guarantees (subgraph nesting, classDef ordering, edge style) live in the
/// inner payload.
fn render_with_full_style(catalogues: &[CatalogueDocument], layer_order: &[LayerId]) -> String {
    ...
    unwrap_mermaid_payload(&result.into_string())
}

/// Strips the `<!-- Generated ... -->\n```mermaid\n` prefix and the trailing
/// "```" + newline so tests see the bare mermaid string the renderer produces.
fn unwrap_mermaid_payload(wrapped: &str) -> String {
    const HEADER: &str =
        "<!-- Generated by ContractMapRendererAdapter — DO NOT EDIT DIRECTLY -->\n```mermaid\n";
    const FOOTER: &str = "\n```\n";
    let after_header = wrapped.strip_prefix(HEADER).unwrap_or_else(|| {
        panic!("wrapped contract-map output missing header; got: {wrapped:?}");
    });
    let payload = after_header.strip_suffix(FOOTER).unwrap_or_else(|| {
        panic!("wrapped contract-map output missing footer; got: {wrapped:?}");
    });
    payload.to_owned()
}
...
fn test_function_node_id_format_matches_spec() {
    ...
    // sl = escape_id_component("domain") = "domain"
    // segments = [escape("domain"), escape("tddd"), escape("register")] joined by "_S_"
    //   → "domain_S_tddd_S_register"
    // body = "domain_domain_S_tddd_S_register"
    // expected: "F22_domain_domain_S_tddd_S_register"
    ...
    assert_eq!(id, "F22_domain_domain_S_tddd_S_register");
}

#[test]
fn test_function_node_id_distinguishes_double_underscore_crate_from_path_segments() {
    // PR #133 review regression guard: `a::b::c` and `a__b::c` are both valid
    // FunctionPaths and both have a 7-char display form. Under the legacy lossy
    // sanitize (`::` → `_`, `_` → `_`) they both collapsed to "a__b__c" inside
    // the F-prefix body, so Mermaid merged distinct function nodes. Per-segment
    // escape_id_component + `_S_` separator must distinguish them.
    let layer = layer("domain");

    let p1 = FunctionPath::new(
        crate_name("a"),
        ModulePath::from_segments(vec!["b".to_owned()]).unwrap(),
        FunctionName::new("c").unwrap(),
    );
    let p2 =
        FunctionPath::new(crate_name("a__b"), ModulePath::root(), FunctionName::new("c").unwrap());

    // Display forms share the 7-char length but the body must differ.
    assert_eq!(p1.to_string().chars().count(), 7);
    assert_eq!(p2.to_string().chars().count(), 7);

    let id1 = function_node_id(&layer, &p1);
    let id2 = function_node_id(&layer, &p2);
    assert_eq!(id1, "F7_domain_a_S_b_S_c");
    assert_eq!(id2, "F7_domain_a____b_S_c");
    assert_ne!(id1, id2, "node IDs must be injective across :: vs __ separators");
}
...
// Test 11b: adapter wraps rendered mermaid in a Markdown fenced-code-block
// (PR #133 review fix — without the fence, Markdown viewers show plain text).
#[test]
fn test_adapter_render_wraps_output_in_markdown_mermaid_fence() {
    let dir = TempDir::new().unwrap();
    let path = write_style_config(&dir, full_toml_content());
    let adapter = ContractMapRendererAdapter::new(path);
    let opts = ContractMapRenderOptions::empty();
    let result = adapter.render(&[], &[], &opts).unwrap();
    let wrapped = result.into_string();

    assert!(
        wrapped.starts_with(
            "<!-- Generated by ContractMapRendererAdapter — DO NOT EDIT DIRECTLY -->\n"
        ),
        "wrapped output must start with the generated marker; got: {wrapped:?}"
    );
    assert!(
        wrapped.contains("\n```mermaid\n"),
        "wrapped output must open a ```mermaid fence; got: {wrapped:?}"
    );
    assert!(wrapped.ends_with("\n```\n"), "wrapped output must close the fence; got: {wrapped:?}");
    assert!(
        wrapped.contains("flowchart TD"),
        "wrapped output must still carry the mermaid payload; got: {wrapped:?}"
    );
}
