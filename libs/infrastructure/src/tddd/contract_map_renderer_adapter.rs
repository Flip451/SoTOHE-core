...
///   - `sfp = escape("domain") + "_S_" + escape("tddd") + "_S_" + escape("register") =
///     "domain_S_tddd_S_register"`
///   - `sl = "domain"`
///   - Result: `F22_domain_domain_S_tddd_S_register`
pub(crate) fn function_node_id(layer: &LayerId, path: &FunctionPath) -> String {
    let sl = render::escape_id_component(layer.as_ref());
    // Display form serves only as the length-prefix (a stable count of the unsanitized
    // `crate::module::name` rendering). The body is composed from escape_id_component
    // applied to each FunctionPath segment (crate_name, module_path segments, name),
    // joined by a `_S_` separator that escape_id_component never emits. This is
    // injective across `::` separators and literal `_` characters — without segment
    // escaping, functions like `a::b::c` (crate=a, module=[b], name=c) and `a__b::c`
    // (crate=a__b, module=[], name=c) collide because `::` and `_` both collapse to
    // `_` under the legacy `sanitize` helper.
    let full_path_raw = path.to_string();
    let len = full_path_raw.chars().count();
    let mut segments: Vec<String> = Vec::new();
    segments.push(render::escape_id_component(path.crate_name.as_str()));
    for seg in path.module_path.segments() {
        segments.push(render::escape_id_component(seg.as_str()));
    }
    segments.push(render::escape_id_component(path.name.as_str()));
    let sfp = segments.join("_S_");
...
impl ContractMapRenderer for ContractMapRendererAdapter {
    ...
    fn render(
        ...
        Ok(ContractMapContent::new(wrapped))
    }
}
