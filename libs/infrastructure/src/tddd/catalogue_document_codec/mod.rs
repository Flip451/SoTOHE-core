...
mod tests {
  ...
  "functions": {}
    ...
    // -----------------------------------------------------------------------
    // IN-13 / AC-12: TraitImplDeclV2 for_ field codec tests
    // -----------------------------------------------------------------------

    #[test]
    fn test_decode_trait_impl_with_for_field_some_succeeds() {
        // IN-13: `for_` field with a Some value must decode to the domain struct field.
        let json = r#"{
  "schema_version": 3,
  "crate_name": "domain",
  "layer": "domain",
      ...
      "trait_impls": [
        {
          "trait_name": "Iterator",
          "origin_crate": "core",
          "for_": "external_crate::ExternalType"
        }
      ]
    }
  },
  ...
  "functions": {}
        ...
        let entry = doc.types.values().next().unwrap();
        assert_eq!(entry.trait_impls.len(), 1);
        assert_eq!(entry.trait_impls[0].for_.as_deref(), Some("external_crate::ExternalType"));
    }

    #[test]
    fn test_decode_trait_impl_without_for_field_defaults_to_none() {
        // IN-13 backward compat: catalogues that predate `for_` must load with None.
        let json = r#"{
  "schema_version": 3,
  "crate_name": "domain",
  "layer": "domain",
  ...
  "functions": {}
        ...
        assert_eq!(entry.trait_impls.len(), 1);
        assert!(
            entry.trait_impls[0].for_.is_none(),
            "omitted for_ field must default to None (CN-01 backward compat)"
        );
    }

    #[test]
    fn test_encode_trait_impl_with_for_field_round_trip() {
        // IN-13: encode → decode must preserve for_ when Some.
        let json = r#"{
  "schema_version": 3,
  "crate_name": "domain",
  "layer": "domain",
  ...
  "functions": {}
    ...
    #[test]
    fn test_encode_trait_impl_with_none_for_omits_field() {
        // IN-13 byte-stability: when for_ is None, the field must NOT appear in JSON.
        let json = r#"{
  "schema_version": 3,
  "crate_name": "domain",
  "layer": "domain",
  ...
  "functions": {}
        ...
        let encoded = CatalogueDocumentCodec::encode(&doc).unwrap();
        assert!(
            !encoded.contains("\"for_\""),
            "for_ must be omitted from JSON when None: {encoded}"
        );
    }

    #[test]
    fn test_decode_trait_impl_with_empty_for_returns_invalid_entry_error() {
        // IN-13 validation: `for_: ""` must be rejected — empty string is not a valid type path.
        let json = r#"{
  "schema_version": 3,
  "crate_name": "domain",
  "layer": "domain",
  "types": {
    "BadType": {
      ...
      "kind": { "kind": "unit_struct" },
      "trait_impls": [
        { "trait_name": "Display", "origin_crate": "core", "for_": "" }
      ]
    }
  },
  "traits": {},
  "functions": {}
        ...
        assert!(
            matches!(result, Err(CatalogueDocumentCodecError::InvalidEntry { .. })),
            "expected InvalidEntry for empty for_, got: {result:?}"
        );
    }

    #[test]
    fn test_decode_trait_impl_with_invalid_type_syntax_for_returns_invalid_entry_error() {
        // IN-13 validation: `for_` must be syntactically valid as a Rust type expression.
        let json = r#"{
  "schema_version": 3,
  "crate_name": "domain",
  "layer": "domain",
  "types": {
    "BadType": {
      ...
      "kind": { "kind": "unit_struct" },
      "trait_impls": [
        { "trait_name": "Display", "origin_crate": "core", "for_": "Vec<" }
      ]
    }
  },
  "traits": {},
  "functions": {}
        ...
        assert!(
            matches!(result, Err(CatalogueDocumentCodecError::InvalidEntry { .. })),
            "expected InvalidEntry for invalid type syntax in for_, got: {result:?}"
        );
    }
}
