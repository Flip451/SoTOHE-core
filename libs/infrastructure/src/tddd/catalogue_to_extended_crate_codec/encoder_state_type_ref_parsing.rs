...
use rustdoc_types::{GenericArg, GenericArgs, GenericBound, Id, Path, Type};
...
impl EncoderState {
    ...
    pub(super) fn parse_type_ref_str(
        ...
        self.parse_type_ref_str_for_context(type_ref_str, None)
    }
    ...
    /// matching an entry in `generic_params` is encoded as `Type::Generic(name)` instead
    /// of falling through to the unresolved-marker path.
    ///
    ...
    pub(super) fn parse_type_ref_str_with_generics(
        ...
        self.parse_type_ref_str_for_context(type_ref_str, Some(generic_params))
    }

    fn parse_type_ref_str_for_context(
        &mut self,
        type_ref_str: &str,
        ...
        G: FnMut(String) -> u32,
    {
        match generic_params {
            Some(generic_params) => parse_type_ref_with_generics(
                type_ref_str,
                resolve_local,
                std_crate_id,
                external_crate_ids,
