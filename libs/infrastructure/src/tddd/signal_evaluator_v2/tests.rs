...
// -----------------------------------------------------------------------
// PR #134 finding 1: orphan-impl ownership preservation for external for_
// -----------------------------------------------------------------------

/// PR #134 finding 1: A-declared `TypeAlias` entry with `TraitImplDeclV2.for_` set to an
/// external type (e.g. `impl MyTrait for external_crate::ExternalTarget`) must still be
/// imported into S by the A-side orphan-impl pass with action `Add`.
///
/// Before this fix, `encode_for_path` set `impl.for_.id` to a synthetic external id, so
/// the orphan pass could not find the parent action via `a_item_actions.get(&for_.id)`.
/// The fallback now uses `a_krate.impl_owner_map()` (populated by the codec) to recover
/// the owning local type and inherit its `ItemAction`.
///
/// Without the fix, the orphan-impl would be silently skipped (parent_action == None),
/// producing a spurious `CMinusSUnionD` Red signal even when the catalogue declaration
/// is correct.
#[test]
fn test_for_override_external_impl_preserves_owning_type_action() {
    use super::phase1::phase1_build_s_and_d;
    use rustdoc_types::{ExternalCrate, Impl, Path as RdPath, TypeAlias};

    let crate_name = "my_crate";
    let root_id = Id(0);
    let alias_id = Id(1);
    // The orphan impl: `impl MyTrait for external_crate::ExternalTarget`.
    let orphan_impl_id = Id(2);
    // Synthetic id for the external target type (`external_crate::ExternalTarget`).
    // This is the id that `encode_for_path` would assign when `for_` is external;
    // it does NOT match `alias_id` (the owning TypeAlias), so the direct lookup
    // `a_item_actions.get(&for_.id)` returns `None` and the fallback must fire.
    let ext_target_id = Id(99);
    // Synthetic id for the external trait `MyTrait`.
    let ext_trait_id = Id(98);

    let mut a_index = HashMap::new();
    let mut a_paths = HashMap::new();
    let mut a_external_crates: HashMap<u32, ExternalCrate> = HashMap::new();

    // Register the external crate (crate_id = 1).
    a_external_crates.insert(
        1,
        ExternalCrate {
            name: "external_crate".to_string(),
            html_root_url: None,
            path: std::path::PathBuf::new(),
        },
    );
    ...
    // TypeAlias `MyAlias` (owning type, action = Add).
    a_index.insert(
        alias_id,
        make_item(
            alias_id,
    ...
    // External target type path entry (crate_id = 1 → external).
    a_paths.insert(
        ext_target_id,
        ItemSummary {
            crate_id: 1,
            path: vec!["external_crate".to_string(), "ExternalTarget".to_string()],
            kind: ItemKind::Struct,
        },
    );

    // External trait path entry (crate_id = 1 → external).
    a_paths.insert(
        ext_trait_id,
        ItemSummary {
            crate_id: 1,
            path: vec!["external_crate".to_string(), "MyTrait".to_string()],
            kind: ItemKind::Trait,
        },
    );

    // The orphan Impl: `impl external_crate::MyTrait for external_crate::ExternalTarget`.
    // `for_.id` = `ext_target_id` (external synthetic id) — NOT `alias_id`.
    // Without `impl_owner_map`, the orphan pass cannot find the parent action and skips
    // this impl, producing a spurious CMinusSUnionD Red.
    let orphan_impl_inner = Impl {
        is_unsafe: false,
        generics: empty_generics(),
        provided_trait_methods: vec![],
        trait_: Some(RdPath {
            path: "external_crate::MyTrait".to_string(),
            id: ext_trait_id,
            args: None,
        }),
        for_: Type::ResolvedPath(RdPath {
            path: "ExternalTarget".to_string(),
            id: ext_target_id,
            args: None,
        }),
        items: vec![],
    ...
    let mut a_actions = BTreeMap::new();
    a_actions.insert(alias_id, ItemAction::Add);
    // `orphan_impl_id` has no action entry — the pass inherits action from the owning type.
    ...
    let a_krate = Crate {
        ...
        index: a_index,
        paths: a_paths,
        external_crates: a_external_crates,
        format_version: FORMAT_VERSION,
        target: Target { triple: String::new(), target_features: vec![] },
    };

    // Populate the `impl_owner_map` as the catalogue codec would: `orphan_impl_id → alias_id`.
    // This simulates what `encode_trait_impl_blocks` writes into `EncoderState.impl_orphan_owner`
    // and what `ExtendedCrate::with_impl_owner_map` attaches to the graph.
    let mut impl_owner = HashMap::new();
    impl_owner.insert(orphan_impl_id, alias_id);
    let a = ExtendedCrate::new(a_krate, a_actions).with_impl_owner_map(impl_owner);
    ...
    // Assert: the orphan impl (identified by its trait path) must appear in S.
    // Before the fix, the orphan pass skipped it because `for_.id` (ext_target_id)
    // was not in `a_item_actions`, and there was no fallback.
    let s_impl = s.krate().index.values().find(|item| {
        if let ItemEnum::Impl(impl_) = &item.inner {
            impl_.trait_.as_ref().map(|p| p.path.as_str()) == Some("external_crate::MyTrait")
        } else {
            false
        }
    });
    assert!(
        s_impl.is_some(),
        "PR#134 finding 1: orphan A-side Impl with external for_ target must be imported \
         into S via impl_owner_map fallback; S Impls: {:?}",
        s.krate()
            .index
            .values()
            .filter_map(|item| {
    ...
    assert_eq!(
        s.action_for(&s_impl_id),
        Some(ItemAction::Add),
        "PR#134 finding 1: orphan Impl with external for_ must inherit the owning \
         TypeAlias's action (Add) via impl_owner_map fallback"
    );
}
