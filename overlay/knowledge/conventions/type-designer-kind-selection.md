...
- 依存注入を必要としない (依存があるなら `role: Interactor` / `role: UseCase` / `role: SecondaryAdapter`)
...
例外: トレイト境界に組み込む必要がある (`Arc<dyn Service>` で渡したい等) 場合は `role: Interactor` + `role: ApplicationService` のペアを使う。`FreeFunction` は trait 境界に組み込めない。
...
`role: ValueObject` で迷ったときの最も多い真の答えは `role: FreeFunction` (R2) である。次に多いのは `role: Interactor` (依存あり) / `role: SecondaryAdapter` (port 実装) / `role: DomainService` (R6: field を持つ domain behavior) である。`role: ValueObject` を選ぶ前に、これらの候補を必ず検討する。
...
- `PostgresUserRepository` を `role: UseCase` で `infrastructure-types.json` に起草する (R1 違反: `UseCase` は usecase ONLY)
  - 正しい修正: usecase 層に `role: Interactor` + `role: ApplicationService` のペアを置き、infrastructure には `role: SecondaryAdapter` を置く
