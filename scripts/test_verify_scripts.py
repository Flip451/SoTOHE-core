...
class VerifyScriptsTest(unittest.TestCase):
    ...
    def test_human_onboarding_doc_exists_and_is_wired(self) -> None:
        ...
        self.assertIn("track/workflow.md", onboarding)
        self.assertIn("レビューや運用判断が必要なとき", onboarding)
        self.assertIn("Task Workflow", onboarding)
        self.assertIn("track/registry.md 更新ルール", onboarding)
        self.assertIn("Quality Gates", onboarding)
